"""
Session-scoped context variables for the NasTech gateway.

Replaces the previous ``os.environ``-based session state
(``NASTECH_SESSION_PLATFORM``, ``NASTECH_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["NASTECH_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("NASTECH_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("NASTECH_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("NASTECH_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("NASTECH_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("NASTECH_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("NASTECH_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("NASTECH_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("NASTECH_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("NASTECH_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("NASTECH_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("NASTECH_SESSION_MESSAGE_ID", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("NASTECH_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
...
_VAR_MAP = {
    ...
    "NASTECH_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "NASTECH_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "NASTECH_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "NASTECH_SESSION_USER_ID": _SESSION_USER_ID,
    "NASTECH_SESSION_USER_NAME": _SESSION_USER_NAME,
    "NASTECH_SESSION_KEY": _SESSION_KEY,
    "NASTECH_SESSION_ID": _SESSION_ID,
    "NASTECH_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "NASTECH_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "NASTECH_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "NASTECH_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``NASTECH_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("NASTECH_SESSION_ID")`` with an ``os.environ`` fallback,
...
def get_session_env(name: str, default: str = "") -> str:
    ...
    Drop-in replacement for ``os.getenv("NASTECH_SESSION_*", default)``.
