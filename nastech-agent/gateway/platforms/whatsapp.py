...
class WhatsAppAdapter(BasePlatformAdapter):
    ...
    DEFAULT_REPLY_PREFIX = "⚕ *NasTech Agent*\n────────────\n"

    # Default bridge location relative to the nastech-agent install
    _DEFAULT_BRIDGE_DIR = Path(__file__).resolve(
    ).parents[2] / "scripts" / "whatsapp-bridge"
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_nastech_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
        self._reply_prefix: Optional[str] = config.extra.get("reply_prefix")
        self._dm_policy = str(
            config.extra.get("dm_policy") or os.getenv(
                "WHATSAPP_DM_POLICY",
                "open")).strip().lower()
        self._allow_from = self._coerce_allow_list(
            config.extra.get("allow_from") or config.extra.get("allowFrom"))
        self._group_policy = str(
            config.extra.get("group_policy") or os.getenv(
    ...
    def _compile_mention_patterns(self):
        ...
        if compiled:
            logger.info(
                "[%s] Loaded %d WhatsApp mention pattern(s)",
                self.name,
                len(compiled))
        return compiled
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            logger.warning(
                "[%s] Node.js not found. WhatsApp requires Node.js.",
                self.name)
            self._set_fatal_error(
                "whatsapp_node_missing",
                "Node.js is not installed — install Node.js and re-run `nastech gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run nastech whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `nastech whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `nastech whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            logger.warning(
                "[%s] Could not acquire session lock (non-fatal): %s",
                self.name,
                e)
        ...
        try:
            ...
            if not (bridge_dir / "node_modules").exists():
                ...
                try:
                    ...
                    npm_install_timeout = int(os.environ.get(
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    if self._bridge_process.poll() is not None:
                        print(
                            f"[{self.name}] Bridge process died during connection")
                        print(f"[{self.name}] Check log: {self._bridge_log}")
                        self._close_bridge_log()
                        return False
                    try:
                    ...
                    print(
                        f"[{self.name}]   If session expired, re-pair: nastech whatsapp")
    ...
    async def _check_managed_bridge_exit(self) -> Optional[str]:
        ...
        if getattr(self, "_shutting_down",
                   False) and returncode in {0, -2, -15}:
            logger.info(
            ...
            try:
                try:
                    _terminate_bridge_process(
                        self._bridge_process, force=False)
                ...
                if self._bridge_process.poll() is None:
                    try:
                        _terminate_bridge_process(
                            self._bridge_process, force=True)
                    except (ProcessLookupError, PermissionError):
    ...
    async def send(
        ...
        try:
            ...
            chunks = self.truncate_message(
                formatted, self._outgoing_chunk_limit())
    ...
    async def _build_message_event(
            self, data: Dict[str, Any]) -> Optional[MessageEvent]:
        ...
        try:
            ...
            for url in raw_urls:
                if msg_type == MessageType.PHOTO and url.startswith(
                    ...
                    try:
                        ...
                        print(
                            f"[{self.name}] Cached user image: {cached_path}", flush=True)
                    except Exception as e:
                        print(
                            f"[{self.name}] Failed to cache image: {e}", flush=True)
                    ...
                    media_types.append("image/jpeg")
                    print(
                        f"[{self.name}] Using bridge-cached image: {url}", flush=True)
                    ...
                    try:
                        ...
                        print(
                            f"[{self.name}] Cached user voice: {cached_path}", flush=True)
                    except Exception as e:
                        print(
                            f"[{self.name}] Failed to cache voice: {e}", flush=True)
                        cached_urls.append(url)
                        media_types.append("audio/ogg")
                    ...
                    media_types.append("audio/ogg")
                    print(
                        f"[{self.name}] Using bridge-cached audio: {url}", flush=True)
                    ...
                    print(
                        f"[{self.name}] Using bridge-cached video: {url}", flush=True)
                else:
                    cached_urls.append(url)
            ...
            if msg_type == MessageType.DOCUMENT and cached_urls:
                for doc_path in cached_urls:
                    ...
                    if ext in {".txt", ".md", ".csv", ".json", ".xml", ".yaml",
                               ".yml", ".log", ".py", ".js", ".ts", ".html", ".css"}:
                        try:
                            file_size = Path(doc_path).stat().st_size
                            if file_size > MAX_TEXT_INJECT_BYTES:
                                print(
                                    f"[{self.name}] Skipping text injection for {doc_path} ({file_size} bytes > {MAX_TEXT_INJECT_BYTES})", flush=True)
                                continue
                            ...
                            if body:
                                ...
                                body = injection
                            print(
                                f"[{self.name}] Injected text content from: {doc_path}", flush=True)
                        except Exception as e:
                            print(
                                f"[{self.name}] Failed to read document text: {e}", flush=True)
