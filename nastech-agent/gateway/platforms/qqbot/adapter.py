...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.nastech/.update_response. Set here so the cross-adapter gateway
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "nastech-agent",
                    "$device": "nastech-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.nastech/.update_response`` for the
          detached ``nastech update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``nastech update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``nastech update --gateway`` watcher. Button
        ...
        ``~/.nastech/.update_response`` so the detached update process
        can read it.
        """
