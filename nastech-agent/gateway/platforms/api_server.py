...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-NasTech-Session-Id header; opt-in long-term memory scoping via X-NasTech-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-NasTech-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists nastech-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible NasTech sessions
- POST /api/sessions               — create an empty NasTech session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to nastech-agent
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES,
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_nastech_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted NASTECH_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # nastech_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same NasTech session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through nastech-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-NasTech-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-NasTech-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-NasTech-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-NasTech-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-NasTech-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``nastech sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the nastech-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-NasTech-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return nastech-agent as an available model."""
    ...
    async def _handle_capabilities(
        ...
        every NasTech version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side NasTech AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-NasTech-Session-Id",
                "session_key_header": "X-NasTech-Session-Key",
    ...
    async def _handle_session_chat(
        ...
        return web.json_response(
            {
                "object": "nastech.session.chat.completion",
    ...
    async def _handle_session_chat_stream(
        ...
        headers = {
            ...
            "X-NasTech-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-NasTech-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(
        ...
        # stable per-channel identifier via X-NasTech-Session-Key.  This
        # is independent of X-NasTech-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-NasTech-Session-Id.
        ...
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-NasTech-Session-Id rejected: "
            ...
            # conversation map to the same NasTech session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``nastech.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-NasTech-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-NasTech-Session-Key"] = gateway_session_key
        ...
        # truncation via finish_reason="length" + NasTech-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-NasTech-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-NasTech-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-NasTech-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-NasTech-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: nastech.tool.progress`` SSE event so
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-NasTech-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-NasTech-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(
        ...
        return web.json_response({
            "object": "nastech.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
