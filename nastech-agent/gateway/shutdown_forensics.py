...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        nastech_home_str = os.environ.get("NASTECH_HOME")
        if nastech_home_str:
            takeover_path = Path(nastech_home_str) / ".gateway-takeover.json"
...
def check_systemd_timing_alignment(
    ...
    upgraded nastech-agent but never re-ran ``nastech setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../nastech-gateway.service"
    ...
    # that's the common case for nastech.
    ...
    # constant in nastech_cli/gateway.py.
    headroom = 30.0
