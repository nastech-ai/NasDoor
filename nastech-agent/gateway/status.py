...
The PID file lives at ``{NASTECH_HOME}/gateway.pid``.  NASTECH_HOME defaults to
``~/.nastech`` but can be overridden via the environment variable.  This means
separate NASTECH_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting NASTECH_HOME."""
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the NasTech gateway."""
...
def acquire_scoped_lock(scope: str, identity: str,
    ...
    at once (e.g. the same Telegram bot token across different NASTECH_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``nastech.service`` + ``nastech-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``nastech gateway stop`` on Windows would be misclassified as an
