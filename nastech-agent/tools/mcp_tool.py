...
transport, discovers their tools, and registers them into the nastech-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.nastech/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.nastech/logs/mcp-stderr.log), tagged with the
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → NasTech MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that NasTech gateways know how to render.
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'nastech-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'nastech-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until NasTech is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `nastech mcp login {server_name}` (or delete the tokens "
            f"file under ~/.nastech/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the NasTech config file.
    ...
    ``os.environ`` (which includes ``~/.nastech/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through NasTech' MEDIA: tag convention and out to messaging
            ...
            # NasTech' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves NasTech's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the NasTech registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``nastech tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the NasTech registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple nastech
