...
(``nastech-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``nastech tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # Piper: fully local neural TTS (VITS), 44 languages, no API key.
    # piper-tts ships cross-platform wheels (Linux/macOS/Windows,
    # x86_64 + ARM64) with embedded espeak-ng — no system deps needed.
    # Voices (.onnx + .onnx.json) are downloaded on first use via
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``nastech update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``nastech update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``nastech update``. Never raises; lazy-install failures
