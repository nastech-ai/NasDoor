...
* ``XAI_API_KEY`` is set in ``~/.nastech/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``nastech auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
def check_x_search_requirements() -> bool:
    ...
    :func:`nastech_cli.auth.resolve_xai_oauth_runtime_credentials` which
