...
def _construct_aggregator_prompt(
        system_prompt: str, responses: List[str]) -> str:
    """
...
async def _run_reference_model_safe(
    ...
    for attempt in range(max_retries):
        try:
            ...
            # terminal failure paths so long-running MoA retries don't flood
            # logs.
            if "invalid" in error_str.lower():
                ...
                logger.warning(
                    "%s unknown error (attempt %s): %s",
                    model,
                    attempt + 1,
                    error_str)

            if attempt < max_retries - 1:
                # Exponential backoff for rate limiting: 2s, 4s, 8s, 16s, 32s,
                # 60s
...
async def mixture_of_agents_tool(
    ...
    try:
        ...
        # Layer 1: Generate diverse responses from reference models (with
        # failure handling)
        logger.info("Layer 1: Generating reference responses...")
        model_results = await asyncio.gather(*[
            _run_reference_model_safe(
                model, user_prompt, REFERENCE_TEMPERATURE)
        ...
        if successful_count < MIN_SUCCESSFUL_REFERENCES:
            raise ValueError(
                f"Insufficient successful reference models ({successful_count}/{
                    len(ref_models)}). Need at least {MIN_SUCCESSFUL_REFERENCES} successful responses.")
        ...
        logger.info(
            "MoA processing completed in %.2f seconds",
            processing_time)

        # Prepare successful response (only final aggregated result, minimal
        # fields)
        result = {
            "success": True,
