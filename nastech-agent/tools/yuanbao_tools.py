...
提供以下工具函数，供 nastech-agent 的 "nastech-yuanbao" toolset 使用：
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（NASTECH_SESSION_CHAT_ID）。
...
def _check_yuanbao():
    ...
    try:
        from gateway.session_context import get_session_env
        if get_session_env("NASTECH_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
