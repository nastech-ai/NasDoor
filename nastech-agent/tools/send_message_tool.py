...
# Discord snowflake IDs are numeric, same regex pattern as Telegram topic
# targets.
...
def _sanitize_error_text(text) -> str:
    ...
    redacted = _GENERIC_SECRET_ASSIGN_RE.sub(
...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(
                    f"Platform '{platform_name}' is not configured. Set up credentials in ~/.nastech/config.yaml or environment variables.")
        else:
            return tool_error(
                f"Platform '{platform_name}' is not configured. Set up credentials in ~/.nastech/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: nastech config set {home_env} <channel_id>"
            })
    ...
    try:
        ...
        if used_home_channel and isinstance(
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env(
                    "NASTECH_SESSION_PLATFORM", "cli")
                user_id = get_session_env(
                    "NASTECH_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    platform = get_session_env(
        ...
        "").strip().lower()
    chat_id = get_session_env("NASTECH_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
...
async def _send_via_adapter(
    ...
    *,
    ...
    """
    platform_name = platform.value if hasattr(
        platform, "value") else str(platform)
    runner = None
    try:
        from gateway.run import _gateway_runner_ref
        runner = _gateway_runner_ref()
    ...
    if entry is not None and entry.standalone_sender_fn is not None:
        try:
            ...
            logger.debug(
                "Plugin standalone send for %s raised",
                platform_name,
                exc_info=True)
...
async def _send_to_platform(platform, pconfig, chat_id, message,
    ...
    if max_len:
        ...
        chunks = BasePlatformAdapter.truncate_message(
            message, max_len, len_fn=_len_fn)
    else:
        chunks = [message]
    ...
    if warning and isinstance(
            last_result, dict) and last_result.get("success"):
...
async def _send_telegram(token, chat_id, message, media_files=None,
    ...
    try:
        ...
        if _has_html:
            ...
            # Reuse the gateway adapter's format_message for
            # markdown→MarkdownV2
            try:
                from gateway.platforms.telegram import TelegramAdapter
                ...
                with open(media_path, "rb") as f:
                    ...
                    try:
                        ...
                        if _is_telegram_thread_not_found(
                                media_err) and media_kwargs.get("message_thread_id"):
            ...
            async with session.post(url, headers=headers, json=payload, **_req_kw) as resp:
                ...
                if data.get("ok"):
                    return {"success": True, "platform": "slack",
                            "chat_id": chat_id, "message_id": data.get("ts")}
                return _error(
    ...
    try:
        ...
        for media_path, _is_voice in valid_media:
            if os.path.exists(media_path):
                ...
                logger.warning(
                    "Signal media file not found, skipping: %s",
                    media_path)
            ...
            if n > 0:
                ...
                if estimated >= SIGNAL_BATCH_PACING_NOTICE_THRESHOLD:
                    await _send_inline_notice(
                        f"(More images coming — pausing ~{
                            _format_wait(estimated)} "
                        f"for Signal rate limit, batch {
                            idx + 1}/{
                            len(att_batches)}.)"
                    )
            ...
            for attempt in range(1, SIGNAL_RATE_LIMIT_MAX_ATTEMPTS + 1):
                try:
                    ...
                    logger.warning(
                        ...
                        idx +
                        1, len(
                            att_batches), attempt, SIGNAL_RATE_LIMIT_MAX_ATTEMPTS, str(e)
                    )

        warnings = []
        if len(attachment_paths) < len(valid_media):
            warnings.append(
                "Some media files were skipped (not found on disk)")
        if failed_batches:
            warnings.append(
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30), **_sess_kw) as session:
            ...
            async with session.post(url, data=form_data, headers=headers, **_req_kw) as resp:
                ...
                return {"success": True, "platform": "sms",
                        "chat_id": chat_id, "message_id": msg_sid}
    except Exception as e:
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        homeserver = (
            extra.get("homeserver") or os.getenv(
                "MATRIX_HOMESERVER",
                "")).rstrip("/")
        ...
        txn_id = f"nastech_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
        ...
        return {"success": True, "platform": "matrix",
                "chat_id": chat_id, "message_id": data.get("event_id")}
    except Exception as e:
...
async def _send_dingtalk(extra, chat_id, message):
    ...
    try:
        ...
        return {"error": "httpx not installed"}
    try:
        webhook_url = extra.get("webhook_url") or os.getenv(
...
async def _send_bluebubbles(extra, chat_id, message):
    ...
    try:
        ...
        if not check_bluebubbles_requirements():
            return {
                "error": "BlueBubbles requirements not met (need aiohttp + httpx)."}
    except ImportError:
    ...
    try:
        ...
        try:
            ...
            return {"success": True, "platform": "bluebubbles",
                    "chat_id": chat_id, "message_id": result.message_id}
        finally:
            await adapter.disconnect()
    except Exception as e:
...
async def _send_feishu(pconfig, chat_id, message,
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {
                "error": "Feishu dependencies not installed. Run: pip install 'nastech-agent[feishu]'"}
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``NASTECH_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``NASTECH_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
async def _send_qqbot(pconfig, chat_id, message):
    ...
    try:
        ...
        return _error(
            "QQBot direct send requires httpx. Run: pip install httpx")
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if token_resp.status_code != 200:
                return _error(
                    f"QQBot token request failed: {token_resp.status_code}")
