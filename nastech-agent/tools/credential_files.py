...
def _resolve_nastech_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.nastech",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``NASTECH_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside NASTECH_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    nastech_home = _resolve_nastech_home()

    # Reject absolute paths — they bypass the NASTECH_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to NASTECH_HOME)",
            relative_path,
        )
        return False

    host_path = nastech_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape NASTECH_HOME.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.nastech/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="nastech-skills-safe-"))
...
def get_cache_directory_mounts(
    ...
    ``get_nastech_dir()`` for backward compatibility with old directory layouts.
    """
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.nastech``,
    SSH ``<remote_home>/.nastech``, etc.) and whether translation is wanted.
