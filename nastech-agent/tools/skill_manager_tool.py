...
~/.nastech/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `nastech config set skills.guard_agent_created true`.
    """
    try:
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`nastech curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.nastech/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER NasTech profiles.
    ...
    # All named profiles (~/.nastech/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`nastech -p "
            ...
            base += (
                ...
                f"Switch profiles (`nastech -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `nastech curator unpin <name>`. "
