...
def _shared_strings(zf: zipfile.ZipFile, names: set[str]) -> list[str]:
    ...
    try:
        ...
        return []
    s = f"{{{_NS_S}}}"
    return ["".join(t.text or "" for t in item.iter(
        f"{s}t")) for item in root.iter(f"{s}si")]
...
def _workbook_sheets(zf: zipfile.ZipFile) -> list[tuple[str, str, str]]:
    ...
    return [
        (sheet.get("name", "Sheet"), sheet.get(
            "state", "visible"), sheet.get(f"{r}id", ""))
...
def _sheet_rows(xml_bytes: bytes, shared: list[str]) -> list[list[str]]:
    ...
    for row in root.iter(f"{s}row"):
        ...
        for cell in row.iter(f"{s}c"):
            col = _col_index(cell.get("r", "")) if cell.get(
                "r") else max_col + 1
            if col >= _MAX_XLSX_COLS:
                continue
            cells[col] = _cell_value(cell, shared, s)
            max_col = max(max_col, col)
        rows.append([cells.get(i, "")
                    for i in range(max_col + 1)] if max_col >= 0 else [])
...
def _cell_value(cell: ET.Element, shared: list[str], s: str) -> str:
    ...
    if typ == "inlineStr":
        ...
        return "" if inline is None else "".join(
            t.text or "" for t in inline.iter(f"{s}t"))
    if typ == "b":
