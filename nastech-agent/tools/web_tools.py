...
Backend is selected during ``nastech tools`` setup (web.backend in config.yaml).
When available, NasTech can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via NasTech config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through NasTech' config/.env layer (``nastech config set``, ``nastech tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nastech/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nastech tools``).
    Falls back to whichever API key is present for users who configured
    keys manually without running setup.
    """
    configured = (_load_web_config().get("backend") or "").lower().strip()
    if configured in {"parallel", "firecrawl", "tavily",
                      "exa", "searxng", "brave-free", "ddgs", "xai"}:
        return configured
    ...
    backend_candidates = (
        ("firecrawl", _has_env("FIRECRAWL_API_KEY") or _has_env(
            "FIRECRAWL_API_URL") or _is_tool_gateway_ready()),
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `nastech tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_nastech_tools_enabled()`` only saved
...
def _is_nastech_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is Nous Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "nastechai.com" or host.endswith(".nastechai.com")


def _resolve_web_extract_auxiliary(
        model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nastech_auxiliary_client(client):
...
async def process_content_with_llm(
    ...
    try:
        ...
        if content_len > MAX_CONTENT_SIZE:
            ...
            logger.warning(
                "Content too large (%.1fMB > 2MB limit). Refusing to process.",
                size_mb)
        ...
        if content_len > CHUNK_THRESHOLD:
            logger.info(
                "Content large (%d chars). Using chunked processing...",
                content_len)
...
async def _process_large_content_chunked(
    ...
    logger.info(
        "Split into %d chunks of ~%d chars each",
        len(chunks),
        chunk_size)

    # Summarize each chunk in parallel
    async def summarize_chunk(
            chunk_idx: int, chunk_content: str) -> tuple[int, Optional[str]]:
...
Create a single, unified markdown summary."""
    ...
    try:
        ...
        if aux_client is None or not effective_model:
            logger.warning(
                "No auxiliary model for synthesis, concatenating summaries")
            fallback = "\n\n".join(summaries)
        ...
        if not final_summary:
            logger.warning(
                "Synthesis LLM returned empty content, retrying once")
            response = await async_call_llm(**call_kwargs)
            final_summary = extract_content_or_reasoning(response)
        ...
        logger.info("Synthesis complete: %d -> %d chars (%.2f%%)",
                    original_len, final_len, compression * 100)
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `nastech tools` to set one up."
                ),
            }
        else:
            logger.info(
    ...
    try:
        ...
        if use_llm_processing and auxiliary_available:
            ...
            for result_item in processed_results:
                if isinstance(result_item, BaseException):
                    logger.warning(
                        "Web result processing task failed: %s", result_item)
                    continue
            ...
            # Print summary of extracted pages for debugging (original
            # behavior)
        ...
        if trimmed_response.get("results") == []:
            ...
            result_json = json.dumps(
                trimmed_response, indent=2, ensure_ascii=False)
...
def check_web_api_key() -> bool:
    ...
    if configured in {"exa", "parallel", "firecrawl",
                      "tavily", "searxng", "brave-free", "ddgs", "xai"}:
...
if __name__ == "__main__":
    ...
    nastech_available = check_auxiliary_model()
    ...
    if web_available:
        ...
        if backend == "exa":
            ...
            print(
                f"   Using SearXNG (search only): {
                    _env_value('SEARXNG_URL')}")
    ...
    if nastech_available:
        print(
            f"🧠 LLM content processing available with {default_summarizer_model}")
        print(
    ...
    if _debug.active:
        ...
        print(
            f"   Debug logs will be saved to: {
                _debug.log_dir}/web_tools_debug_{
                _debug.session_id}.json")
    else:
    ...
    if nastech_available:
        ...
        print(
            "  content = await web_extract_tool(['https://python.org/about/'])")
        print("")
