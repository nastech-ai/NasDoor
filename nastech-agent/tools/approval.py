...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("NASTECH_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from nastech_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NASTECH_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("NASTECH_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("NASTECH_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set NASTECH_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind NASTECH_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds NASTECH_SESSION_PLATFORM via
...
# active profile home path such as /home/nastech/.nastech/config.yaml. The
# resolved-absolute form is folded into the ~/.nastech/ patterns at detection
...
# go stale when NASTECH_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NASTECH_ENV_PATH = (
    r'(?:~\/\.nastech/|'
    r'(?:\$home|\$\{home\})/\.nastech/|'
    r'(?:\$nastech_home|\$\{nastech_home\})/)'
    r'\.env\b'
)
# ~/.nastech/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _NASTECH_ENV_PATH; matches the NASTECH_HOME override form as
# well as ~/.nastech/.
_NASTECH_CONFIG_PATH = (
    r'(?:~\/\.nastech/|'
    r'(?:\$home|\$\{home\})/\.nastech/|'
    r'(?:\$nastech_home|\$\{nastech_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_NASTECH_ENV_PATH}|'
    rf'{_NASTECH_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.nastech/ form so the NasTech config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/nastech/.nastech/config.yaml`) rather than
    # ~, $HOME, or $NASTECH_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live NASTECH_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_nastech_home(command)
    return command


def _rewrite_resolved_nastech_home(command: str) -> str:
    """Rewrite the resolved absolute NasTech home prefix to ``~/.nastech/``.

    Resolves the active ``NASTECH_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.nastech/`` so the static ``_NASTECH_CONFIG_PATH`` / ``_NASTECH_ENV_PATH``
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate NASTECH_HOME (e.g. "/" or "") rewriting
        ...
        # /home/nastech/.nastech or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.nastech/")
    return command
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NASTECH_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set NASTECH_INTERACTIVE or NASTECH_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("NASTECH_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NASTECH_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("NASTECH_EXEC_ASK")
