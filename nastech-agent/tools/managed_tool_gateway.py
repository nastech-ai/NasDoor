...
def _read_nastech_provider_state() -> Optional[dict]:
    try:
        ...
        if not isinstance(providers, dict):
            return None
        nastech_provider = providers.get("nous", {})
        if isinstance(nastech_provider, dict):
...
def peek_nastech_access_token() -> Optional[str]:
    ...
    that call :func:`read_nastech_access_token`.
    """
    ...
    access_token = nastech_provider.get("access_token")
...
def read_nastech_access_token() -> Optional[str]:
    ...
    if cached_token and not _access_token_is_expiring(
        nastech_provider.get("expires_at"),
        _NASTECH_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token
    ...
    try:
        ...
        refreshed_token = resolve_nastech_access_token(
            refresh_skew_seconds=_NASTECH_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_nastech_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    nastech_user_token = resolved_token_reader()
    ...
    return ManagedToolGatewayConfig(
        ...
        nastech_user_token=nastech_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    Defaults to :func:`peek_nastech_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_nastech_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_nastech_access_token,
    ) is not None
