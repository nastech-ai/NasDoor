...
`nastech skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.nastech.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nastech.blueprint`` is a mapping containing
    ...
    blueprint = nastech.get("blueprint") if isinstance(nastech, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.nastech.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *,
    ...
    and emit a SKILL.md (with a ``metadata.nastech.blueprint`` block) they can hand
    to ``nastech skills publish`` to share. ``body`` is the plain-language
