...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``nastech tools`` repaint, tool-registration scans,
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.nastech/.env`` first, then ``os.environ``.

    Wraps :func:`nastech_cli.config.get_env_value` so tests can patch
