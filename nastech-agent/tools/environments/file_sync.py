...
def iter_sync_files(
    ...
    the hardcoded /root/.nastech to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``NASTECH_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, nastech_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.nastech/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="nastech-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.nastech/skills/a.md`` →
        ``~/.nastech/skills/a.md``, a new remote file at
        ``/root/.nastech/skills/b.md`` maps to ``~/.nastech/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
