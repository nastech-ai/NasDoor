...
# Bedrock-specific bearer token, which is a NasTech inference secret exactly
...
def _make_run_env(env: dict) -> dict:
    ...
    # NasTech is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    _inject_context_nastech_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {NASTECH_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, NasTech trusts them.
    """
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = get_nastech_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "nastech_terminal"
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_nastech_pgid", None)
                    if pgid is None:
                        raise
