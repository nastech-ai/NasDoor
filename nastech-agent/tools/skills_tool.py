...
# All skills live in ~/.nastech/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from NASTECH_HOME/.env."""
    env_path = get_nastech_home() / ".env"
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("NASTECH_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_nastech_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.nastech/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``NASTECH_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``NASTECH_PLATFORM`` environment variable
    3. ``NASTECH_SESSION_PLATFORM`` from gateway session context
    """
    try:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.nastech/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``nastech skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_nastech_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from nastech_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: nastech plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `nastech skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
