"""Cross-session rate limit guard for NasTech Portal.  # noqa: D400

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether NasTech Portal is currently rate-limited
...
def _state_path() -> str:
    ...
    try:
        ...
        base = os.path.join(os.path.expanduser("~"), ".nastech")
...
def record_nous_rate_limit(
    *,
    ...
    """Record that NasTech Portal is rate-limited.
...
def nous_rate_limit_remaining() -> Optional[float]:
    """Check if NasTech Portal is currently rate-limited.
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from NasTech Portal is a real account rate limit.

    NasTech Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
