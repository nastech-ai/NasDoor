...
from nastech_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # The NasTech Docker image runs s6-overlay as PID 1 (/init). s6 plants
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``NASTECH_PLATFORM`` or
            ``NASTECH_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``nastech`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.nastech/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``nastech`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.nastech/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(nastech, dict):
        nastech = {}
    return {
        "fallback_for_toolsets": nastech.get("fallback_for_toolsets", []),
        "requires_toolsets": nastech.get("requires_toolsets", []),
        "fallback_for_tools": nastech.get("fallback_for_tools", []),
        "requires_tools": nastech.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    nastech = metadata.get("nastech")
    if not isinstance(nastech, dict):
        return []
    raw = nastech.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes NasTech metadata, VCS, virtualenv/dependency, and cache
