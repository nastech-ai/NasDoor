...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`nastech_cli.plugins.invoke_hook` and its aggregators.  Python
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``nastech_cli.config.load_config``
...
def allowlist_path() -> Path:
    ...
    return get_nastech_home() / ALLOWLIST_FILENAME
...
# ---------------------------------------------------------------------------
# Introspection (used by `nastech hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``nastech hooks test`` and ``nastech hooks doctor``.

    ``kwargs`` is the same dict that :func:`nastech_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``nastech hooks test`` could
