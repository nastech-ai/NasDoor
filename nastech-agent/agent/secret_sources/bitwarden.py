...
NasTech pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.nastech/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<nastech_home>/bin/bws`` on
  first use.  NasTech pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.nastech/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``nastech`` invocations don't
  hammer the API.
* Failures NEVER block NasTech startup.  Missing binary, no network,
...
# <nastech_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.nastech/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under nastech_home/cache/.

    `home_path` is what `load_nastech_dotenv()` already resolved; falling back
    to `$NASTECH_HOME` / `~/.nastech` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("NASTECH_HOME", Path.home() / ".nastech"))
...
def _nastech_bin_dir() -> Path:
    """Where NasTech stores its managed binaries.  Profile-aware."""
    from nastech_constants import get_nastech_home

    return get_nastech_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<nastech_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _nastech_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``nastech secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _nastech_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="nastech-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "nastech-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<nastech_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from nastech_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_nastech_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `nastech secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `nastech secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `nastech secrets bitwarden setup` to install."
        )
        return result
