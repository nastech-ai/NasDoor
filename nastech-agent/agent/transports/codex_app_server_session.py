...
Owns one Codex thread per NasTech session. Drives `turn/start`, consumes
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse NasTech/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per NasTech session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NASTECH_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NASTECH_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but NasTech' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into NasTech' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own nastech-tools callback we
            # auto-accept — the user already approved NasTech' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map NasTech approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    NasTech returns 'once', 'session', 'always', or 'deny'.
...
def _get_nastech_version() -> str:
    """Best-effort NasTech version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("nastech-agent")
