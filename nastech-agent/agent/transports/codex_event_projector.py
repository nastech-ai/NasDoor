"""Projects codex app-server events into NasTech' messages list.

The translator that lets NasTech' memory/skill review keep working under the
...
NasTech' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape NasTech
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way NasTech' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but NasTech stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way NasTech already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — NasTech' messages store text only.
        text_parts: list[str] = []
