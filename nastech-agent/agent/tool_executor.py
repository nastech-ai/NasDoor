...
def _apply_tool_request_middleware_for_agent(
    ...
    *,
    ...
    try:
        ...
        payload = result.payload if isinstance(
            result.payload, dict) else function_args
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    def _execute(next_args: dict) -> Any:
        ...
        observed_args = next_args if isinstance(
...
def execute_tool_calls_concurrent(
        agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    if agent._interrupt_requested:
        ...
        for tc in tool_calls:
            messages.append(make_tool_result_message(
                tc.function.name,
                f"[Tool execution cancelled — {
                    tc.function.name} was skipped due to user interrupt]",
                tc.id,
            ))
        return
    ...
    # block_result, blocked_by_guardrail)
    parsed_calls = []
    for tool_call in tool_calls:
                ...
                if not guardrail_decision.allows_execution:
                    ...
                    _emit_terminal_post_tool_call(
                        ...
                        error_message=getattr(
                            guardrail_decision,
                            "message",
                            None) or "Tool blocked by guardrail policy",
                        middleware_trace=list(middleware_trace),
                    )
        ...
        if block_result is None:
            ...
            if function_name in {"write_file",
                                 "patch"} and agent._checkpoint_mgr.enabled:
                try:
    ...
    if not agent.quiet_mode:
        ...
        for i, (tc, name, args, middleware_trace, block_result,
                blocked_by_guardrail) in enumerate(parsed_calls, 1):
            ...
            if agent.verbose_logging:
                ...
                args_preview = args_str[:agent.log_prefix_chars] + \
                    "..." if len(
                        args_str) > agent.log_prefix_chars else args_str
                print(
    ...
    # Each slot holds (function_name, function_args, function_result,
    # duration, error_flag, blocked_flag, middleware_trace)
    results = [None] * num_tools
    for i, (tc, name, args, middleware_trace, block_result,
            blocked_by_guardrail) in enumerate(parsed_calls):
        if block_result is not None:
            results[i] = (
                name,
                args,
    ...
    def _run_tool(index, tool_call, function_name,
        ...
        try:
            ...
            if is_error:
                ...
                logger.info("tool %s completed (%.2fs, %d chars)",
                            function_name, duration, len(result))
            results[index] = (
                function_name,
                function_args,
                result,
        ...
        if runnable_calls:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for i, tc, name, args in runnable_calls:
                    ...
                    # callbacks into the worker thread; clears callbacks on
                    # exit.
                    f = executor.submit(
                        propagate_context_to_thread(
                            _run_tool), i, tc, name, args, parsed_calls[i][3]
                    )
                    futures.append(f)
    ...
    for i, (tc, name, args, middleware_trace, block_result,
        ...
        if r is None:
            ...
            if is_error:
                ...
                result_preview = _err_text[:200] if len(
                    _err_text) > 200 else _err_text
                logger.warning(
            ...
            if agent.verbose_logging:
                logging.debug(
                    f"Tool {function_name} completed in {
                        tool_duration:.2f}s")
                logging.debug(
                    f"Tool result ({
                        len(function_result)} chars): {function_result}")
        ...
        if agent._should_emit_quiet_tool_messages():
            ...
            if agent.verbose_logging:
                ...
                response_preview = _preview_str[:agent.log_prefix_chars] + "..." if len(
                    _preview_str) > agent.log_prefix_chars else _preview_str
                print(
                    f"  ✅ Tool {
                        i +
                        1} completed in {
                        tool_duration:.2f}s - {response_preview}")
        ...
        if not blocked and agent.tool_complete_callback:
            try:
                agent.tool_complete_callback(
                    tc.id, name, args, function_result)
            except Exception as cb_err:
...
def execute_tool_calls_sequential(
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if agent._interrupt_requested:
            ...
            if remaining_calls:
                agent._vprint(
                    f"{
                        agent.log_prefix}⚡ Interrupt: skipping {
                        len(remaining_calls)} tool call(s)",
                    force=True)
        ...
        if not _execution_blocked and function_name in {
                "write_file", "patch"} and agent._checkpoint_mgr.enabled:
            try:
                file_path = function_args.get("path", "")
                if file_path:
                    work_dir = agent._checkpoint_mgr.get_working_dir_for_path(
        ...
        if _block_msg is not None:
            ...
            function_result = json.dumps(
                {"error": _block_msg}, ensure_ascii=False)
            tool_duration = 0.0
            _emit_terminal_post_tool_call(
                agent,
                function_name=function_name,
            ...
            function_result = agent._guardrail_block_result(
            ...
            def _execute(next_args: dict) -> Any:
                ...
                # Bridge: notify external memory provider of built-in memory
                # writes
            ...
            if tasks_arg and isinstance(tasks_arg, list):
                spinner_label = f"🔀 delegating {
                    len(tasks_arg)} tasks · (/agents to monitor)"
            else:
        ...
        if isinstance(function_result, str):
            result_preview = function_result if agent.verbose_logging else (
                function_result[:200] if len(
                    function_result) > 200 else function_result
            )
            _result_len = len(function_result)
        else:
            # Multimodal dict result (_multimodal=True) — not sliceable as
            # string
        ...
        if agent.verbose_logging:
            ...
            logging.debug(
                f"Tool result ({
                    len(_log_result)} chars): {_log_result}")
        ...
        if not agent.quiet_mode:
            if agent.verbose_logging:
                ...
                _fr_str = function_result if isinstance(
                    function_result, str) else str(function_result)
                response_preview = _fr_str[:agent.log_prefix_chars] + \
                    "..." if len(_fr_str) > agent.log_prefix_chars else _fr_str
                print(
                    f"  ✅ Tool {i} completed in {
                        tool_duration:.2f}s - {response_preview}")

        if agent._interrupt_requested and i < len(
                assistant_message.tool_calls):
            remaining = len(assistant_message.tool_calls) - i
            agent._vprint(
                f"{agent.log_prefix}⚡ Interrupt: skipping {remaining} remaining tool call(s)", force=True)
    ...
    if num_tools_seq > 0:
        enforce_turn_budget(messages[-num_tools_seq:],
                            env=get_active_env(effective_task_id))
