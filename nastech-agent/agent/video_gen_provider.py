...
as ``kind: backend``) or ``~/.nastech/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``nastech tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NASTECH_HOME/cache/videos/``, creating parents as needed."""
    from nastech_constants import get_nastech_home

    path = get_nastech_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NASTECH_HOME/cache/videos/``.
