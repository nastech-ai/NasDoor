...
4. Store the token in `~/.nastech/.env` (or via `nastech setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.nastech/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
