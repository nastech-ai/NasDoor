...
| NasTech (Nous) | No jailbreak needed | Already uncensored — use directly |
...
5. **NasTech models don't need jailbreaking** — nastech-ai/nastech-3-* and nastech-4-* are already uncensored. Use them directly for the fastest path.
...
12. **execute_code sandbox has no env vars** — When NasTech runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.nastech/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.nastech/.env"))`
