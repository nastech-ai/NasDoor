...
NasTech Agent is an open-source AI agent framework by NasTech AI that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. NasTech works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.

What makes NasTech different:

- **Self-improving through skills** — NasTech learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
...
**Docs:** https://docs.nastech.ai/

## Quick Start

```bash
# Install
curl -fsSL https://nastech.ai/install.sh | bash
...
nastech chat [flags]
  ...
  --provider PROVIDER       Force provider (openrouter, anthropic, nastech, etc.)
...
nastech auth add PROVIDER    Add OAuth or API-key credential (e.g. nastech, openai-codex, qwen-oauth)
...
Platform docs: https://docs.nastech.ai/user-guide/messaging/
...
```
nastech auth add             Interactive credential wizard
...
nastech honcho setup/status  Honcho memory integration (requires honcho plugin)
...
authoritative list or see the [live slash commands reference](https://docs.nastech.ai/reference/slash-commands).
...
Full config reference: https://docs.nastech.ai/user-guide/configuration
...
| NasTech Portal | OAuth | `nastech auth` |
...
Full provider docs: https://docs.nastech.ai/integrations/providers
...
```bash
nastech config set approvals.mode smart       # recommended middle ground
nastech config set approvals.mode off         # bypass everything (not recommended)
```

Per-invocation bypass without changing config:
- `nastech --yolo …`
- `export NASTECH_YOLO_MODE=1`
...
User docs: https://docs.nastech.ai/user-guide/features/cron
...
- **CLI:** `nastech curator <verb>` — `status`, `run`, `pause`, `resume`,
...
User docs: https://docs.nastech.ai/user-guide/features/curator
...
User docs: https://docs.nastech.ai/user-guide/features/kanban

---

## Windows-Specific Quirks

NasTech runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `nastech config edit` writes without BOM; manual edits in
...
NasTech's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The NasTech-installed venv at
...
### Tool not available
1. `nastech tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `nastech doctor` — check config and dependencies
2. `nastech auth` — re-authenticate OAuth providers (or `nastech auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `nastech model` → GitHub Copilot.
...
### Skills not showing
1. `nastech skills list` — verify installed
2. `nastech skills config` — check platform enablement
3. Load explicitly: `/skill name` or `nastech -s name`
...
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed nastech-gateway`
...
|----------------|----------|
| Config options | `nastech config edit` or [Configuration docs](https://docs.nastech.ai/user-guide/configuration) |
| Available tools | `nastech tools list` or [Tools reference](https://docs.nastech.ai/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://docs.nastech.ai/reference/slash-commands) |
| Skills catalog | `nastech skills browse` or [Skills catalog](https://docs.nastech.ai/reference/skills-catalog) |
| Provider setup | `nastech model` or [Providers guide](https://docs.nastech.ai/integrations/providers) |
| Platform setup | `nastech gateway setup` or [Messaging docs](https://docs.nastech.ai/user-guide/messaging/) |
| MCP servers | `nastech mcp list` or [MCP guide](https://docs.nastech.ai/user-guide/features/mcp) |
| Profiles | `nastech profile list` or [Profiles docs](https://docs.nastech.ai/user-guide/profiles) |
| Cron jobs | `nastech cron list` or [Cron docs](https://docs.nastech.ai/user-guide/features/cron) |
| Memory | `nastech memory status` or [Memory docs](https://docs.nastech.ai/user-guide/features/memory) |
| Env variables | `nastech config env-path` or [Env vars reference](https://docs.nastech.ai/reference/environment-variables) |
| CLI commands | `nastech --help` or [CLI reference](https://docs.nastech.ai/reference/cli-commands) |
...
For occasional contributors and PR authors. Full developer docs: https://docs.nastech.ai/developer-guide/
...
Config: `~/.nastech/config.yaml` (settings), `~/.nastech/.env` (API keys).
...
**2. Add to `toolsets.py`** → `_NASTECH_CORE_TOOLS` list.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual list needed.

All handlers must return JSON strings. Use `get_nastech_home()` for paths, never hardcode `~/.nastech`.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `nastech_cli/commands.py`
...
- Tests auto-redirect `NASTECH_HOME` to temp dirs — never touch real `~/.nastech/`
- Run full suite before pushing any change
- Use `-o 'addopts='` to clear any baked-in pytest flags

**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The NasTech-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/nastech_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
...
- Use `get_nastech_home()` from `nastech_constants` for all paths (profile-safe)
