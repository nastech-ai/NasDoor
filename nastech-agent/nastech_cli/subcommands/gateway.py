"""``nastech gateway`` and ``nastech proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if NASTECH_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy nastech.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy NasTech gateway unit files "
            "(e.g. nastech.service) left over from older installs. Profile "
            "units (nastech-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `nastech proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
