"""``nastech dashboard`` subcommand parser.

Extracted verbatim from ``nastech_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the NasTech Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `nastech dashboard` cmdlines and SIGTERM them directly — the same
    # path `nastech update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running nastech dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running nastech dashboard processes and exit",
    )
    # Backward-compat shim: older NasTech desktop app shells (<= 0.15.x) spawn the
    # backend as `nastech dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "NasTech couldn't start" with no actionable cause. Accept and silently
    ...
    # client with Nous Portal and write the client_id into ~/.nastech/.env.
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://nastech.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
