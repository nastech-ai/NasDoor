"""
Top-level argparse construction for the nastech CLI.
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``nastech_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    nastech                        Start interactive chat
    nastech chat -q "Hello"        Single query mode
    nastech --tui                  Launch the modern TUI (or set display.interface: tui)
    nastech --cli                  Force the classic REPL (overrides display.interface: tui)
    nastech -c                     Resume the most recent session
    nastech -c "my project"        Resume a session by name (latest in lineage)
    nastech --resume <session_id>  Resume a specific session by ID
    nastech setup                  Run setup wizard
    nastech logout                 Clear stored authentication
    nastech auth add <provider>    Add a pooled credential
    nastech auth list              List pooled credentials
    nastech auth remove <p> <t>    Remove pooled credential by index, id, or label
    nastech auth reset <provider>  Clear exhaustion status for a provider
    nastech model                  Select default model
    nastech fallback [list]        Show fallback provider chain
    nastech fallback add           Add a fallback provider (same picker as `nastech model`)
    nastech fallback remove        Remove a fallback provider from the chain
    nastech config                 View configuration
    ...
    nastech logs                   View agent.log (last 50 lines)
    nastech logs -f                Follow agent.log in real time
    ...
    nastech dashboard --status     List running dashboard processes

For more help on a command:
    nastech <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="nastech",
        description="NasTech Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `nastech chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via NASTECH_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `nastech setup` or edit the file to change it."
        ),
    )
    parser.add_argument(
        "-t",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to NASTECH_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.nastech/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with NasTech Agent",
    )
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also NASTECH_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.nastech/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
