"""Per-provider model-selection wizard flows for ``nastech setup`` / ``nastech model``.

Extracted from ``nastech_cli/main.py`` as part of the god-file decomposition
campaign (``~/.nastech/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from nastech_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``nastech_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from nastech_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``nastech_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # bad paste short of re-running `nastech setup` from scratch. OpenRouter
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from nastech_cli.nastech_subscription import prompt_enable_tool_gateway
    ...
    if not free_tier:
        try:
            refreshed_creds = resolve_nastech_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _nastech_portal_url = ""
    try:
        _nastech_state = get_provider_auth_state("nous")
        if _nastech_state:
            _nastech_portal_url = _nastech_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _nastech_portal_url,
        )
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_nastech_portal_url,
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `nastech auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``nastech model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``nastech model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``nastech auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `nastech model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  NasTech will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  NasTech will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from nastech_cli.copilot_auth import copilot_device_code_login
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates NasTech turns to `copilot --acp`.")
    print("  NasTech currently starts its own ACP subprocess for each request.")
    print("  NasTech uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set NASTECH_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so nastech knows where to find it
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   NasTech typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with NasTech, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'nastech model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
