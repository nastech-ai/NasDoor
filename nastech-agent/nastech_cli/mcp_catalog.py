...
entries via ``nastech mcp catalog`` or the interactive ``nastech mcp picker``,
and install them with ``nastech mcp install <name>`` (or by toggling in the
...
def _install_root() -> Path:
    ...
    root = get_nastech_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.nastech/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.nastech/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from nastech_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `nastech mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `nastech mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `nastech mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `nastech auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`nastech auth {entry.auth.provider}` if you have not "
