"""nastech claw — OpenClaw migration commands.

Usage:
    nastech claw migrate              # Preview then migrate (always shows preview first)
    nastech claw migrate --dry-run    # Preview only, no changes
    nastech claw migrate --yes        # Skip confirmation prompt
    nastech claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    nastech claw migrate --no-backup  # Skip pre-migration snapshot
    nastech claw cleanup              # Archive leftover OpenClaw directories
    nastech claw cleanup --dry-run    # Preview what would be archived
"""
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_nastech.py script in known locations."""
...
def claw_command(args):
    """Route nastech claw subcommands."""
...
def _cmd_migrate(args):
    ...
    # migrate-nastech posture (two-phase: run once without secrets, rerun with
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.nastech/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: nastech claw migrate --yes")
            return
    ...
    # restorable with `nastech import`.  Mirrors OpenClaw's
    ...
    # 'nastech claw cleanup' separately.
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        if skipped_keys:
            ...
            print_info("  nastech config set OPENROUTER_API_KEY sk-or-v1-...")
