...
Add, remove, or reorder entries here — both `nastech setup` and
`nastech` provider-selection will pick up the change automatically.
"""
...
_NASTECH_USER_AGENT = f"nastech-cli/{_NASTECH_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nastech model`
    flow without maintaining a separate static list.
    """
    from nastech_cli.codex_models import (
...
# $NASTECH_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NASTECH_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. NasTech refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # Curated HF model list — only agentic models that map to OpenRouter
    # defaults.
    "huggingface": [
        "moonshotai/Kimi-K2.5",
        "Qwen/Qwen3.5-397B-A17B",
...
def _is_model_free(model_id: str, pricing: dict[str, dict[str, str]]) -> bool:
    ...
    if not p:
        return False
    try:
        return float(p.get("prompt", "1")) == 0 and float(
            p.get("completion", "1")) == 0
...
# ---------------------------------------------------------------------------
def is_nastech_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_nastech_models_by_tier(
    model_ids: list[str],
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older NasTech that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older NasTech that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_nastech_models_by_tier` keeps free models selectable;
    ...
    paid_block = payload.get("paidRecommendedModels") if isinstance(
        payload, dict) else None
...
def check_nastech_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        ...
        account_info = get_nastech_portal_account_info(force_fresh=force_fresh)
...
NASTECH_RECOMMENDED_MODELS_PATH = "/api/nous/recommended-models"
_NASTECH_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_nastech_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}
...
def _read_nastech_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_nastech_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_nastech_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _nastech_recommended_disk_path()
    try:
        try:
...
def fetch_nastech_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
    ...
    ``_NASTECH_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$NASTECH_HOME/cache/nastech_recommended_cache.json``) as last-known-good.
    ...
    now = time.monotonic()
    cached = _nastech_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _NASTECH_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{NASTECH_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _nastech_recommended_cache[base] = (data, now)
        _write_nastech_recommended_disk(base, data)
        return data
    ...
    disk = _read_nastech_recommended_disk(base)
    if disk:
        _nastech_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_nastech_portal_url() -> str:
    ...
    try:
        ...
        return str(DEFAULT_NASTECH_PORTAL_URL).rstrip("/")
    except Exception:
...
def get_nastech_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_nastech_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_nastech_portal_url()
    payload = fetch_nastech_recommended_models(
    ...
    if free_tier is None:
        try:
            free_tier = check_nastech_free_tier()
        except Exception:
...
# this list:  nastech model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nastech model` interactive picker
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("kimi-coding", "Kimi / Kimi Coding Plan",
                  "Kimi Coding Plan (api.kimi.com & Moonshot API)"),
    ProviderEntry("kimi-coding-cn", "Kimi / Moonshot (China)",
...
try:
    ...
    for _pp in _list_providers_for_canonical():
        if _pp.name in _canonical_slugs:
            continue
        if _pp.auth_type in {"oauth_device_code", "oauth_external",
                             "external_process", "aws_sdk", "copilot"}:
...
# special case: not a named provider
...
#
# Some vendors expose several NasTech provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `nastech model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``nastech model``, the
...
# path. The *interactive* default (GUI onboarding / ``nastech model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in nastech_cli/web_server.py and ``partition_nastech_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``nastech auth add openai-codex`` without
    ``nastech model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``nastech model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nastech-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nastech_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — nastech-agent
        # requires it and surfacing them leads to immediate runtime failures
        # when the user selects them. Ported from Kilo-Org/kilocode#9068.
        if not _openrouter_model_supports_tools(live_item):
            continue
        desc = "free" if _openrouter_model_is_free(
            live_item.get("pricing")) else ""
...
def model_ids(*, force_refresh: bool = False) -> list[str]:
    ...
    return [mid for mid, _ in fetch_openrouter_models(
        force_refresh=force_refresh)]


def get_curated_nastech_model_ids() -> list[str]:
    ...
    """
    try:
        from nastech_cli.model_catalog import get_curated_nastech_models
        remote = get_curated_nastech_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": _NASTECH_USER_AGENT,
    }
    if api_key:
...
_DEFAULT_NASTECH_INFERENCE_BASE = "https://inference-api.nastechai.com"
...
def _resolve_nastech_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        ...
        pass
    return ("", _DEFAULT_NASTECH_INFERENCE_BASE)


def get_pricing_for_provider(
        provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        api_key, base_url = _resolve_nastech_pricing_credentials()
        if base_url:
            # Nous base_url typically looks like https://inference-api.nastechai.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nastech model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nastech_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from nastech_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If the model belongs to the current provider's catalog, don't suggest
    # switching
...
def detect_provider_for_model(
    ...
    if _model_in_provider_catalog(
            name.lower(), _provider_keys(current_provider)):
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to NasTech' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nastech_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _is_openai_fast_model(model_id: Optional[str]) -> bool:
    ...
    if "codex" in base:
        return False
    return any(base.startswith(prefix)
               for prefix in _OPENAI_FAST_MODE_PREFIXES)
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether NasTech should expose the /fast toggle for this model."""
    return _is_anthropic_fast_model(
        model_id) or _is_openai_fast_model(model_id)
...
def resolve_fast_mode_overrides(
        model_id: Optional[str]) -> dict[str, Any] | None:
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nastech auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nastech/.env``.
...
def provider_model_ids(
        provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a NasTech release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a NasTech release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "xai-oauth":
        return list(_PROVIDER_MODELS.get(
            "xai-oauth", _PROVIDER_MODELS.get("xai", [])))
    ...
    if normalized == "nous":
        ...
        try:
            from nastech_cli.auth import (
                fetch_nastech_models,
                resolve_nastech_runtime_credentials,
            )
            creds = resolve_nastech_runtime_credentials()
            if creds:
                live = fetch_nastech_models(
                    api_key=creds.get(
                        "api_key", ""), inference_base_url=creds.get(
                        "base_url", ""))
                if live:
                    return live
        except Exception:
        ...
        # added Portal models still surface without a NasTech release.
        manifest_ids = get_curated_nastech_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``nastech model``
            # shows.
            ...
            try:
                ...
                if live:
                    if is_default_openai:
                        ...
                        filtered = [
                            m for m in curated if m.lower() in live_lower]
                        if filtered:
                            return filtered
...
#
# Cache strategy:
#   - One JSON file at $NASTECH_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$NASTECH_HOME/auth.json`` and external
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    try:
        ...
        # Sort: latest/largest first (opus > sonnet > haiku, higher version
        # first)
        ...
        import logging
        logging.getLogger(__name__).debug(
            "Failed to fetch Anthropic models: %s", e)
        return None
...
def get_copilot_model_context(
        model_id: str, api_key: Optional[str] = None) -> Optional[int]:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NASTECH_USER_AGENT}
...
def _lmstudio_fetch_raw_models(
    ...
    request = urllib.request.Request(
        server_root + "/api/v1/models", headers=headers)
    try:
...
def ensure_lmstudio_model_loaded(
    ...
    for inst in target_entry.get("loaded_instances") or []:
        ...
        loaded_ctx = cfg.get("context_length") if isinstance(
            cfg, dict) else None
...
def lmstudio_model_reasoning_options(
    ...
    for raw in raw_models:
        ...
        opts = reasoning.get("allowed_options") if isinstance(
...
def _fetch_github_models(
        api_key: Optional[str] = None, timeout: float = 5.0) -> Optional[list[str]]:
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: NasTech' default Claude IDs elsewhere use
...
def _github_reasoning_efforts_for_model_id(model_id: str) -> list[str]:
    ...
    if raw.startswith(
            ("openai/o1", "openai/o3", "openai/o4", "o1", "o3", "o4")):
...
def copilot_model_api_mode(
    ...
    *,
    ...
    normalized = normalize_copilot_model_id(
        model_id, catalog=catalog, api_key=api_key)
    if not normalized:
        return "chat_completions"

    # Primary: model ID pattern (matches opencode's
    # shouldUseCopilotResponsesApi)
    if _should_use_copilot_responses_api(normalized):
        return "codex_responses"

    # Secondary: check catalog for non-GPT-5 models (Claude via /v1/messages,
    # etc.)
    if catalog:
        catalog_entry = next(
...
def normalize_opencode_model_id(
        provider_id: Optional[str], model_id: Optional[str]) -> str:
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _NASTECH_USER_AGENT}
...
def fetch_api_models(
    ...
    return probe_api_models(
        api_key, base_url, timeout=timeout, api_mode=api_mode).get("models")
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "ollama_cloud_models_cache.json"
...
def _save_ollama_cloud_cache(models: list[str]) -> None:
    ...
    try:
        ...
        atomic_json_write(
            cache_path, {
                "models": models, "cached_at": time.time()}, indent=None)
    except Exception:
        pass
...
def fetch_ollama_cloud_models(
    ...
    *,
    ...
    # 4. Merge: live first, then models.dev additions (deduped,
    # order-preserving)
    ...
    *,
    ...
    if normalized == "lmstudio":
        ...
        if requested_for_lookup in set(models):
            return {"accepted": True, "persist": True,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        if api_models is not None:
            ...
            suggestions = get_close_matches(
                requested, api_models, n=3, cutoff=0.5)
            suggestion_text = ""
            if suggestions:
                suggestion_text = "\n  Similar models: " + \
        ...
        message = (
            f"Note: could not reach this custom endpoint's model listing at `{
                probe.get('probed_url')}`. "
            f"NasTech will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    # Providers with non-standard catalog validation — /v1/models probing is
    # not the right path.
    ...
    if normalized in {"minimax", "minimax-cn"}:
        try:
            ...
            catalog_models = []
        if catalog_models:
            # Case-insensitive lookup (catalog uses mixed case like
            # MiniMax-M2.7)
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so NasTech cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if normalized == "gemini":
            api_models = [
                m[len("models/"):] if isinstance(m,
                                                 str) and m.startswith("models/") else m
                for m in api_models
            ]
        if requested_for_lookup in set(api_models):
