...
* ``nastech profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nastech skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nastech profile``, not a parallel tree):

    nastech profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "nastech_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".nastech_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "nastech-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nastech_requires=str(data.get("nastech_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nastech_requires:
            out["nastech_requires"] = self.nastech_requires
        if self.author:
...
def check_nastech_requires(spec: str, current_version: str) -> None:
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nastech profile update`` can
...
def plan_install(
    ...
    check_nastech_requires(manifest.nastech_requires, nastech_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nastech).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="nastech_dist_install_") as tmp:
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nastech profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="nastech_dist_update_") as tmp:
        plan = plan_install(
