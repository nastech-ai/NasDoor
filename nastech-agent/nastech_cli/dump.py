"""
Dump command for nastech CLI.

Outputs a compact, plain-text summary of the user's NasTech setup
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.nastech_build_sha`` by the Dockerfile's
    ``NASTECH_GIT_SHA`` build-arg (see ``nastech_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``nastech dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nastech_cli.gateway import get_gateway_runtime_snapshot
...
def _cron_summary(nastech_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = nastech_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"nastech_home:      {display_nastech_home()}")
