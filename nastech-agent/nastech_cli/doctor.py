"""
Doctor command for nastech CLI.

Diagnoses issues with NasTech Agent setup.
"""
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.nastech/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("NASTECH_KANBAN_TASK"):
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches nastech_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``nastech --version`` reporting a stale version while
    ...
    """
    try:
        from nastech_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != nastech_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'nastech update', or set "
            "nastech_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-nastech and dashboard static services are up
    ...
    for static in ("main-nastech", "dashboard"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from nastech_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.nastech/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`nastech doctor --ack {hit.advisory.id}`."
                )
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'nastech setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'nastech setup' to configure API keys")
                fixed_count += 1
            else:
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'nastech config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from nastech_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.nastech/.env or run 'nastech setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'nastech setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'nastech config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
        try:
            from nastech_cli.config import check_config_version, migrate_config
        ...
        # Detect stale NASTECH_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives NASTECH_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from nastech_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("NASTECH_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"NASTECH_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `nastech setup` run)",
                )
                if should_fix:
                    if remove_env_value("NASTECH_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale NASTECH_MAX_ITERATIONS from .env "
                    ...
                    issues.append(
                        "Stale NASTECH_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'nastech doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if nastech_status.get("logged_in"):
    ...
    for subdir_name in expected_subdirs:
        subdir_path = nastech_home / subdir_name
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                "# NasTech Agent Persona\n\n"
                "<!-- Edit this file to customize how NasTech communicates. -->\n\n"
                "You are NasTech, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "nastech"
        ...
        _cmd_link = _cmd_link_dir / "nastech"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(nastech not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/nastech → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/nastech points to wrong target",
                ...
                check_fail(
                    f"{_cmd_link_display}/nastech not found",
                    "(nastech command may not work outside the venv)"
                )
                if should_fix:
    ...
    try:
        from nastech_constants import is_container as _is_container
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'nastech config set model.provider <provider>' "
    ...
    # never the legitimate source for `nastech doctor`.
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = NASTECH_HOME / "config.yaml"
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                _fail_and_issue(
                    ...
                    "No Honcho API key — run 'nastech memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run nastech memory setup)",
