...
the existing module-level functions in nastech_cli.gateway and
nastech_cli.gateway_windows directly. This protocol is a thin facade
...
``nastech gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``nastech gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged nastech
...
# in ``nastech_cli.gateway`` (systemd/launchd) and ``nastech_cli.gateway_windows``
...
# active (set via the ``nastech -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in nastech_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from nastech_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from nastech_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from nastech_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from nastech_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in nastech_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from nastech_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from nastech_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from nastech_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from nastech_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``nastech_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `nastech profile create`
# runs inside the container (Phase 4). Static services (main-nastech, dashboard)
...
# UID/GID of the in-image ``nastech`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the nastech user.
    ...
    is dispatched as the nastech user.
    ...
    with nastech ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           nastech:nastech, 0755 (parent must already exist)
    ``svc_dir/event/``                     nastech:nastech, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 nastech:nastech, 0755
    ``svc_dir/supervise/event/``           nastech:nastech, 03730
    ``svc_dir/supervise/control``          nastech:nastech, 0660    (FIFO)
    ...
    slot's supervise/ was nastech-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _NASTECH_UID, _NASTECH_GID)
        except PermissionError:
            # Running as the nastech user already — directory is nastech-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _NASTECH_UID, _NASTECH_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is nastech-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _NASTECH_UID, _NASTECH_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``nastech -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`nastech profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-nastech, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources NASTECH_HOME (and any extra env) via with-contenv —
             so e.g. ``-e NASTECH_HOME=/data/nastech`` is honored at run
          ...
          4. Drops to the nastech user and exec's
             ``nastech -p <profile> gateway run`` (or just ``nastech
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``nastech gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root NASTECH_HOME profile" (the implicit profile that exists at
        the top of $NASTECH_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``nastech_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$NASTECH_HOME/profiles/default/`` — a
        ...
        ``nastech_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        if profile == "default":
            gateway_cmd = "nastech gateway run"
        else:
            gateway_cmd = f"nastech -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid nastech {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${NASTECH_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the NASTECH_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e NASTECH_HOME=/data/nastech``
        gets its logs under /data/nastech/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``nastech logs`` reads and what
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # the nastech user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is nastech-owned (see ``_seed_supervise_skeleton``), and on
