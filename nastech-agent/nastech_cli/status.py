"""
Status command for nastech CLI.

Shows the status of all NasTech Agent components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``nastech config``'s
...
def show_status(args):
    ...
    nastech_account_info = None
    if (
        nastech_status.get("logged_in")
        or nastech_status.get("access_token")
        or nastech_status.get("portal_base_url")
        or nastech_status.get("inference_credential_present")
        or nastech_status.get("error_code")
    ):
        try:
            nastech_account_info = get_nastech_portal_account_info()
        except Exception:
            nastech_account_info = None

    nastech_logged_in = bool(
        nastech_status.get("logged_in")
        or (nastech_account_info and nastech_account_info.logged_in)
    )
    nastech_inference_present = bool(
        nastech_status.get("inference_credential_present")
        or (nastech_account_info and nastech_account_info.inference_credential_present)
    )
    nastech_error = nastech_status.get("error")
    if nastech_logged_in:
        nastech_label = "logged in"
    elif nastech_inference_present:
        nastech_label = "not logged in (NasTech inference key configured)"
    else:
        nastech_label = "not logged in (run: nastech portal)"
    print(
        f"  {'NasTech Portal':<14}  {check_mark(nastech_logged_in)} "
        f"{nastech_label}"
    )
    portal_url = nastech_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        nastech_status.get("inference_base_url")
        or (nastech_account_info.inference_base_url if nastech_account_info else None)
    )
    access_exp = _format_iso_timestamp(nastech_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(nastech_status.get("agent_key_expires_at"))
    refresh_label = "yes" if nastech_status.get("has_refresh_token") else "no"
    if nastech_logged_in or portal_url != "(unknown)" or nastech_error:
        print(f"    Portal URL: {portal_url}")
    if nastech_inference_present and inference_url:
    ...
    # =========================================================================
    if managed_nastech_tools_enabled():
        features = get_nastech_subscription_features(config)
        print()
        print(color("◆ NasTech Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.nastech_auth_present:
            print("  NasTech Portal   ✗ not logged in")
        else:
            print("  NasTech Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nous:
                state = "active via NasTech subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.nastech_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.nastech_auth_present:
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: nastech model)"
    ...
    jobs_file = get_nastech_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    print(color("  Run 'nastech doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'nastech setup' to configure", Colors.DIM))
    print()
