"""
MCP Server Management CLI — ``nastech mcp`` subcommand.

Implements ``nastech mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.nastech/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.nastech/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``nastech mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return NasTechTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # nastech_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  nastech mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  nastech mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  nastech mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: nastech mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `nastech mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  nastech mcp add <name> --url <endpoint>')
        _info('  nastech mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def mcp_command(args):
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `nastech plugin`.
        from nastech_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("nastech mcp                                    Open the catalog picker (default)")
        _info("nastech mcp catalog                            List Nous-approved MCPs")
        _info("nastech mcp install <name>                     Install a catalog MCP")
        _info("nastech mcp serve                              Run as MCP server")
        _info("nastech mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("nastech mcp add <name> --command <cmd>         Add a stdio server")
        _info("nastech mcp add <name> --preset <preset>       Add from a known preset")
        _info("nastech mcp remove <name>                      Remove a server")
        _info("nastech mcp list                               List configured servers")
        _info("nastech mcp test <name>                        Test connection")
        _info("nastech mcp configure <name>                   Toggle tools")
        _info("nastech mcp login <name>                       Re-authenticate OAuth")
        print()
