...
from nastech_cli.dashboard_auth.cookies import read_session_cookies
from nastech_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /nastech``, the
    ``login_url`` is prefixed (``/nastech/login?next=...``) so the
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        from nastech_cli.dashboard_auth.cookies import clear_session_cookies
        from nastech_cli.dashboard_auth.prefix import prefix_from_request
