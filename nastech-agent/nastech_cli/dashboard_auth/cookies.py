...
Three cookies in play:
  ...
  - nastech_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/nastech) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/nastech`` covers
...
setter wrote ``__Secure-nastech_session_at`` would never find the value.
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /nastech`` we want ``Path=/nastech`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    SKIPPED when empty — NasTech Portal contract v1 issues no refresh tokens
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/nastech``)
