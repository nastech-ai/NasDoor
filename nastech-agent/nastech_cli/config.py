...
# integration credentials follow that prefix (NASTECH_GEMINI_CLIENT_ID,
# NASTECH_LANGFUSE_PUBLIC_KEY, NASTECH_SPOTIFY_CLIENT_ID, ...). The
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade nastech-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how NasTech was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.nastech/.install_method`` file (written by installers)
    2. NASTECH_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nastechai/nastech-agent`` image stamps ``docker``
    ...
    so ``nastech update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_nastech_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.nastech/.install_method."""
    stamp = get_nastech_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* NasTech lives in a ``uv tool`` layout.

    ``uv tool install nastech-agent`` places the install at
    ``.../uv/tools/nastech-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``nastech-agent`` happens to be uv-tool-installed on the machine while
    the *active* NasTech is a regular pip/venv install, causing
    ``nastech update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/nastech-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade nastech-agent"
    if method == "docker":
        return "docker pull nastechai/nastech-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade nastech-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade nastech-agent"
        return "pip install --upgrade nastech-agent"
...
NasTech Agent runs as a published image (nastechai/nastech-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nastechai/nastech-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nastech-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nastechai/nastech-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nastechai/nastech-agent/tags
...
def format_managed_message(
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this NasTech installation is managed by Homebrew "
            f"(NASTECH_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nastech-agent"
        )
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NASTECH_HOME if the user doesn't have one yet."""
...
DEFAULT_CONFIG = {
    ...
    "sessions": {
        ...
        # default of ``nastech sessions prune``.
        "retention_days": 90,
    ...
    "updates": {
        # Run a full ``nastech backup``-style zip of NASTECH_HOME before every
        # ``nastech update``.  Backups land in ``<NASTECH_HOME>/backups/`` and
        # can be restored with ``nastech import <path>``.  Off by default —
        # on large NASTECH_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. nastechai.com -> firecrawl-gateway.nastechai.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the NasTech auth store)",
    ...
    # NASTECH_TOOL_PROGRESS and NASTECH_TOOL_PROGRESS_MODE are deprecated —
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older NasTech versions) write ``models`` as
...
def warn_deprecated_cwd_env_vars(
    ...
    if lines:
        hint_path = os.environ.get("NASTECH_HOME", "~/.nastech")
...
def migrate_config(interactive: bool = True,
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NASTECH_TOOL_PROGRESS")
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nastech plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nastech curator status` has no
    ...
    #   3. Creates `~/.nastech/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nastech_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_nastech_home() / "logs" / "curator"
...
# Alias used by nastech config set to sync terminal.* keys → env vars.
# Tests inspect this literal by name via AST.
_config_to_env_sync = TERMINAL_CONFIG_ENV_MAP = {
...
#   nous         (OAuth — nastech auth) — Nous Portal
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real NasTech env var names, avoiding false positives from values
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nastech/.env or environment."""
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nastech config set <key> <value>")
            print()
            print("Examples:")
