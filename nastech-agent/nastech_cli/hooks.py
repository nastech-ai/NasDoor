"""nastech hooks — inspect and manage shell-script hooks.

Usage::

    nastech hooks list
    nastech hooks test <event> [--for-tool X] [--payload-file F]
    nastech hooks revoke <command>
    nastech hooks doctor

Consent records live under ``~/.nastech/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nastech/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nastech hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nastech hooks {list|test|revoke|doctor}")
        print("Run 'nastech hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nastech/config.yaml.")
        print("See `nastech hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nastech hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nastech hooks test` and `nastech hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (NasTech wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nastech hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nastech hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `nastech hooks doctor`.")
