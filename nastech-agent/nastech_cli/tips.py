...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or nastech -c.",
    ...
    "Ctrl+Z suspends NasTech to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "nastech -c resumes your most recent CLI session. nastech -c \"project name\" resumes by title.",
    "nastech -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "nastech -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "nastech chat -t web,terminal enables only specific toolsets for a focused session.",
    "nastech chat -s github-pr-workflow preloads a skill at launch.",
    "nastech chat -q \"query\" runs a single non-interactive query and exits.",
    "nastech chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "nastech chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "nastech --yolo bypasses all dangerous command approval prompts for the entire session.",
    "nastech chat --source telegram tags the session for filtering in nastech sessions list.",
    "nastech -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "nastech doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "nastech dump outputs a compact setup summary — great for bug reports.",
    "nastech config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "nastech config edit opens config.yaml in your default editor.",
    "nastech config check scans for missing or stale configuration options.",
    "nastech sessions browse opens an interactive session picker with search.",
    "nastech sessions stats shows session counts by platform and database size.",
    "nastech sessions prune --older-than 30 cleans up old sessions.",
    "nastech skills search react --source skills-sh searches the skills.sh public directory.",
    "nastech skills check scans installed hub skills for upstream updates.",
    "nastech skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "nastech skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "nastech mcp add github --command npx adds MCP servers from the command line.",
    "nastech mcp serve runs NasTech itself as an MCP server for other agents.",
    "nastech auth add lets you add multiple API keys for credential pool rotation.",
    "nastech completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "nastech logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "nastech backup creates a zip backup of your entire NasTech home directory.",
    "nastech profile create coder creates an isolated profile that becomes its own command.",
    "nastech profile create work --clone copies your current config and keys to a new profile.",
    "nastech update syncs new bundled skills to ALL profiles automatically.",
    "nastech gateway install sets up NasTech as a system service (systemd/launchd).",
    "nastech memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "nastech webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: nastech tools disables unused tools, nastech skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "nastech models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as nastech -w).",
    ...
    "NASTECH_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call NasTech tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'nastech profile create coder' creates the 'coder' command.",
    "nastech profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, NasTech prints a resume command with session ID and stats.",
    "nastech sessions export backup.jsonl exports all sessions for backup or analysis.",
    "nastech -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "nastech skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .nastech/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: nastech cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "NasTech runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "nastech gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to NasTech via a local macOS server.",
    ...
    "Context files (.nastech.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "NasTech loads project context from .nastech.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — nastech tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.nastech/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "nastech chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "nastech plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "nastech acp runs NasTech as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "Cron scripts live in ~/.nastech/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make NasTech your own.",
    ...
    "Before a gateway session reset, NasTech auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.nastech/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at NASTECH_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "NASTECH_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NASTECH_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.nastech/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'nastech profile create ls' would be rejected.",
    "nastech profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "nastech claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Skills from trusted repos (NasTech) get a 'trusted' security level; community skills get extra scanning.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — NasTech auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of NasTech config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while NasTech is working.',
    ...
    '/reload reloads ~/.nastech/.env into the running session — pick up new API keys without restarting.',
    ...
    "NASTECH_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.nastech/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.nastech/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'nastech curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "nastech curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'nastech curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    ...
    # --- TUI & Dashboard ---
    'NASTECH_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NASTECH_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'nastech dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'nastech dashboard embeds the full NasTech TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.nastech/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.nastech/dashboard-plugins/ — no npm build required.',
    ...
    'NASTECH_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'NASTECH_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'NASTECH_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NASTECH_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.nastech/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.nastech/skills/ so drop-in skills appear without restarting the session.',
    ...
    'nastech -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'nastech chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'nastech chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'nastech chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    ...
    'nastech status --deep runs the full health sweep across every component; plain nastech status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NASTECH_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NASTECH_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.nastech/plugins/ — trust-gated by design.',
    "NASTECH_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NASTECH_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NASTECH_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NASTECH_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NASTECH_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NASTECH_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NASTECH_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NASTECH_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NASTECH_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NASTECH_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NASTECH_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes NasTech block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from nastech sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.nastech/dashboard-plugins/.',
]
