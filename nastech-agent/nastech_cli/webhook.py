"""nastech webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.nastech/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _nastech_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: nastech gateway run
"""
...
def webhook_command(args):
    """Entry point for 'nastech webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: nastech webhook {subscribe|list|remove|test}")
        print("Run 'nastech webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (nastech gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: nastech webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (nastech gateway run)")
