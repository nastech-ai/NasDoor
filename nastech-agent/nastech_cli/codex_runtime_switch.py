...
Toggles `model.openai_runtime` between "auto" (= chat_completions, NasTech'
...
The actual runtime resolution happens in nastech_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "nastech"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate NasTech' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into NasTech for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from nastech_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the nastech-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "nastech-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + NasTech tool callback are always-on production
            ...
            if "nastech-tools" in mig_report.migrated:
                msg_lines.append(
                    "NasTech tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default NasTech runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "NasTech tools available via MCP callback)."
        )
        msg_lines.append(
