...
Usage examples::
    ...
    nastech logs                    # last 50 lines of agent.log
    nastech logs -f                 # follow agent.log in real time
    nastech logs errors             # last 50 lines of errors.log
    nastech logs gateway -n 100    # last 100 lines of gateway.log
    nastech logs gui -f            # follow gui.log (dashboard/pty/ws)
    nastech logs desktop -f        # follow desktop.log (Electron app boot/backend)
    nastech logs --level WARNING    # only WARNING+ lines
    nastech logs --session abc123   # filter by session ID substring
    nastech logs --component tools  # only tool-related lines
    nastech logs --since 1h         # lines from the last hour
    nastech logs --since 30m -f     # follow, starting 30 min ago
"""
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nastech_home()}/logs/")
        return

    print(f"Log files in {display_nastech_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nastech chat' to generate logs)")
