"""CLI handlers for the ``nastech proxy`` subcommand."""
...
from nastech_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def cmd_proxy_start(args: Any) -> int:
    ...
    print(
        f"Starting NasTech proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("NasTech proxy upstream adapters\n")
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``nastech proxy <subcommand>``."""
    ...
    print(
        "nastech proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  nastech proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
