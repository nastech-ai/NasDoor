        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`nastech auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`nastech auth reset xai-oauth` or re-authenticate with "
                    "`nastech auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `nastech auth add xai-oauth --type oauth`."
            )
