...
Reads the user's Nous OAuth state from ``~/.nastech/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def __init__(self) -> None:
        ...
        # and persistence are handled by resolve_nastech_runtime_credentials().
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `nastech auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `nastech auth add nous` to re-authenticate."
                )
