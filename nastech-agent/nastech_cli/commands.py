"""Slash command definitions and autocomplete for the NasTech CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of NasTech config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal NasTech works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while NasTech is working", "Configuration",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nastech_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nastech`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full NasTech
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nastech skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nastech skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nastech skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nastech <verb>`` subcommand.
    ...
    can still reach them via ``/nastech <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nastech`` is always reserved as the first entry so the
    legacy ``/nastech <subcommand>`` form keeps working for anything that
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nastech handler.

    Maps both canonical names and aliases so /nastech bg do stuff works
    the same as /nastech background do stuff.

    Plugin-registered slash commands are included so ``/nastech <plugin-cmd>``
