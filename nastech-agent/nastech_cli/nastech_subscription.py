"""Helpers for NasTech subscription managed-tool capabilities."""
...
# backward-compat alias (NasTech rebrand)
...
# coverage category (nastech_cli.nastech_account.TOOL_COVERAGE_CATEGORIES). Lets the
...
# consistent with the per-category feature gates in get_nastech_subscription_features.
...
@dataclass(frozen=True)
class NastechSubscriptionFeatures:
    subscribed: bool
    nous_auth_present: bool
    provider_is_nous: bool
    features: Dict[str, NousFeatureState]
    account_info: Optional[NastechPortalAccountInfo] = None
    ...
    @property
    def nastech_auth_present(self) -> bool:
        return self.nous_auth_present
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("NASTECH_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nastech_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `nastech model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("NASTECH_LOCAL_STT_COMMAND")
        )
    except Exception:
    ...
    return NastechSubscriptionFeatures(
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    # Quick provider check without the heavy get_nastech_subscription_features call
    model_cfg = config.get("model")
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from nastech_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "NasTech subscription"
    ...
    if pool_only:
        title = "Your free NasTech tool pool — pick the tools to enable:"
    else:
        title = (
...
# ---------------------------------------------------------------------------
# Inline NasTech Portal login for the Tool Gateway picker (`nastech tools`)
# ---------------------------------------------------------------------------


def ensure_nous_portal_access(
    *,
    capability: str = "the NasTech Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the NasTech Tool Gateway, logging in if
    needed.

    Used by ``nastech tools`` when a user selects a NasTech-managed Tool Gateway
    backend (e.g. "Firecrawl (NasTech Portal)").  Unlike ``nastech model``'s
    NasTech login, this:
    ...
    It only performs the NasTech Portal device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the NasTech Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start NasTech Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a NasTech Portal login.")
    try:
        proceed = input("  Log in to NasTech Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped NasTech Portal login.")
        return False
    ...
    try:
        ...
        _sync_nous_pool_from_auth_store()
        print("  NasTech Portal login successful.")
        return True
        ...
        print(f"  NasTech Portal login failed: {exc}")
        return False


# Alias — tests and callers import apply_nastech_managed_defaults
apply_nastech_managed_defaults = apply_nous_managed_defaults
