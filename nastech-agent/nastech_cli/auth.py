...
DEFAULT_NASTECH_PORTAL_URL = "https://portal.nastechai.com"
DEFAULT_NASTECH_INFERENCE_URL = "https://inference-api.nastechai.com/v1"
DEFAULT_NASTECH_CLIENT_ID = "nastech-cli"
NASTECH_INFERENCE_INVOKE_SCOPE = "inference:invoke"
DEFAULT_NASTECH_SCOPE = NASTECH_INFERENCE_INVOKE_SCOPE
NASTECH_DEVICE_CODE_SOURCE = "device_code"
NASTECH_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
NASTECH_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
XAI_OAUTH_DOCS_URL = "https://nastech-agent.nastechai.com/docs/guides/xai-grok-oauth"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "nous": ProviderConfig(
        ...
        portal_base_url=DEFAULT_NASTECH_PORTAL_URL,
        inference_base_url=DEFAULT_NASTECH_INFERENCE_URL,
        client_id=DEFAULT_NASTECH_CLIENT_ID,
        scope=DEFAULT_NASTECH_SCOPE,
    ),
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.nastech/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``nastech auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `nastech model` to re-authenticate."
...
def _format_nastech_entitlement_auth_error(error: AuthError) -> str:
    try:
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("NASTECH_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.nastech/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_nastech_home() / "auth.json"
    # Seat belt: if pytest is running and NASTECH_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch NASTECH_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set NASTECH_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom NASTECH_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_nastech_root()
    except Exception:
        return None
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.nastech/auth.json`` even when NASTECH_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_nastech_root()`` for a profile-shaped NASTECH_HOME can
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared NasTech store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_nastech_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared NasTech lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "nastech_portal" in systems:
            providers["nous"] = systems["nastech_portal"]
...
def _load_provider_state(
    ...
    global scope. Once the user runs ``nastech auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``nastech auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``nastech auth add <provider>`` inside the profile, profile entries
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `nastech logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'nastech doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'nastech model' for available providers, or run 'nastech doctor' to diagnose config issues."
    ...
    # `nastech auth list` showing the credential while requests go out with no
    ...
    raise AuthError(
        "No inference provider configured. Run 'nastech model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.nastech/.env.",
...
# (NASTECH_INFERENCE_BASE_URL) bypass validation — that's the documented
...
def _validate_nastech_inference_url_from_network(
    ...
    The env-var override path (``NASTECH_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        ...
        return None
    if parsed.hostname not in _ALLOWED_NASTECH_INFERENCE_HOSTS:
        logger.warning(
...
def _nastech_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = NASTECH_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if NASTECH_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _nastech_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_nastech_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: nastech auth add nastech",
        provider="nous",
        code=reason,
...
def _log_nastech_invoke_jwt_selected(
    *,
    access_token: Any,
...
def _set_nastech_agent_key_from_invoke_jwt(
    ...
    *,
    ...
    expires_at = _nastech_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_nastech_invoke_jwt(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
    sequence_id: Optional[str] = None,
    ...
    _set_nastech_agent_key_from_invoke_jwt(state)
    _log_nastech_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_NASTECH_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _nastech_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _NASTECH_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `nastech model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.nastech/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `nastech auth list` / `nastech status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.nastech/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("NASTECH_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set NASTECH_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("NASTECH_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NASTECH_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NASTECH_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but NasTech times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``NASTECH_LOG_LEVEL=INFO`` (or check
            # ``~/.nastech/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>nastech auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `nastech auth spotify` again.",
            provider="spotify",
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `nastech auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `nastech auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.nastech/.env, and return it.
    ...
    print("       App name:     anything (e.g. nastech-agent)")
    ...
    save_env_value("NASTECH_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("NASTECH_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved NASTECH_SPOTIFY_CLIENT_ID to ~/.nastech/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "NASTECH_SPOTIFY_CLIENT_ID is required".
...
def _is_remote_session() -> bool:
    ...
    **browser-only remote consoles** (GCP Cloud Shell, GitHub
    Codespaces, AWS EC2 Instance Connect, Gitpod, cloud shells, etc.) hit
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `nastech auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `nastech auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `nastech auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `nastech auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``nastech setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``nastech auth add openai-codex``
      ...
      ``nastech auth add openai-codex`` and must not be overwritten by a
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `nastech auth` to re-authenticate."
            )
            relogin_required = True
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.nastech/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `nastech model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `nastech model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `nastech model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `nastech model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.nastech/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``NASTECH_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `nastech model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``nastech model``
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("NASTECH_CA_BUNDLE")
...
# -----------------------------------------------------------------------------
# Shared NasTech token store — lets OAuth credentials persist across profiles
# so a new `nastech --profile <name> auth add nastech --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${NASTECH_SHARED_AUTH_DIR}/nastech_auth.json, defaulting to
# ``<nastech-root>/shared/nastech_auth.json`` where ``<nastech-root>`` is what
# ``get_default_nastech_root()`` returns — ``~/.nastech`` on Linux/macOS,
# ``%LOCALAPPDATA%\nastech`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's NASTECH_HOME so named profiles (which
# typically live under ``<nastech-root>/profiles/<name>/``) all see the
# same file.
#
...
NASTECH_SHARED_STORE_FILENAME = "nastech_auth.json"
_nastech_shared_lock_holder = threading.local()


def _nastech_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared NasTech token store.

    Honors ``NASTECH_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<nastech-root>/shared/``, where ``<nastech-root>`` is what
    :func:`nastech_constants.get_default_nastech_root` returns — so
    Linux/macOS classic installs land at ``~/.nastech/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\nastech\\shared\\``, and
    Docker / custom ``NASTECH_HOME`` deployments at
    ``<NASTECH_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("NASTECH_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nastech_shared_store_path() -> Path:
    path = _nastech_shared_auth_dir() / NASTECH_SHARED_STORE_FILENAME
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from nastech_constants import get_default_nastech_root
        real_home_shared = (
            get_default_nastech_root() / "shared" / NASTECH_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared NasTech auth store during test run: "
...
def _nastech_shared_store_lock(
    ...
    ``_try_import_shared_nastech_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _nastech_shared_store_path().with_suffix(".lock")
    except RuntimeError:
...
def _merge_shared_nastech_oauth_state(state: Dict[str, Any]) -> bool:
...
def _write_shared_nastech_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the NasTech OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_NASTECH_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_NASTECH_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_NASTECH_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_NASTECH_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _nastech_shared_store_lock():
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "nastech_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared NasTech auth store: %s", exc)


def _read_shared_nastech_state() -> Optional[Dict[str, Any]]:
    """Return the shared NasTech OAuth state if present and well-formed.
...
def _clear_shared_nastech_state(reason: str) -> None:
    """Remove the shared NasTech OAuth store after a terminal token failure."""
    try:
        with _nastech_shared_store_lock():
            path = _nastech_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("nastech_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared NasTech auth store: %s", exc)


def _is_terminal_nastech_refresh_error(exc: Exception) -> bool:
    """True when retrying the same NasTech refresh token cannot succeed."""
    return (
...
def _quarantine_nastech_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_nastech_state(reason)
    invalidate_nastech_auth_status_cache()


def _quarantine_nastech_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    if removed:
        pool["nous"] = retained
        _oauth_trace(
            "nastech_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_nastech_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate NasTech OAuth state from the shared store.
    ...
    for ``persist_nastech_credentials()``.
    ...
    try:
        with _nastech_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            # Build a full state dict so refresh_nastech_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_NASTECH_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_NASTECH_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_NASTECH_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_NASTECH_SCOPE,
            ...
            _write_shared_nastech_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "nastech_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-nastech-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "NasTech refresh tokens are single-use — only NasTech may call the "
            "refresh endpoint. For health checks, use `nastech auth status` "
            "instead.\n"
            "Re-authenticate with: nastech auth add nastech"
        )
        relogin = True
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _nastech_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_nastech_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "NasTech is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def refresh_nastech_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_NASTECH_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh NasTech OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_NASTECH_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_NASTECH_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_NASTECH_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_NASTECH_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _nastech_invoke_jwt_status(
        ...
        _assert_nastech_inference_jwt_usable(state)
        _select_nastech_invoke_jwt(state)

    return state


def refresh_nastech_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh NasTech OAuth from a state dict. Thin wrapper around refresh_nastech_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_nastech_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "nastech-cli"),
        state.get("portal_base_url", DEFAULT_NASTECH_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_NASTECH_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_NASTECH_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_nastech_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist NasTech OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    NasTech credentials are read at runtime from two independent locations:

    - ``providers.nastech``: singleton state read by
      ``resolve_nastech_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.nastech``: used by the runtime ``pool.select()`` path.

    Historically ``nastech auth add nastech`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.nastech``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.nastech`` then calls ``load_pool("nastech")`` so
    ...
    ``nastech auth add nastech --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("nastech")`` instead of the
    ...
    # these credentials via `nastech auth add nastech --type oauth`. Best-
    ...
    return next(
        (e for e in pool.entries() if e.source == NASTECH_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_nastech_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.nastech changes; never fail login."""
    try:
        from agent.credential_pool import load_pool
...
def resolve_nastech_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("NasTech is not logged into Nous Portal.",
        ...
        inference_base_url = (
            ...
            or os.getenv("NASTECH_INFERENCE_BASE_URL")
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed NasTech auth-status cache warm during read paths.
            if (
                _nastech_effective_provider_state(state)
                == _nastech_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "nastech_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            invoke_jwt_status = _nastech_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _nastech_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_nastech_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        invoke_jwt_status = _nastech_invoke_jwt_status(
                            access_token,
        ...
        _persist_state("resolve_nastech_runtime_credentials_final")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": NASTECH_AUTH_PATH_INVOKE_JWT,
        "auth_path": NASTECH_AUTH_PATH_INVOKE_JWT,
    }
...
def _empty_nastech_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_nastech_pool_status() -> Dict[str, Any]:
    """Best-effort status from the credential pool.

    This is a fallback only. The auth-store provider state is the runtime source
    of truth because it is what ``resolve_nastech_runtime_credentials()`` refreshes.
    """
    try:
        from agent.credential_pool import load_pool
...
# ── Process-level memo for get_nastech_auth_status() ──
# get_nastech_auth_status() validates state by calling resolve_nastech_runtime_credentials(),
...
def invalidate_nastech_auth_status_cache() -> None:
    """Clear the get_nastech_auth_status() process-level memo.

    Call this from any code path that mutates NasTech auth state without going
    through resolve_nastech_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _nastech_auth_status_cache
    _nastech_auth_status_cache = None


def get_nastech_auth_status() -> Dict[str, Any]:
    """Status snapshot for NasTech auth.
    ...
    ``invalidate_nastech_auth_status_cache()`` explicitly.
    """
    global _nastech_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _nastech_auth_status_cache
    if cached is not None:
        cached_at, cached_mtime, cached_status = cached
        if (
            cached_mtime == mtime
            and (now - cached_at) < _NASTECH_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_nastech_auth_status()
    _nastech_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_nastech_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_nastech_auth_status(). See that function."""
    ...
    return _snapshot_nastech_pool_status()
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    """
    # Check credential pool first — this is where `nastech auth` and
    # `nastech model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "nous":
        return get_nastech_auth_status()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``nastech doctor`` runs the live
        ...
        try:
            ...
            if not installed:
                ...
                info["hint"] = (
                    ...
                    "is skipped here. Run `nastech doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `nastech logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_NASTECH_PORTAL_URL).rstrip("/")
...
def login_command(args) -> None:
    """Deprecated: use 'nastech model' or 'nastech setup' instead."""
    print("The 'nastech login' command has been removed.")
    print("Use 'nastech auth' to manage credentials,")
    print("'nastech model' to select a provider, or 'nastech setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.nastech/auth.json."""
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=nastech-agent` lets xAI attribute NasTech-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in NasTech — please report at "
            "https://github.com/NasTech/nastech-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize NasTech with xAI:")
        print(authorize_url)
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `nastech model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``nastech status`` that just want to know
...
def _nastech_device_code_login(
    *,
    ...
    pconfig = PROVIDER_REGISTRY["nastech"]
    portal_base_url = (
        portal_base_url
        or os.getenv("NASTECH_PORTAL_BASE_URL")
        or os.getenv("NASTECH_PORTAL_BASE_URL")
    ...
    try:
        return refresh_nastech_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `nastech model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_nastech(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        # flow, check the shared store for an existing NasTech credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_nastech_state()
        if shared:
            try:
                shared_path = _nastech_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                auth_state = _try_import_shared_nastech_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        _write_shared_nastech_state(auth_state)
        _sync_nastech_pool_from_auth_store()
        ...
        try:
            ...
            if model_ids:
                ...
                free_tier = check_nastech_free_tier(force_fresh=True)
                ...
                if free_tier:
                    ...
                    model_ids, unavailable_models = partition_nastech_models_by_tier(
        ...
        # them to Nous with a mismatched model.  The NasTech OAuth tokens
        ...
        if not selected_model:
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. NasTech credentials saved for future use.")
            print("  Run `nastech model` again to switch to Nous Portal.")
            return
