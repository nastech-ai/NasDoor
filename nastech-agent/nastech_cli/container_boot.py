...
``$NASTECH_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-nastech-setup
(the stage2 hook) has chowned the volume and seeded $NASTECH_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NASTECH_HOME``,
    not under ``profiles/``). The dispatcher in ``nastech_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``nastech gateway start`` (no ``-p``) targets. Without it,
    bare ``nastech gateway start`` inside the container would land on
    ...
    ``$NASTECH_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        nastech_home: The container's NASTECH_HOME (typically /opt/data).
            Profiles live under ``<nastech_home>/profiles/<name>/``;
            the default profile lives at ``<nastech_home>`` itself.
    ...
    # ``nastech gateway start`` (no ``-p``) has somewhere to land;
    ...
    profiles_root = nastech_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `nastech profile create` (config.yaml
            # is not — that comes later via `nastech setup`). Use it as the
            ...
            # via ``nastech -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(nastech_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = nastech_home / "gateway_state.json"
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "nastech":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `nastech -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit nastech
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the nastech user) won't hit EACCES. See
...
def _write_reconcile_log(
    nastech_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $NASTECH_HOME/logs/container-boot.log.
