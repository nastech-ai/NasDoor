...
configured for "cli" in `nastech tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (NASTECH_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `nastech chat`:
...
def _validate_explicit_toolsets(
    ...
    try:
        ...
        return None, f"nastech -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "nastech -z: --toolsets all enables every toolset; "
    ...
    if not valid:
        return None, "nastech -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to NASTECH_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("NASTECH_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "nastech -z: --provider requires --model (or NASTECH_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"nastech -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``nastech -z`` / oneshot mode.

    Oneshot bypasses ``NasTechCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when nastech is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from nastech_cli.config import load_config
    from nastech_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("NASTECH_INFERENCE_MODEL", "").strip()
    ...
    agent = AIAgent(
        ...
        #                NASTECH_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via NASTECH_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via NASTECH_YOLO_MODE=1
