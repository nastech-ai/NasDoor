...
Profiles live under ``~/.nastech/profiles/<name>/`` by default.

The "default" profile is ``~/.nastech`` itself — backward compatible,
zero migration needed.

Usage::

    nastech profile create coder          # fresh profile + bundled skills
    nastech profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nastech profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nastech -p coder chat                 # or via flag
    nastech profile use coder             # set as sticky default
    nastech profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See nastech_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.nastech``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   nastech-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `nastech profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `nastech
...
# `nastech skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_nastech_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.nastech) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nastech-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nastech root, NOT to the current NASTECH_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NASTECH_HOME points outside
    ``~/.nastech``, profiles live under ``NASTECH_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_nastech_home() / "profiles"


def _get_default_nastech_home() -> Path:
    """Return the default (pre-profile) NASTECH_HOME path.

    In standard deployments this is ``~/.nastech``.
    In Docker/custom deployments where NASTECH_HOME is outside ``~/.nastech``
    (e.g. ``/opt/data``), returns NASTECH_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_nastech_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NASTECH_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nastech_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, nastech subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _NASTECH_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nastech subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``nastech -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``nastech profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nastech profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nastech update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nastech)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nastech/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `nastech update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`nastech profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `nastech update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `nastech profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `nastech -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``nastech profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nastech).\n"
            "To remove everything, use: nastech uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from nastech_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NASTECH_HOME so _profile_suffix resolves correctly
    ...
    try:
        ...
        if old_home is not None:
            os.environ["NASTECH_HOME"] = old_home
        elif "NASTECH_HOME" in os.environ:
            del os.environ["NASTECH_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nastech/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nastech profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NASTECH_HOME.

    Returns ``"default"`` if NASTECH_HOME is not set or points to ``~/.nastech``.
    Returns the profile name if NASTECH_HOME points into ``~/.nastech/profiles/<name>``.
    Returns ``"custom"`` if NASTECH_HOME is set to an unrecognized path.
    """
    from nastech_constants import get_nastech_home
    nastech_home = get_nastech_home()
    resolved = nastech_home.resolve()

    default_resolved = _get_default_nastech_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        ...
        # directory name is ".nastech", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nastech profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.nastech itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nastech). "
            "Specify a different name: nastech profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="nastech_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    new_host = f"nastech_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_nastech_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NASTECH_HOME path string.

    Called early in the CLI entry point, before any nastech modules
    are imported, to set the NASTECH_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
