"""
Backup and import commands for nastech CLI.

`nastech backup` creates a zip archive of the entire ~/.nastech/ directory
(excluding the nastech-agent repo and transient files).

`nastech import` restores from a backup zip, overlaying onto the current
NASTECH_HOME root.
"""
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nastech root) should be skipped."""
...
def _should_skip_backup_file(
    ...
    # write can copy data from outside NASTECH_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the NasTech home directory."""
    nastech_root = get_default_nastech_root()

    if not nastech_root.is_dir():
        print(f"Error: NasTech home directory not found at {nastech_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nastech-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nastech-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_nastech_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nastech_root)
    ...
    print(f"\nRestore with: nastech import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a NasTech backup.
    ...
    # Look for telltale files that a nastech home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a NasTech backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nastech/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nastech dir name
        if prefix in {".nastech", "nastech"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nastech_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nastech_root / "config.yaml").exists()
        has_env = (nastech_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has NasTech configuration.")
        ...
        for member in members:
            ...
            target = nastech_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(nastech_root.resolve())
            except ValueError:
        ...
        profiles_dir = nastech_root / "profiles"
        restored_profiles = []
        ...
        # Guidance
        print()
        if not (nastech_root / "nastech-agent").is_dir():
            print("Note: The nastech-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: nastech update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nastech -p {pname} gateway install")

        print("Done. Your NasTech configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nastech backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NASTECH_HOME).
...
# — `nastech update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(nastech_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nastech_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``nastech update``.
    ...
    Args:
        ...
        nastech_home: Override for the NasTech home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for nastech backup --quick."""
...
def _pre_update_backup_dir(nastech_home: Optional[Path] = None) -> Path:
    home = nastech_home or get_nastech_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NASTECH_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NASTECH_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``nastech update``) should continue even if the backup fails.
    """
    nastech_root = nastech_home or get_default_nastech_root()
    if not nastech_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(nastech_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, nastech_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nastech claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NASTECH_HOME under ``backups/`` before a
    ``nastech claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nastech import <archive>``.  Writes to
    ``<NASTECH_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nastech import` and the
