...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NasTech/nastech-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_nastech_home() / "cache" / "model_catalog.json"
...
def get_curated_nastech_models() -> list[str] | None:
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``nastech update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.nastech/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``nastech model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
