"""CLI handlers for ``nastech secrets bitwarden ...``.
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from nastech_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``nastech_cli.main`` as part of building the top-level
    ``nastech secrets`` parser.
    """
    ...
    sync = sub.add_parser(
        "sync", help="Fetch secrets now and report what changed")
    sync.add_argument(
        "--apply",
        action="store_true",
    ...
    disable = sub.add_parser(
        "disable",
        help="Turn off the Bitwarden integration")
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if not token:
        token = masked_secret_prompt(
            f"  Paste access token ({token_env}): ").strip()
    if not token:
    ...
    if args.project_id and args.project_id.strip():
        ...
        projects = _list_projects(
            binary, token, console, server_url=server_url)
        if projects is None:
            return 1
        if not projects:
            console.print(
        ...
        while True:
            choice = console.input(
                f"  Select project [1-{len(projects)}]: ").strip()
            if not choice:
                continue
            try:
                idx = int(choice)
            ...
            if 1 <= idx <= len(projects):
                ...
                break
            console.print(
                f"  [red]Out of range — pick 1-{len(projects)}.[/red]")
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every NasTech process."
    )
    console.print(
        "  Status:  [cyan]nastech secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]nastech secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]nastech secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print(
            "\n  Run [cyan]nastech secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — NasTech will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`nastech secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    for key in sorted(secrets):
        ...
        if args.apply:
            ...
            table.add_row(key, "[green]exported[/green]" +
                          (" (overrode)" if already else ""))
        else:
            table.add_row(key, "[green]would export[/green]" +
                          (" (overrides)" if already else ""))
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]nastech[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]nastech secrets bitwarden setup[/cyan] and pick EU or "
