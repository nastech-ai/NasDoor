...
def create_task(
    ...
    *,
    ...
    each name to ``nastech --skills ...`` alongside the built-in
...
def _kanban_worker_skill_available(nastech_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``nastech -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset NASTECH_HOME means the worker falls back to the default root
    # home (``~/.nastech``), which ships the bundled skill.
    base = _Path(nastech_home) if nastech_home else (_Path.home() / ".nastech")
...
def _default_spawn(
    ...
    *,
    ...
    if _kanban_worker_skill_available(env.get("NASTECH_HOME")):
