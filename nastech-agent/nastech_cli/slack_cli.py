"""``nastech slack ...`` CLI subcommands.

Today only ``nastech slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ nastech slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of NasTech. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a NasTech deployment — users can tweak them in the Slack UI after
    pasting.
    """
    ...
    return {
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with NasTech in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``nastech_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$NASTECH_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "NasTech")
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your NasTech app\n"
