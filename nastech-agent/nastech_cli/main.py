...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``NASTECH_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``nastech --version`` before config/logging imports on Termux."""
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NASTECH_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
...
_model_flow_nastech = _model_flow_nous
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @nastechai/ink is missing or node_modules is behind package-lock.json.
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        # Keep the local @nastechai/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @nastechai/ink resolves through
...
def _launch_tui(
    ...
    env["NASTECH_PYTHON_SRC_ROOT"] = os.environ.get(
        "NASTECH_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("NASTECH_PYTHON", sys.executable)
    env.setdefault("NASTECH_CWD", os.getcwd())
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``NASTECH_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("NASTECH_TUI") == "1":
        return True
    try:
...
def cmd_postinstall(args):
    ...
    print("⚕ NasTech post-install bootstrap")
    print()
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``nastech model`` flow, then route aux tasks to them here.
    """
    from nastech_cli.config import load_config
...
# Critical files that every ``nastech`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``nastech update`` again to roll forward. The post-pull syntax
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``nastech_cli/web_dist/`` (per vite.config.ts
    outDir: "../nastech_cli/web_dist"), NOT to ``web/dist/``, so Python
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Uses platform_detect.npm_install_web_with_fallbacks so the install never
    hard-stops regardless of environment (Termux, Linux, macOS, Windows, NixOS,
    WSL).  Falls back through: workspace-scoped → direct → --ignore-scripts →
    --no-optional → --legacy-peer-deps before giving up.
    ...
    # Console-encoding-safe print so Windows cp1252 consoles don't raise on
    # arrow/check glyphs.
    ...
    npm = shutil.which("npm")
    if not npm:
        _say("  ⚠ npm not found — attempting auto-install…")
        _ensure_node(verbose=True)
        npm = shutil.which("npm")

    if not npm:
        if fatal:
            _say("  ✗ Web UI frontend not built: npm/Node.js is not available.")
            _say("    Install Node.js for your platform:")
            p = _get_platform()
            if p.is_termux:
                _say("      pkg install nodejs-lts")
            elif p.is_macos:
                _say("      brew install node")
            elif p.is_windows:
                _say("      choco install nodejs  OR  winget install OpenJS.NodeJS")
            else:
                _say("      apt install nodejs npm  OR  see https://nodejs.org")
        return not fatal

    _say("→ Building web UI...")

    # --- 2. Install deps with progressive fallbacks --------------------------
    workspace_root = _workspace_root(web_dir)
    nixos_env = _nixos_build_env()

    install_ok = _npm_install_web_fallbacks(
        npm,
        workspace_root,
        web_dir,
        env=nixos_env,
    ...
    # --- 3. Build (stream + idle-timeout so hangs are observable) -----------
    # First attempt via idle-timeout helper (issue #33788).
    r2 = _run_with_idle_timeout([npm, "run", "build"], cwd=web_dir)
    if r2.returncode != 0:
        # Retry once after a short delay — covers boot-time races on Windows
        # (antivirus, npm cache not ready, transient I/O on logon). #23817.
        _time.sleep(3)
        r2 = _run_with_idle_timeout([npm, "run", "build"], cwd=web_dir)
    ...
    if r2.returncode != 0:
        ...
        # Stale dist fallback — a stale UI beats no UI for headless callers.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "NasTech is damaged and can't be opened." The
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``nastech dashboard`` processes other than ourselves.

    ``nastech dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``nastech update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the NasTech Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``nastech dashboard`` as
    ...
    environment variable ``NASTECH_DESKTOP_CHILD_PID`` on the spawned
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "nastech.*dashboard"`) keeps us consistent
            # with `nastech_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://nastech-agent.nastechai.com/docs/user-guide/features/curator"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``nastech dashboard`` processes.

    Called at the end of ``nastech update`` (default ``reason``) and also
    from ``nastech dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the NasTech Desktop Electron app spawns this dashboard as a
    # backend child, it sets NASTECH_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("NASTECH_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/NasTech/nastech-agent/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NasTech/nastech-agent.git",
    "git@github.com:NasTech/nastech-agent.git",
    "https://github.com/NasTech/nastech-agent",
    "git@github.com:NasTech/nastech-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NasTech/nastech-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NasTech/nastech-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NasTech/nastech-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NasTech/nastech-agent.git' to add later."
            )
...
def _nastech_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "nastech.exe",
        scripts_dir / "nastech-gateway.exe",
    ]
...
def _detect_concurrent_nastech_instances(
    ...
    The NasTech Desktop Electron app spawns ``nastech.EXE`` as a backend child,
    so during ``nastech update`` the user-invoked process and the desktop's
    ...
    shims (``nastech.exe`` / ``nastech-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``nastech update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``nastech.exe``) is a separate process from the Python
    ...
    # setuptools-generated nastech.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``nastech update``
    ...
    #      nastech.exe sitting *under* a non-NasTech parent (e.g. a NasTech
...
def _refresh_active_lazy_features() -> None:
    ...
    if failed:
        ...
        print("  `nastech update` once the upstream issue is resolved.")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of NASTECH_HOME before running the update.
    ...
    NASTECH_HOME directories.  The ``--backup`` flag on ``nastech update``
...
def _cmd_update_pip(args):
    """Update NasTech via pip (for PyPI installs)."""
    from nastech_cli import __version__
    from nastech_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "nastech-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "nastech-agent"]
        if in_venv:
    ...
    print("✓ Update complete! Restart nastech to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # though `git pull` can't touch $NASTECH_HOME, this is cheap
        ...
        # modules in this process.  Reload nastech_constants so that any lazy
        ...
        # Tying the refresh to ``nastech update`` gives users a predictable
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. nastech-gateway.service or nastech-gateway-coder.service
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.nastech)")
            else:
...
def _report_dashboard_status() -> int:
    """Print ``nastech dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``nastech dashboard --status``
    ...
    print(f"{len(pids)} nastech dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        from nastech_cli.platform_detect import pip_install_with_fallbacks
        ok = pip_install_with_fallbacks(["fastapi", "uvicorn"], verbose=True)
        if ok:
            try:
                import fastapi  # noqa: F401  # type: ignore[no-redef]
                import uvicorn  # noqa: F401  # type: ignore[no-redef]
                print("  ✓ fastapi + uvicorn installed successfully")
            except ImportError:
                ok = False
        if not ok:
            print(
                f"  Re-install manually:\n"
                f"    cd {PROJECT_ROOT}\n"
    ...
    # Auto-ensure Node.js / npm are available before attempting the web build.
    # This is a no-op on platforms that already have Node installed, and will
    # silently attempt platform-appropriate install (pkg/brew/apt/choco) when it
    # is missing — so Termux, fresh Linux VMs, CI all work out of the box.
    _ensure_node(verbose=False)
...
def main():
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.nastech/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://nastech-agent.nastechai.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    # =========================================================================
    # postinstall command  (parser built in nastech_cli/subcommands/postinstall.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch nastech --resume <id> by replacing the current process
