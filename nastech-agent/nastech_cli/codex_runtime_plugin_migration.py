"""Migrate NasTech' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads NasTech' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  NasTech mcp_servers.<n>.command/args/env  → codex stdio transport
  NasTech mcp_servers.<n>.url/headers       → codex streamable_http transport
  NasTech mcp_servers.<n>.timeout           → codex tool_timeout_sec
  NasTech mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  NasTech-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. NasTech' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in NasTech config.")
...
# NasTech keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # NasTech' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, nastech_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one NasTech MCP server config to the codex inline-table dict
    ...
    if not isinstance(nastech_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(nastech_cfg.get("command"))
    has_url = bool(nastech_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(nastech_cfg["command"])
        args = nastech_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = nastech_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = nastech_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(nastech_cfg["url"])
        headers = nastech_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # NasTech' transport: sse hint is informational; codex auto-negotiates
        if nastech_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in nastech_cfg:
        try:
            out["tool_timeout_sec"] = float(nastech_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in nastech_cfg:
        try:
            out["startup_timeout_sec"] = float(nastech_cfg["connect_timeout"])
        except (TypeError, ValueError):
    ...
    for key in nastech_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_NASTECH_KEYS:
            skipped.append(f"{key} (unknown NasTech key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (NASTECH_HOME, PYTHONPATH) could
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before NasTech' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior NasTech
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="nastech-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a NASTECH_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed nastech-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.nastech`` symlink that happens to live under ``/private/var/folders``
...
def _build_nastech_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches NasTech' own
    ...
    so a nastech installed under /opt/, /usr/local/, or a venv all work.
    NASTECH_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # NASTECH_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_nastech_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever NASTECH_HOME its launcher sets
    ...
    # — that would override the launcher's NASTECH_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("NASTECH_HOME", tmp_path) would
    ...
    nastech_home = os.environ.get("NASTECH_HOME") or ""
    if nastech_home and _looks_like_test_tempdir(nastech_home):
        nastech_home = ""
    if nastech_home:
        env["NASTECH_HOME"] = nastech_home
    # PYTHONPATH passes through so a worktree-launched nastech finds the
    ...
    env["NASTECH_QUIET"] = "1"
    env["NASTECH_REDACT_SECRETS"] = env.get("NASTECH_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_nastech_tools: bool = True,
) -> MigrationReport:
    """Translate NasTech mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        nastech_config: full ~/.nastech/config.yaml dict
        ...
        expose_nastech_tools: when True (default), register NasTech' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into NasTech for tools
    ...
    nastech_servers = (nastech_config or {}).get("mcp_servers") or {}
    if not isinstance(nastech_servers, dict):
        report.errors.append(
            "mcp_servers in NasTech config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in nastech_servers.items():
    ...
    # Inject NasTech' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into NasTech for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/nastech_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_nastech_tools:
        translated["nastech-tools"] = _build_nastech_tools_mcp_entry()
        if "nastech-tools" not in report.migrated:
            report.migrated.append("nastech-tools")
