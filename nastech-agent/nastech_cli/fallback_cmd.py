"""
nastech fallback — manage the fallback provider chain.
...
Subcommands:
  nastech fallback [list]   Show the current fallback chain (default when no subcommand)
  nastech fallback add      Pick provider + model via the same picker as `nastech model`,
                           then append the selection to the chain
  nastech fallback remove   Pick an entry to delete from the chain
  nastech fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nastech/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `nastech model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nastech fallback add")
        print()
        return
    ...
    print("  Docs: https://nastech-agent.nastechai.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    print("  `nastech model` — select the provider + model you want as a fallback.")
    print()
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nastech fallback [subcommand]``."""
