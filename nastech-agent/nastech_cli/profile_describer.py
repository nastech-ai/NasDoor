...
Used by ``nastech profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``nastech_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the NasTech Agent kanban board.
...
Rules:
  ...
  - Never write "NasTech Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from nastech_constants import get_nastech_home  # type: ignore
            profile_dir = Path(get_nastech_home())
        else:
