...
on Japanese locales, etc.).  NasTech's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nastech_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. NasTech is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NASTECH_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    # already-running shells don't see the change — which means nastech
    ...
    # install without NasTech-managed PortableGit).
    ...
    # stdin is re-configured for completeness; NasTech's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and NasTech's
    ``nastech config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching NasTech (User env var in Windows Settings, or
    export in a PowerShell profile) and NasTech picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\nastech\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nastech`` from right after install) retain their old PATH.

    Any subprocess NasTech spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known NasTech-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nastech", "git", "cmd"),
        os.path.join(local_appdata, "nastech", "git", "bin"),
        os.path.join(local_appdata, "nastech", "git", "usr", "bin"),
