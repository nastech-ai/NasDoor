...
This command:
  ...
  3. Writes ``NASTECH_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``NASTECH_DASHBOARD_PORTAL_URL`` into ``~/.nastech/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``NASTECH_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from nastech_cli.auth import DEFAULT_NASTECH_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``nastech dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `nastech auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from nastech_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    NASTECH_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    NASTECH_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    NASTECH_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `nastech dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    nastech dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from nastech_cli.auth import AuthError, resolve_nastech_access_token
    ...
    # stamped in by the orchestrator (NAS sets NASTECH_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `nastech dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        access_token = resolve_nastech_access_token()
    ...
    # NASTECH_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (NASTECH_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("NASTECH_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    #      NASTECH_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://portal.nastechai.com"
    existing_portal = None
    try:
        existing_portal = get_env_value("NASTECH_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("NASTECH_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://nastech.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking NASTECH_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as NASTECH_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("NASTECH_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("NASTECH_DASHBOARD_PUBLIC_URL", public_url)
