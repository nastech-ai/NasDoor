"""CLI subcommand: ``nastech send`` — pipe text from shell scripts to any
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"nastech send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"nastech send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `nastech gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.nastech/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_nastech_env() -> None:
    """Populate ``os.environ`` from ``~/.nastech/.env`` AND bridge top-level
    ...
    startup that ``nastech send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.nastech/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.nastech/config.yaml`` into
       ...
       via ``nastech config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``nastech send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.nastech/.env and ~/.nastech/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "nastech send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  nastech send --to telegram \"hello\"\n"
            "  nastech send --to discord:#ops --file report.md\n"
            "  nastech send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "nastech send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `nastech send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform NasTech "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.nastech/.env + ~/.nastech/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `nastech send --list telegram`.",
    )
