"""
Cron subcommand for nastech CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(nastech\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*nastech)"
    r"|(systemctl\s+(restart|stop|start)\s+.*nastech)"
    r"|(p?kill\s+.*nastech.*gateway)"
)
...
def cron_status():
    ...
    if pids:
        ...
        print("    sudo nastech gateway install --system  # Linux servers: boot-time system service")
        print("    nastech gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `nastech gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
