"""
Unified self-relaunch for NasTech CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``nastech sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``nastech`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from nastech_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched nastech."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_nastech_bin() -> Optional[str]:
    """Find the nastech entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("nastech")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m nastech_cli.main``.
    ...
    ``nastech.exe`` on PATH or the ``sys.executable -m nastech_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with nastech.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh nastech invocation.
    ...
    relaunch contract wants: "run nastech again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``nastech.exe`` (a Python console-script shim that wraps
    ``python -m nastech_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "nastech exited, then
    new nastech started" — just with two PIDs in play instead of one.
    """
