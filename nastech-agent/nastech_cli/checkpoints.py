"""`nastech checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nastech/checkpoints/``.  Actions:

    nastech checkpoints               # same as `status`
    nastech checkpoints status        # total size, project count, breakdown
    nastech checkpoints list          # per-project checkpoint counts + workdir
    nastech checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nastech checkpoints clear [-f]    # nuke the entire base (asks first)
    nastech checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nastech checkpoints
    nastech checkpoints prune --retention-days 3 --max-size-mb 200
    nastech checkpoints clear -f
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nastech checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nastech checkpoints` → status
