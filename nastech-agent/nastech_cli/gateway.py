"""
Gateway subcommand for nastech CLI.

Handles: nastech gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "nastech CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``nastech gateway status`` from falsely counting the ``nastech`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``nastech gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "nastech_cli.main gateway",
        "nastech_cli.main --profile",
        "nastech_cli.main -p",
        "nastech_cli/main.py gateway",
        "nastech_cli/main.py --profile",
        "nastech_cli/main.py -p",
        "nastech gateway",
        ...
        # ``nastech.exe --profile`` / ``nastech.exe -p`` would also match
        # ``nastech.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "nastech.exe gateway",
        "nastech-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_nastech_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"nastech_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. NASTECH_HOME
        ...
        # NASTECH_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "nastech_home=" in command_lc
            and f"nastech_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``nastech update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            NasTech profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to NasTech profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "nastech_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``nastech update`` users
    # with no running gateway until they re-invoke ``nastech gateway``
...
def _sync_nastech_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``NASTECH_HOME``.

    Under ``sudo``, ``NASTECH_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_nastech_home` falls back to ``/root/.nastech`` — the wrong
    profile. The unit file pins ``NASTECH_HOME`` for the actual gateway
    ...
    unit_home = env.get("NASTECH_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("NASTECH_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["NASTECH_HOME"] = unit_home
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``nastech gateway status`` output so users with
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (NASTECH_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our NasTech Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``nastech gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``NASTECH_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("NASTECH_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "nastech-gateway"
SERVICE_DESCRIPTION = "NasTech Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current NASTECH_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (NASTECH_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_nastech_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary NASTECH_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(nastech_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when NASTECH_HOME is a named profile.

    For ``~/.nastech/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        nastech_home: Optional explicit NASTECH_HOME path. Defaults to the current
            ``get_nastech_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this NASTECH_HOME.

    Default ``~/.nastech`` returns ``nastech-gateway`` (backward compatible).
    Profile ``~/.nastech/profiles/coder`` returns ``nastech-gateway-coder``.
    Any other NASTECH_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older NasTech installs that predate the
# nastech-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (nastech-gateway-*.service) and unrelated third-party
# "nastech" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("nastech.service",)
...
def _find_legacy_nastech_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy NasTech gateway units.

    Detects unit files installed by older NasTech versions that used a
    different service name (e.g. ``nastech.service`` before the rename to
    ``nastech-gateway.service``). When both a legacy unit and the current
    ``nastech-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``nastech-gateway-coder.service`` and unrelated third-party
      ``nastech-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``nastech.service`` running an unrelated
...
def has_legacy_nastech_units() -> bool:
    """Return True when any legacy NasTech gateway unit files exist."""
    return bool(_find_legacy_nastech_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy NasTech gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current nastech-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    nastech gateway migrate-legacy")


def remove_legacy_nastech_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy NasTech gateway unit files.

    Iterates over whatever ``_find_legacy_nastech_units()`` returns — which is
    ...
    legacy = _find_legacy_nastech_units()
    if not legacy:
        print("No legacy NasTech gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy NasTech gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: nastech gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo nastech gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    nastech gateway uninstall")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so NasTech can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo nastech gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode NasTech often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.nastech`` → ``ai.nastech.gateway.plist`` (backward compatible).
    Profile ``~/.nastech/profiles/coder`` → ``ai.nastech.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.nastech.gateway-{suffix}" if suffix else "ai.nastech.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.nastech/nastech-agent  -> /home/alice/.nastech/nastech-agent
      /opt/nastech                 -> /opt/nastech  (kept as-is)
...
def _nastech_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current NASTECH_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_nastech_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.nastech                    → /home/alice/.nastech
      /root/.nastech/profiles/coder     → /home/alice/.nastech/profiles/coder
      /opt/custom-nastech               → /opt/custom-nastech  (kept as-is)
    """
    current_nastech = get_nastech_home().resolve()
    current_default = (Path.home() / ".nastech").resolve()
    target_default = Path(target_home_dir) / ".nastech"

    # Default ~/.nastech → remap to target user's default
    if current_nastech == current_default:
        return str(target_default)

    # Profile or subdir of ~/.nastech → preserve the relative structure
    try:
        relative = current_nastech.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.nastech) — keep as-is
        return str(current_nastech)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    nastech_node = nastech_home / "node" / "bin"
    if _is_dir(nastech_node):
        candidates.append(str(nastech_node))
    nastech_nm = nastech_home / "node_modules" / ".bin"
    if _is_dir(nastech_nm):
        candidates.append(str(nastech_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m nastech_cli.main``, so module
    ...
    dir, or a clone that ``nastech update`` later relocates/removes — the path
    ...
    ``NASTECH_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if NASTECH_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        nastech_home = _nastech_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m nastech_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="NASTECH_HOME={nastech_home}"
Restart=always
RestartSec=5
...
"""
    ...
    nastech_home = str(get_nastech_home().resolve())
    profile_arg = _profile_arg(nastech_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only NASTECH_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp NASTECH_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../nastech_test`` path into
    # ``Environment="NASTECH_HOME=..."``. Writing that to the developer's
...
def systemd_install(
    ...
    # Offer to remove legacy units (nastech.service from pre-rename installs)
    # before installing the new nastech-gateway.service. If both remain, they
    ...
    if has_legacy_nastech_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_nastech_units(interactive=False)
            print()
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}nastech gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}nastech gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `nastech gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `nastech gateway status` or logs for final state."
            )
            return
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.nastech.gateway-{suffix}" if suffix else "ai.nastech.gateway"
...
# (the documented `nohup nastech gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m nastech_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active NASTECH_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "nastech_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup nastech gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: nastech gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup nastech gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>nastech_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current NasTech install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  nastech gateway status             # Check status")
    from nastech_constants import display_nastech_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current NasTech install")
    else:
        print("⚠ Service definition is stale relative to the current NasTech install")
        print("  Run: nastech gateway start")
    ...
    if deep:
        log_file = get_nastech_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("NASTECH_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the NasTech gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'nastech' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/nastech/docker/entrypoint.sh before the NasTech command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$NASTECH_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set NASTECH_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `nastech gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set NASTECH_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `nastech gateway restart` which already
    ...
    # `nastech gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ NasTech Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # NASTECH_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and NasTech will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @nastech:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. nastech) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where NasTech delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your NasTech agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address NasTech will use (e.g., nastech@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. NasTech connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: nastech pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and NasTech will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``nastech setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run nastech under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.nastech/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_nastech_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'nastech pairing approve')",
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from nastech_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. NasTech will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. NasTech will store the returned account_id/token in ~/.nastech/.env."
    )
    print_info(
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_nastech_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `nastech pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `nastech pairing approve`."
        )
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``nastech_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from nastech_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.nastech/plugins/
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    print_info("  Or run in foreground:  nastech gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: nastech gateway run")
                print_info(
                    "  For persistence:   tmux new -s nastech 'nastech gateway run'"
                )
                print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`nastech_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # NASTECH_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``nastech gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``nastech gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``nastech gateway run`` are unaffected.
      2. ``NASTECH_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``nastech gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``NASTECH_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    # `docker logs` and ${NASTECH_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if NASTECH_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or NASTECH_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-nastech service's
    # pattern in docker/s6-rc.d/main-nastech/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: nastech gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: nastech gateway run"
                )
                print_info(
            ...
            print()
            print(
                "  nastech gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s nastech 'nastech gateway run'         # persistent via tmux"
            )
            print(
                "  nohup nastech gateway run > ~/.nastech/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: nastech gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: nastech gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: nastech gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        # slot — start them individually via `nastech -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: nastech gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_NASTECH_GATEWAY") == "1":
            print_error(
                ...
                "Use `nastech gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_NASTECH_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `nastech gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: nastech gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  nastech gateway install")
                else:
                    print("To install as a service:")
                    print("  nastech gateway install")
                    print("  sudo nastech gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  nastech gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup nastech gateway run > ~/.nastech/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s nastech 'nastech gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  nastech gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  nastech gateway install  # Install as user service")
                    print(
                        "  sudo nastech gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy NasTech gateway unit files from
        # pre-rename installs (e.g. nastech.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_nastech_units(interactive=not yes, dry_run=dry_run)
