"""Shell completion script generation for nastech CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nastech_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_nastech_profiles() {{
    local profiles_dir="$HOME/.nastech/profiles"
...
_nastech_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nastech_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nastech_completion nastech
"""
...
_nastech_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nastech/profiles" ]]; then
        profiles+=($HOME/.nastech/profiles/*(N/:t))
    fi
...
_nastech() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nastech_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nastech command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "function __nastech_profiles",
        "    echo default",
        "    if test -d $HOME/.nastech/profiles",
        "        for d in $HOME/.nastech/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c nastech -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nastech -f -s p -l profile"
        " -d 'Profile name' -xa '(__nastech_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nastech_profiles)' -d 'Profile name'"
                )
