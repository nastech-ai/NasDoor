...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``nastech model`` CLI picker. Adds network calls
    ...
    - ``force_fresh_nastech_tier``: bypass the short Nous free-tier cache when
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_nastech_tier=force_fresh_nastech_tier,
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_nastech_tier=force_fresh_nastech_tier)
    if capabilities:
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `nastech model` to configure ({auth_type})"
        )
...
def _apply_pricing(
    ...
    *,
    ...
    nastech_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        if slug == "nous":
            try:
                if nastech_free_tier is None:
                    nastech_free_tier = check_nastech_free_tier(
                        force_fresh=force_fresh_nastech_tier
                    )
                row["free_tier"] = bool(nastech_free_tier)
                if nastech_free_tier:
                    _selectable, unavailable = partition_nastech_models_by_tier(
