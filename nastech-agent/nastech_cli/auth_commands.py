...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:nastech_pkce",
    ...
    if provider == "nous":
        ...
        # <nastech-root>/shared/nastech_auth.json (written by any previous
        ...
        shared = auth_mod._read_shared_nastech_state()
        if shared:
            try:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_nastech_state(
        ...
        entry = auth_mod.persist_nastech_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `nastech auth` is called bare."""
