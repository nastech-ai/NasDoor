...
NousAccountInfoSource = Literal["jwt",
                                "account_api", "inference_key", "none", "error"]
...
def nous_portal_billing_url(
        account_info: Optional[NousPortalAccountInfo] = None) -> str:
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"NasTech could not verify your Nous Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but NasTech cannot verify "
        ...
        return (
            f"Log in to Nous Portal to use {capability}: run `nastech model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"NasTech could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"NasTech could not find a Nous Portal account or organisation for this "
...
def _subscription_from_payload(
        value: Any) -> Optional[NousPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
