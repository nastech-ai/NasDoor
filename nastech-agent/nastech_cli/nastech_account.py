...
class NastechPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class NastechPortalAccountInfo:
    logged_in: bool
    source: NastechAccountInfoSource
    fresh: bool
    ...
    subscription: Optional[NastechPortalSubscriptionInfo] = None
...
def nous_portal_billing_url(
    ...
    try:
        ...
        DEFAULT_NOUS_PORTAL_URL = "https://portal.nastechai.com"
...
def format_nous_portal_entitlement_message(
    account_info: Optional[NastechPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing NasTech Portal tool-gateway entitlement.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current NasTech Portal "
    ...
    if account_info is None:
        return (
            f"NasTech could not verify your NasTech Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"NasTech credentials are configured, but NasTech cannot verify "
                f"your NasTech Portal paid access for {capability}. Log in with "
                f"`nastech model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to NasTech Portal to use {capability}: run `nastech model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"NasTech could not verify your NasTech Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `nastech model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"NasTech could not find a NasTech Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `nastech model` to "
            f"authenticate again; if the problem persists, contact NasTech support."
        )
    ...
    return (
        f"Your NasTech Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


format_nastech_portal_entitlement_message = format_nous_portal_entitlement_message


def _no_paid_access_message(
    account_info: NastechPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current NasTech Portal plan does not include paid service access, "
    ...
    *,
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        if pool_info is not None:
            return pool_info
        return NastechPortalAccountInfo(
            logged_in=False,
            source="none",
            fresh=False,
            portal_base_url=portal_base_url,
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[NastechPortalAccountInfo]:
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_nastech_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _fetch_nastech_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
# NasTech-branded aliases — these are the canonical public names used
# throughout the codebase. The underlying nous_* functions remain for
# backward-compat with any code that still references the old names.
get_nastech_portal_account_info = get_nous_portal_account_info
reset_nastech_portal_account_info_cache = reset_nous_portal_account_info_cache
