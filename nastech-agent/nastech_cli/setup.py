...
from nastech_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nastech_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nastech setup')")
...
def _print_setup_summary(config: dict, nastech_home):
    ...
    tool_status = []
    subscription_features = get_nastech_subscription_features(config)
    ...
    # Spotify (OAuth via nastech auth spotify — check auth.json, not env vars)
    try:
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nastech model``)
    ...
    provider added to ``nastech model`` is automatically available here.
    ...
    # Delegate to the shared nastech model flow — handles provider picker,
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nastech model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nastech setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nastech setup terminal` / config.yaml.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NASTECH_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NASTECH_MAX_ITERATIONS")
    ...
    print_info("  Run `nastech setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NASTECH_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from nastech_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in nastech_home:
            profile_name = nastech_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nastech update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        print_info(
            "   Re-run `nastech slack manifest --write` anytime to refresh after "
            "NasTech adds new commands."
        )
...
def _setup_qqbot():
    ...
    from nastech_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_nastech_units():
            print_legacy_unit_warning()
            print()
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nastech setup tools` and `nastech tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from nastech_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nastech_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_nastech migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nastech", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(nastech_home: Path) -> bool:
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nastech claw migrate"
        )
        print_info(
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print_info("  Run `nastech portal info` to inspect routing.")
    print_info("  Run `nastech` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nastech setup           — full or quick (auto-detected)
      nastech setup model     — just model/provider
      nastech setup tts       — just text-to-speech
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, nastech_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(nastech_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, nastech_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {nastech_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nastech config edit'")
    ...
    # Tune later with `nastech setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, nastech_home)
...
def _run_first_time_quick_setup(config: dict, nastech_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nastech setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nastech setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nastech setup gateway")
    print()
...
def _run_quick_setup(config: dict, nastech_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nastech setup' and choose 'Full Setup' to reconfigure,")
