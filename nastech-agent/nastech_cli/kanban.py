"""CLI for the NasTech Kanban board — ``nastech kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/nastech-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from nastech_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this NASTECH_HOME and its
    ...
    Used by ``nastech kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`nastech kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(
        parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across NasTech profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://nastech-agent.nastechai.com/docs/user-guide/features/kanban "
            "or docs/nastech-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # NASTECH_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `nastech kanban boards switch <slug>` or the "
            "NASTECH_KANBAN_BOARD env var). Use `nastech kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_swarm.add_argument(
        "--synthesizer",
        required=True,
        help="Synthesizer/writer profile")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $NASTECH_PROFILE as assignee")
    ...
    p_assign.add_argument(
        "profile",
        help="Profile name (or 'none' to unassign)")
    ...
    p_unlink = sub.add_parser(
        "unlink", help="Remove a parent->child dependency")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $NASTECH_PROFILE or 'user')")
    ...
    p_schedule.add_argument(
        "reason",
        nargs="*",
        help="Reason/timing note (also appended as a comment)")
    ...
    p_unblock = sub.add_parser(
        "unblock",
        help="Return one or more blocked/scheduled tasks to ready")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `nastech gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.nastech/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $NASTECH_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $NASTECH_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``nastech kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: nastech kanban <action> [options]\n"
                "Run 'nastech kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using NASTECH_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `nastech kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # NASTECH_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        try:
            ...
            print(
                f"kanban: could not initialize database: {exc}",
                file=sys.stderr)
            return 1
...
def _profile_author() -> str:
    ...
    for env in ("NASTECH_PROFILE_NAME", "NASTECH_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (nastech kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``nastech kanban boards <action>``.
    ...
    DB, so a fresh NASTECH_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `nastech kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `nastech kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(
            f"  Use `nastech kanban boards switch {
                meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `nastech kanban boards delete <slug>` (alias), the
...
def _parse_duration(val) -> Optional[int]:
    ...
    raise ValueError(
        f"malformed duration {
            val!r} (expected 30s, 5m, 2h, 1d, or a number)")
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `nastech profile
    ...
    try:
        ...
        if result:
            ...
            if copied:
                print(
                    f"Seeded skill(s) into profile {profile_name}: {
                        ', '.join(copied)}")
    except Exception:
        pass  # best-effort
    ...
    if profiles:
        ...
        print("No profiles found under ~/.nastech/profiles/.")
        print("Create one with `nastech -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  nastech gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `nastech -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        print(json.dumps([_task_to_dict(t)
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.max_retries is not None:
        ...
        if cfg_val is not None and int(cfg_val) != kb.DEFAULT_FAILURE_LIMIT:
            print(
                f"  max-retries: {int(cfg_val)} (config kanban.failure_limit)")
        else:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``nastech kanban show <id>``
...
def _cmd_assign(args: argparse.Namespace) -> int:
    profile = None if args.profile.lower() in {
        "none", "-", "null"} else args.profile
    with kb.connect_closing() as conn:
    ...
    with kb.connect_closing() as conn:
        ...
        if sev:
            for tid in list(diags_by_task.keys()):
                kept = [
                    d for d in diags_by_task[tid] if kd.SEVERITY_ORDER.index(
                        d.severity) >= kd.SEVERITY_ORDER.index(sev)]
                if kept:
...
def _cmd_unlink(args: argparse.Namespace) -> int:
    ...
    if not ok:
        print(
            f"No such link: {args.parent_id} -> {args.child_id}", file=sys.stderr)
        return 1
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("NASTECH_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("NASTECH_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        default_assignee = (_kanban_cfg.get(
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "nastech kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    nastech gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`nastech kanban list --status ready` / "
                    f"`nastech kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real NasTech profile (i.e. one the
    ...
    try:
        while True:
            ...
            for r in rows:
                ...
                try:
                    payload = json.loads(
                        r["payload"]) if r["payload"] else None
                except Exception:
                    payload = None
...
def _cmd_stats(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    print("By status:")
    for k in ("triage", "todo", "scheduled",
...
def _cmd_notify_list(args: argparse.Namespace) -> int:
    ...
    for s in subs:
        ...
        owner = f"  owner={s['notifier_profile']}" if s.get(
            "notifier_profile") else ""
