"""CLI handlers for ``nastech migrate ...``.

Currently exposes only ``nastech migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``nastech migrate <subtype>``."""
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `nastech migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `nastech doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
