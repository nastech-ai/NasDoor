"""MCP picker — interactive `nastech mcp picker` (also the default `nastech mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``nastech mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `nastech plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new NasTech session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `nastech mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: nastech mcp install <name>    Picker: nastech mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their NasTech is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer NasTech — run `nastech update` "
...
def show_catalog() -> None:
    """`nastech mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`nastech mcp picker` (and default `nastech mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`nastech mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `nastech mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
