...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m nastech_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current NasTech profile."""
    from nastech_cli.gateway import _profile_arg
    ...
    *,
    ...
    try:
        if start_now is not None:
            os.environ["NASTECH_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["NASTECH_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["NASTECH_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("NASTECH_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("NASTECH_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("NASTECH_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``NasTech_Gateway``
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\nastech\\gateway-service\\<task_name>.cmd``
    (or ``<NASTECH_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    NasTech installs stay self-contained).
    """
    _assert_windows()
    from nastech_cli.config import get_nastech_home

    script_dir = Path(get_nastech_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``NASTECH_HOME`` whenever it
    ...
    source checkout only if ``NASTECH_HOME`` cannot be resolved yet.
    """
    from nastech_cli.config import get_nastech_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NASTECH_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m nastech_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "NASTECH_HOME={nastech_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "NASTECH_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports nastech_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "nastech_cli.main"]
    if profile_arg:
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "nastech_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m nastech_cli.main gateway run``
    ...
    log_dir = Path(get_nastech_home()) / "logs"
...
def _install_startup_fallback(
    ...
    # Re-running `nastech -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"nastech {profile_arg} gateway start" if profile_arg else "nastech gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: nastech gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: nastech gateway start")
        _print_next_steps()
        return
...
def _print_next_steps() -> None:
    ...
    nastech_home = Path(get_nastech_home()).resolve()
    print()
    print("Next steps:")
    print("  nastech gateway status                      # Check status")
    print(f"  type {nastech_home}\\logs\\gateway.log       # View logs")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_nastech_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: nastech gateway start")
            return
