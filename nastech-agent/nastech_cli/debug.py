"""``nastech debug`` debug tools for NasTech Agent.

Currently supports:
    nastech debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[nastech debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.nastech/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``nastech debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``nastech debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, NasTech version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `nastech debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nastech-agent/debug-share"},
    )
...
def _schedule_auto_delete(
    ...
    every ``nastech debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.nastech/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``nastech debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nastech-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----NasTechDebugBoundary9f3c"
...
def _capture_dump() -> str:
    """Run ``nastech dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``nastech dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``nastech debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "nastech debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nastech debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'nastech debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``nastech debug`` call.
    ...
    # best-effort — any failure is swallowed so ``nastech debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: nastech debug <command>")
        print()
        print("Commands:")
