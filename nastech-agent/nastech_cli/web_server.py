"""
NasTech Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m nastech_cli.main web          # Start on http://127.0.0.1:9119
...
try:
    ...
    # running `nastech dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    desktop app spawns a ``nastech dashboard`` backend, not a gateway, so a cron
    ...
    NASTECH_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (NASTECH_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `nastech
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("NASTECH_DESKTOP") == "1":
...
app = FastAPI(title="NasTech Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# NASTECH_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
#
# This list is defined in ``nastech_cli.dashboard_auth.public_paths`` so the
...
async def _dashboard_auth_gate(request: Request, call_next):
    from nastech_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates NasTech (no terminal prompt), "
...
async def get_status():
    ...
    # SPA's StatusPage can show "OAuth gate ON via NasTech" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from nastech_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the nastech home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_nastech_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    try:
        ...
        auth = get_nastech_auth_status() or {}
    except Exception:
        auth = {}
    ...
    try:
        ...
        feats = get_nastech_subscription_features(cfg)
...
async def run_prompt_size():
    try:
        proc = _spawn_nastech_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_nastech_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_nastech_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
# streamed to a per-action log file under ``~/.nastech/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_nastech_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "nastech-update": "nastech-update.log",
...
def _spawn_nastech_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "NASTECH_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``nastech gateway restart`` in the background."""
    try:
...
@app.post("/api/nastech/update")
async def update_nastech():
    """Kick off ``nastech update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("nastech-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "nastech-update",
...
@app.get("/api/nastech/update/check")
async def check_nastech_update(force: bool = False):
    """Report whether a NasTech update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/nastech/update`` actually runs ``nastech update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed NasTech version string
    ...
    try:
        ...
        if force:
            try:
                (get_nastech_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.nastech/config.yaml`` under ``tts.``.
    """
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    NasTech supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in nastech_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `nastech model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `nastech model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_nastech_models_by_tier(
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.nastech/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (nastech_cli/main.py
            # prompt_enable_tool_gateway / tools_config apply_nastech_managed_defaults):
            # auto-route any *unconfigured* tools through the Nous Tool Gateway.
            # This is purely additive — apply_nastech_managed_defaults skips every
            ...
            if provider.strip().lower() == "nous":
                try:
                    ...
                    changed = apply_nastech_managed_defaults(
                        cfg,
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run NasTech from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect NasTech to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use NasTech from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect NasTech to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use NasTech in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use NasTech through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from NasTech via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to NasTech through an IMAP/SMTP mailbox.",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect NasTech to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use NasTech inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use NasTech through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect NasTech to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect NasTech to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose NasTech as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://nastech-agent.nastechai.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.nastech-agent.nastechai.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"NasTechDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "NasTech Agent").strip() or "NasTech Agent"
...
# the canonical ``nastech auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    NasTech resolves Anthropic creds in this order at runtime:
    1. ``~/.nastech/.anthropic_oauth.json`` — NasTech-managed PKCE flow
    ...
    try:
        ...
        read_nastech_oauth_credentials = None  # type: ignore
        _NASTECH_OAUTH_FILE = None  # type: ignore

    nastech_creds = None
    if read_nastech_oauth_credentials:
        try:
            nastech_creds = read_nastech_oauth_credentials()
        except Exception:
            nastech_creds = None
    if nastech_creds and nastech_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "nastech_pkce",
            "source_label": f"NasTech PKCE ({_NASTECH_OAUTH_FILE})",
            "token_preview": _truncate_token(nastech_creds.get("accessToken")),
            "expires_at": nastech_creds.get("expiresAt"),
            "has_refresh_token": bool(nastech_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into NasTech even
    when they also have a separate NasTech-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "nastech auth add nous",
        "docs_url": "https://portal.nastechai.com",
        "status_fn": None,  # dispatched via auth.get_nastech_auth_status
    },
    {
        "id": "openai-codex",
        "name": "OpenAI OAuth (ChatGPT)",
        "flow": "device_code",
        "cli_command": "nastech auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "nastech auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "nastech auth add xai-oauth",
        "docs_url": "https://nastech-agent.nastechai.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "nastech auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from nastech_cli import auth as hauth
        if provider_id == "nous":
            raw = hauth.get_nastech_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "nastech_portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("nastech_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _NASTECH_OAUTH_FILE
            if _NASTECH_OAUTH_FILE.exists():
                _NASTECH_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from nastech_cli.auth import clear_provider_auth
...
#          → persists to ~/.nastech/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``nastech auth add anthropic``.
    """
    from agent.anthropic_adapter import _NASTECH_OAUTH_FILE
    payload = {
    ...
    _NASTECH_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _NASTECH_OAUTH_FILE.with_name(
        f"{_NASTECH_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _NASTECH_OAUTH_FILE)
        try:
            _NASTECH_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nastech-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        def _do_nastech_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_nastech_device_request
        )
        ...
        threading.Thread(
            target=_nastech_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
...
# the tokens exactly like `nastech auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `nastech auth add xai-oauth`'s credential-pool insert.
...
def _nastech_poller(session_id: str) -> None:
    ...
    try:
        ...
        full_state = refresh_nastech_oauth_from_state(
            auth_state,
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_nastech_poller` but calls the MiniMax-specific token endpoint,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from nastech_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `nastech sessions stats`).
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, NASTECH_HOME)."""
    from nastech_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["nastech_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process NASTECH_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (nastech_cli.mcp_config), so
# servers managed here show up under `nastech mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from nastech_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from nastech_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from nastech_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `nastech mcp catalog` / `nastech mcp install` read.
    """
    try:
...
#
# Wraps the same JSON store the CLI uses (nastech_cli.webhook); the webhook
...
# spawn the real `nastech gateway <verb>` so behaviour matches the CLI exactly.
...
# `nastech memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `nastech memory setup` to configure a new one.",
            )
...
async def list_checkpoints():
    ...
    # Checkpoints live under <nastech_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_nastech_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_setup_command(name: str) -> str:
    ...
    return "nastech setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local NASTECH_HOME override so the write lands in the target
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``nastech profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from NASTECH_HOME).
    """
    from nastech_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``nastech profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the NASTECH_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``nastech profile describe
    <name> --auto``.
...
async def toggle_skill(body: SkillToggle):
    from nastech_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``nastech tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``nastech tools`` picker shows
    ...
    if cat:
        for prov in _visible_providers(cat, config, force_fresh=True):
            ...
            providers.append({
                ...
                "requires_nastech_auth": bool(prov.get("requires_nastech_auth")),
                "is_active": is_active,
            })
    return {
        "name": name,
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``nastech tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.nastech/.env`` via ``save_env_value`` —
    the same store ``nastech tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``nastech tools post-setup <key>`` and the frontend tails the log via
...
#
# The endpoint spawns the same ``nastech --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``nastech --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``NASTECH_TUI_RESUME`` env var —
    matching what ``nastech_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``NASTECH_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``NASTECH_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from nastech_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("NASTECH_TUI_DISABLE_MOUSE", "1")
    env.setdefault("NASTECH_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["NASTECH_TUI_RESUME"] = resume

    if sidecar_url:
        env["NASTECH_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["NASTECH_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from nastech_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall NasTech inside WSL2 to use the dashboard's /chat "
...
# NASTECH_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`nastech_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from nastech_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/nastech/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /nastech`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__NASTECH_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/nastech``)
        ...
        ``__NASTECH_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__NASTECH_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__NASTECH_BASE_PATH__="{prefix}";'
                f"window.__NASTECH_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__NASTECH_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__NASTECH_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__NASTECH_BASE_PATH__="{prefix}";'
                f"window.__NASTECH_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/nastech`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the NasTech backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.nastech/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.nastech/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_nastech_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.nastech/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as nastech_cli.plugins):
    1. User plugins:    ~/.nastech/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.nastech/plugins/  (only if NASTECH_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from nastech_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_nastech_home() / "plugins", "user"),
    ...
    # ``nastech_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("NASTECH_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".nastech" / "plugins", "project"))
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"nastech auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nastech_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nastech_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.nastech/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.nastech/plugins/ if you trust it)",
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing NASTECH_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with NasTech Agent exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import nous as _nastech_plugin

                if _nastech_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • nous: {_nastech_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
    ...
    print(f"  NasTech Web UI → http://{host}:{port}")
