...
- ``nastech_cli.providers``        -- canonical provider identity + overlays
- ``nastech_cli.model_normalize``  -- per-provider name formatting
...
_NASTECH_MODEL_WARNING = (
    "NasTech 3 & 4 models are NOT agentic and are not designed "
    "for use with NasTech Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real NasTech 3 / NasTech 4 chat families.
...
#   NasTech/NasTech-3-Llama-3.1-70B, nastech-4-405b, openrouter/nastech3:70b
# Negative examples it must NOT match:
#   nastech-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NASTECH_NASTECH_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nastech[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nastech_nastech_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous NasTech 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NASTECH_NASTECH_NON_AGENTIC_RE.search(model_name))


def _check_nastech_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous NasTech 3/4 chat model."""
    if is_nastech_nastech_non_agentic(model_name):
        return _NASTECH_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nastech config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nastech_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nastech doctor' — config issues detected:"
        ...
        from nastech_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from nastech_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    nastech_warn = _check_nastech_model_warning(new_model)
    if nastech_warn:
        warnings.append(nastech_warn)
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_nastech_tier: bool = False,
    max_models: int = 8,
    current_model: str = "",
) -> List[dict]:
    """Detect which providers have credentials and list their curated models.

    Uses the curated model lists from nastech_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_nastech_tier`` bypasses the short Nous tier cache for explicit
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nastech_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nastech provider ID
    ...
    curated["nous"] = get_curated_nastech_model_ids()
    ...
    # --- 1. Check NasTech-mapped providers ---
    from nastech_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from nastech_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for nastech_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(nastech_id)
        if (
            _alias_target
            and _alias_target != nastech_id
        ...
        pconfig = PROVIDER_REGISTRY.get(nastech_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (NASTECH_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                ...
                if store and store.get("credential_pool", {}).get(nastech_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `nastech model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(nastech_id)
        if not model_ids:
            model_ids = curated.get(nastech_id, [])
            if nastech_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nastech_id, model_ids)
        total = len(model_ids)
    ...
    # while _PROVIDER_MODELS and config.yaml use NasTech IDs ("copilot").
    _mdev_to_nastech = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NASTECH_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve NasTech slug — e.g. "github-copilot" → "copilot"
        nastech_slug = _mdev_to_nastech.get(pid, pid)
        if nastech_slug.lower() in seen_slugs:
            continue
        ...
        # configured.
        if not has_creds and nastech_slug == "anthropic":
            try:
                ...
                if (nastech_creds and nastech_creds.get("accessToken")) or \
        ...
        if nastech_slug in {"openai-codex", "copilot", "copilot-acp"}:
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `nastech model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                _portal = ""
                try:
                    _st = _nastech_state("nous") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _nastech_free(force_fresh=force_fresh_nastech_tier):
        ...
        results.append({
            "slug": nastech_slug,
            "name": get_label(nastech_slug),
            "is_current": nastech_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nastech",
        ...
        seen_slugs.add(nastech_slug.lower())
        _record_builtin_endpoint(nastech_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NASTECH_OVERLAYS (keeps /model in sync
    # with `nastech model`).
    try:
        from nastech_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is NasTech's canonical write key (matches
            ...
            # NasTech writes ``models:`` as a dict keyed by model id
            # (see nastech_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # NasTech's own writer uses; a hyphen variant is accepted
            ...
            # active model. NasTech's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nastech_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`nastech_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from nastech_cli.models import fetch_openrouter_models
