...
Storage mirrors ``cron/jobs.py``: ``~/.nastech/cron/suggestions.json``, atomic
...
CRON_DIR = get_nastech_home().resolve() / "cron"
    ...
    *,
    ...
    with _suggestions_lock:
        ...
        record = {
            ...
            "created_at": _nastech_now().isoformat(),
        }
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _nastech_now().isoformat()
                changed = True
                break
        if changed:
            _save_raw(suggestions)
