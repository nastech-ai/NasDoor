...
Uses a file-based lock (~/.nastech/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `nastech update` reloads
# the module) fail with ModuleNotFoundError for nastech_time et al.
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``nastech tools`` config for the ``cron`` platform.
...
# Backward-compatible module override used by tests and emergency
# monkeypatches.
_nastech_home: Path | None = None


def _get_nastech_home() -> Path:
    """Resolve NasTech home dynamically while preserving test monkeypatch hooks."""
    return _nastech_home or get_nastech_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    nastech_home = _get_nastech_home()
    lock_dir = nastech_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific NasTech profile.
    ...
    the scheduler's test/override hook and a context-local NasTech home override
    both point at the resolved profile directory so _get_nastech_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_nastech_home() callers agree on the same home.
    ...
    global _nastech_home
    prior_override = _nastech_home
    env_snapshot = os.environ.copy()

    from nastech_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    override_token = None
    try:
        override_token = set_nastech_home_override(profile_home)
        _nastech_home = profile_home
        logger.info(
            "Job '%s': using NasTech profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _nastech_home = prior_override
        if override_token is not None:
            reset_nastech_home_override(override_token)
...
def _cron_job_origin_log_suffix(job: dict) -> str:
    ...
    for key in ("platform", "chat_id", "thread_id",
                "source_ip", "remote", "forwarded_for"):
...
def _resolve_single_delivery_target(
        job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        try:
            ...
            if resolved:
                parsed_chat_id, parsed_thread_id, resolved_is_explicit = _parse_target_ref(
...
def _resolve_delivery_targets(job: dict) -> List[dict]:
    ...
    for part in parts:
        ...
        if target:
            key = (
                target["platform"].lower(), str(
                    target["chat_id"]), target.get("thread_id"))
            if key not in seen:
                seen.add(key)
...
# via should_send_media_as_audio() so Telegram-specific rules stay in one
# place.
...
def _send_media_via_adapter(
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            route_platform = platform if platform is not None else getattr(
                adapter, "platform", None)
            if should_send_media_as_audio(
                    route_platform, ext, is_voice=_is_voice):
            ...
            if result and not getattr(result, "success", True):
                logger.warning(
                    ...
                    job.get(
                        "id", "?"), media_path, getattr(
                        result, "error", "unknown"),
                )
        except Exception as e:
            logger.warning(
                "Job '%s': failed to send media %s: %s", job.get(
                    "id", "?"), media_path, e)


def _deliver_result(job: dict, content: str, adapters=None,
                    loop=None) -> Optional[str]:
    """
    ...
    if not targets:
        if job.get("deliver", "local") != "local":
            msg = f"no delivery target resolved for deliver={
                job.get(
                    'deliver',
                    'local')}"
    ...
    for target in targets:
        ...
        delivered = False
        if runtime_adapter is not None and loop is not None and getattr(
                ...
                if text_to_send:
                    ...
                    if future is None:
                        ...
                        try:
                            ...
                            future.cancel()
                            raise
                        if send_result and not getattr(
                            ...
                            requested_thread_id = send_result.raw_response.get(
                                "requested_thread_id") or thread_id
                            msg = (
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("NASTECH_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning(
                "Invalid NASTECH_CRON_SCRIPT_TIMEOUT=%r; using config/default",
                env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within NASTECH_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against NASTECH_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_nastech_home() / "scripts"
    ...
    # escape — scripts MUST reside within NASTECH_HOME/scripts/.
    try:
    ...
    if suffix in {".sh", ".bash"}:
        ...
        if _bash is None:
            return False, (
                f"Cannot run .sh/.bash script {
                    path.name!r}: bash not found on PATH. "
    ...
    run_env["NASTECH_HOME"] = str(_get_nastech_home())
    try:
...
def _parse_wake_gate(script_output: str) -> bool:
    ...
    if not script_output:
        return True
    stripped_lines = [
        line for line in script_output.splitlines() if line.strip()]
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    for skill_name in skill_names:
        ...
        if not loaded.get("success"):
            error = loaded.get(
                "error") or f"Failed to load skill '{skill_name}'"
            logger.warning(
                "Cron job '%s': skill not found, skipping — %s", job.get(
                    "name", job.get("id")), error)
            skipped.append(skill_name)
            continue
...
def _scan_assembled_cron_prompt(
        assembled: str, job: dict, *, has_skills: bool = False) -> str:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    """
    job_id = job["id"]
    job_name = str(job.get("name") or job.get(
        "prompt") or job_id or "cron job")
    ...
    if job.get("no_agent"):
        ...
        now_iso = _nastech_now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        if not output.strip():
            logger.info(
                "Job '%s' (no_agent): empty stdout — silent run",
                job_id)
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_nastech_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    if prompt is None:
        logger.info(
            "Job '%s': script produced no output, skipping AI call.",
            job_name)
        return True, "", SILENT_MARKER, None
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{
        _nastech_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["NASTECH_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed NASTECH_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads NASTECH_SESSION_PLATFORM
    #     and NASTECH_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via NASTECH_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     NASTECH_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read NASTECH_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the NASTECH_CRON_AUTO_DELIVER_* vars set
    # below, so clearing NASTECH_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "NASTECH_CRON_AUTO_DELIVER_PLATFORM",
        "NASTECH_CRON_AUTO_DELIVER_CHAT_ID",
        "NASTECH_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_nastech_home() / ".env"),
                        override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_nastech_home() / ".env"),
                        override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["NASTECH_CRON_AUTO_DELIVER_PLATFORM"].set(
                delivery_target["platform"])
            _VAR_MAP["NASTECH_CRON_AUTO_DELIVER_CHAT_ID"].set(
                str(delivery_target["chat_id"]))
            _VAR_MAP["NASTECH_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("NASTECH_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider
        # routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_nastech_home() / "config.yaml")
        ...
        prefill_file = os.getenv(
            "NASTECH_PREFILL_MESSAGES_FILE", "") or _cfg.get(
            "prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_nastech_home() / pfpath
            if pfpath.exists():
                try:
        ...
        # Max iterations
        max_iterations = _cfg.get("agent", {}).get(
            "max_turns") or _cfg.get("max_turns") or 90
        ...
        try:
            # Do not inject NASTECH_INFERENCE_PROVIDER here. resolve_runtime_provider()
            ...
            # Primary provider auth failed — try fallback chain before giving
            # up.
            logger.warning(
        ...
        fallback_model = _cfg.get("fallback_providers") or _cfg.get(
        ...
        agent = AIAgent(
            ...
            openrouter_min_coding_score=(
                _cfg.get("openrouter") or {}).get("min_coding_score"),
            enabled_toolsets=_resolve_cron_enabled_toolsets(job, _cfg),
            disabled_toolsets=_resolve_cron_disabled_toolsets(_cfg),
            quiet_mode=True,
            # Cron jobs should always inherit the user's SOUL.md identity from
            # NASTECH_HOME. When a workdir is configured, also inject project
        ...
        # override via NASTECH_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("NASTECH_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid NASTECH_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
        ...
        # Guard against non-dict returns from run_conversation under error
        # conditions
        if not isinstance(result, dict):
            raise RuntimeError(
                f"agent.run_conversation returned {
        ...
        # Strip leaked placeholder text that upstream may inject on empty
        # completions.
...
**Job ID:** {job_id}
**Run Time:** {_nastech_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        try:
            ...
            logger.debug(
                "Job '%s': failed to close agent resources: %s",
                job_id,
                e)
        ...
        try:
            ...
            logger.debug(
                "Job '%s': failed to reap stale auxiliary clients: %s",
                job_id,
                e)
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Set NASTECH_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("NASTECH_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning(
                "Invalid NASTECH_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        def _process_job(job: dict) -> bool:
            ...
            try:
                ...
                deliver_content = final_response if success else f"⚠️ Cron job '{
                    job.get(
                        'name',
                        job['id'])}' failed:\n{error}"
                ...
                if should_deliver:
                    try:
                        ...
                        logger.error(
                            "Delivery failed for job %s: %s", job["id"], de)
        ...
        # NasTech home override, scheduler _nastech_home hook, and temporary
