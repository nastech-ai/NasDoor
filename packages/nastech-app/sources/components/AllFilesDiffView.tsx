    ...
    (async () => {
      const fetched = await Promise.all(
        toFetch.map(async (file): Promise<FileDiffResult> => {
          ...
          const gitDiffPath = resolved?.withinSessionRoot
          ...
          try {
            if (file.status === "untracked") {
              ...
              const res = await sessionReadFile(
                sessionId,
            ...
            if (!res.success) {
              return {
                file,
                content: null,
                error: res.error || "Failed to fetch diff",
              };
            }
            return {
              file,
              content: { kind: "patch", patch: res.stdout ?? "" },
              error: null,
            };
          } catch (err) {
            return {
...
const DiffHeaderRight = React.memo(function DiffHeaderRight({
  ...
  return (
    <>
      <Text
        style={[styles.headerRightCount, { color: theme.colors.textSecondary }]}
      >
...
const FileDiffSection = React.memo(function FileDiffSection({
  ...
  const stats = React.useMemo(() => {
    ...
    const lineCount =
      content.contents === "" ? 0 : content.contents.split("\n").length;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {file.status === "deleted" && (
          <Text style={[styles.statusBadge, { color: "#FF3B30" }]}>
            deleted
          </Text>
        )}
        ...
        {stats && (stats.additions > 0 || stats.deletions > 0) && (
          <View style={styles.stats}>
            {stats.additions > 0 && (
              <Text style={styles.added}>+{stats.additions}</Text>
            )}
            {stats.deletions > 0 && (
              <Text style={styles.removed}>-{stats.deletions}</Text>
            )}
          </View>
        )}
      </Pressable>
...
const DiffStyleToggle = React.memo<{
  value: "unified" | "split";
