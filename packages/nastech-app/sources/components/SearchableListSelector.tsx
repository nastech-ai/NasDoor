 ...
 * />
 */
export function SearchableListSelector<T>(
  props: SearchableListSelectorProps<T>,
) {
  ...
  const isControlled =
    collapsedSections !== undefined && onCollapsedSectionsChange !== undefined;
  ...
  const [internalShowRecentSection, setInternalShowRecentSection] =
    React.useState(false);
  const [internalShowFavoritesSection, setInternalShowFavoritesSection] =
    React.useState(false);
  const [internalShowAllItemsSection, setInternalShowAllItemsSection] =
    React.useState(true);

  // Use controlled or uncontrolled state
  const showRecentSection = isControlled
    ? !collapsedSections?.recent
    : internalShowRecentSection;
  const showFavoritesSection = isControlled
    ? !collapsedSections?.favorites
    : internalShowFavoritesSection;
  const showAllItemsSection = isControlled
    ? !collapsedSections?.all
    : internalShowAllItemsSection;
  ...
  const filteredRecentItems = React.useMemo(() => {
    ...
    return recentItems.filter((item) =>
      config.filterItem(item, inputText, context),
    );
  ...
  const filteredFavoriteItems = React.useMemo(() => {
    ...
    if (
      favoriteItems.some(
        (item) => config.formatForDisplay(item, context) === inputText,
      )
    ) {
      return favoriteItems; // Show all favorites, don't filter
    }
  ...
  const renderItem = (
    ...
    return (
      <Item
        ...
        selected={isSelected}
        showDivider={
          showDividerOverride !== undefined ? showDividerOverride : !isLast
        }
        style={[
