...
const AgentInputStatusRow = React.memo(function AgentInputStatusRow(
  p: StatusRowProps,
) {
  const { theme } = useUnistyles();
  const showPermissionBadge =
    !!p.displayPermissionMode &&
  ...
  return (
    ...
    >
              ...
              <>
                ...
                >
                  <Text
                    style={{
                      fontSize: 11,
                      color: p.connectionStatus.cliStatus.claude
                ...
                >
                  <Text
                    style={{
                      fontSize: 11,
                      color: p.connectionStatus.cliStatus.codex
                        ? theme.colors.success
                        : theme.colors.textDestructive,
                      ...Typography.default(),
                    }}
                ...
                {p.connectionStatus.cliStatus.gemini !== undefined && (
                  ...
                  >
                    <Text
                      style={{
                        fontSize: 11,
                        color: p.connectionStatus.cliStatus.gemini
                          ? theme.colors.success
                          : theme.colors.textDestructive,
                        ...Typography.default(),
                      }}
      ...
      {showPermissionBadge &&
        (() => {
          const permColor = p.isSandboxedYoloMode
            ? "#4169E1"
            : p.permissionModeKey === "acceptEdits"
              ? theme.colors.permission.acceptEdits
              : p.permissionModeKey === "bypassPermissions"
                ? theme.colors.permission.bypass
                : p.permissionModeKey === "plan"
                  ? theme.colors.permission.plan
                  : p.permissionModeKey === "read-only"
                    ? theme.colors.permission.readOnly
                    : p.permissionModeKey === "safe-yolo"
                      ? theme.colors.permission.safeYolo
                      : p.permissionModeKey === "yolo"
...
const AgentInputContextChips = React.memo(function AgentInputContextChips(
  p: ContextChipsProps,
) {
  const { theme } = useUnistyles();
...
export const AgentInput = React.memo(
  React.forwardRef<MultiTextInputHandle, AgentInputProps>((props, ref) => {
    ...
    const canPressSendButton =
      !props.isSending &&
      !props.isSendDisabled &&
      (isSendBlocked
        ? hasText || hasImages
        : hasText || hasImages || !!props.onMicPress);
    ...
    const isOpenClaw =
      props.metadata?.flavor === "openclaw" || props.agentType === "openclaw";
    ...
    React.useEffect(() => {
      ...
      const handlePaste = async (e: ClipboardEvent) => {
        ...
        const { getImagesFromClipboard, fileToAttachmentPreview } =
          await import("@/utils/pasteImages.web");
      ...
      const handleDrop = async (e: DragEvent) => {
        if (!isFileDrag(e)) return;
        e.preventDefault();
        const { getImagesFromDrop, fileToAttachmentPreview } =
          await import("@/utils/pasteImages.web");
    ...
    const [inputState, setInputState] = React.useState<TextInputState>(() => ({
      ...
      selection: {
        start: props.initialValue.length,
        end: props.initialValue.length,
      },
    }));
                        ...
                        {props.onPickImages && (
                          ...
                          >
                            <Ionicons
                              ...
                              size={16}
                              color={
                                (props.selectedImages?.length ?? 0) > 0
                                  ? theme.colors.radio.active
