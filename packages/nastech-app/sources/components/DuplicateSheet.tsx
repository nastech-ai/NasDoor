...
export const DuplicateSheet = React.memo(function DuplicateSheet(
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      const result = await claudeListRewindPoints({
        machineId,
        directory,
        claudeSessionId,
      });
  ...
  React.useEffect(() => {
    if (
      points &&
      selectedUuid &&
      !points.some((p) => p.uuid === selectedUuid)
    ) {
    ...
    <View style={styles.sheet}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          {t("session.duplicateSheetSubtitle")}
        </Text>
      </View>
