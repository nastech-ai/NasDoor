...
function getPatchChanges(input: any): Record<string, CodexPatchEntry> | null {
  ...
  if (
    input?.fileChanges &&
    typeof input.fileChanges === "object" &&
    !Array.isArray(input.fileChanges)
  ) {
      ...
      <>
        {entries.map(([file, change]) => {
          ...
          const movePath = change.kind?.move_path
            ? resolvePath(change.kind.move_path, metadata)
            : null;
            ...
            <ToolSectionView key={file} fullWidth>
              <View style={styles.patchContainer}>
                <View style={styles.fileHeader}>
                  ...
                  {movePath ? (
                    <Text style={styles.movePath}>{movePath}</Text>
                  ) : null}
                </View>
                {diffInput?.kind === "patch" ? (
                  <ToolDiffView patch={diffInput.patch} fileName={fileName} />
                ) : diffInput?.kind === "pair" &&
                  (diffInput.oldText.length > 0 ||
                    diffInput.newText.length > 0) ? (
                  <ToolDiffView
