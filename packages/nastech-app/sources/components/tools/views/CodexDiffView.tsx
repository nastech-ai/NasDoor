...
export const CodexDiffView = React.memo<CodexDiffViewProps>(
  ({ tool, metadata }) => {
    const { input } = tool;
    const patch =
      typeof input?.unified_diff === "string" ? input.unified_diff : undefined;
    const fileName = patch ? parseUnifiedDiff(patch).fileName : undefined;
    const stats = React.useMemo(
      () => (patch ? getPatchDiffStats(patch) : null),
      [patch],
    );
...
const DiffStats = React.memo<{ additions: number; deletions: number }>(
  ({ additions, deletions }) => (
