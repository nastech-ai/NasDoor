...
export const ChatList = React.memo((props: { session: Session }) => {
  const { messages, hasMoreOlder, isLoadingOlder } = useSessionMessages(
    props.session.id,
  );
  return (
    <ChatListInternal
...
const ChatListInternal = React.memo(
  (props: {
    ...
    const [toggledGroups, setToggledGroups] = React.useState<Set<string>>(
      new Set(),
    );
    ...
    const handleForkFromMessage = useCallback(
      (_messageId: string, claudeUuid: string) => {
        Modal.show({
    ...
    const renderItem = useCallback(
      ({ item }: { item: DisplayItem }) => {
        if (item.type === "tool-group") {
          ...
          const expanded = toggledGroups.has(item.id)
            ? !defaultExpanded
            : defaultExpanded;
          return (
            <ToolGroupView
    ...
    return (
      <View style={{ flex: 1 }}>
        <FlatList
          ...
          ListFooterComponent={
            <ListHeader isLoadingOlder={props.isLoadingOlder} />
          }
