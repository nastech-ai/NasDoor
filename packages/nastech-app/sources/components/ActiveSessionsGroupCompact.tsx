...
const STATUS_CONFIG: Record<
  SessionState,
  { color: string; dotColor: string; isPulsing: boolean; isConnected: boolean }
> = {
  disconnected: {
    color: "#999",
    dotColor: "#999",
...
function useSectionGitInfo(sessionId: string) {
  ...
  return React.useMemo(() => {
    ...
    return {
      ...
      hasChanges:
        gitStatus.unstagedLinesAdded > 0 || gitStatus.unstagedLinesRemoved > 0,
    };
  }, [gitStatus]);
}
...
const SectionHeader = React.memo(
  ({
    ...
    const handleAdd = React.useCallback(() => {
      ...
      const pathToSet = formatPathRelativeToHome(
        repoPath,
        session.homeDir ?? undefined,
      );
...
const MachineSeparator = React.memo(
  ({ machineName, machineId }: { machineName: string; machineId: string }) => {
    const styles = stylesheet;
    const { theme } = useUnistyles();
    const router = useRouter();
  ...
  ({
    ...
    const status = session.hasUnread
      ? {
          ...
          dotColor: "#007AFF",
          isPulsing: false,
          isConnected: baseStatus.isConnected,
        }
      : baseStatus;
    ...
    const [actionsAnchor, setActionsAnchor] =
      React.useState<SessionActionsAnchor | null>(null);
    ...
    const renderLeadingIndicator = () => {
      ...
      return <View style={styles.leadingIndicatorSlot}>{indicator}</View>;
    };
