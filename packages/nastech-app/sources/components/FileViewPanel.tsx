...
export const FileViewPanel = React.memo(function FileViewPanel({
  ...
  const [displayMode, setDisplayMode] = React.useState<"edit" | "preview">(
    "edit",
  );

  // External change detection
  const [externalChange, setExternalChange] = React.useState<string | null>(
    null,
  ); // new content from device
  ...
  const hasChanges =
    fileState.kind === "loaded" && editContent !== fileState.content;
    ...
    (async () => {
      try {
        ...
        if (!fileResponse.success || !fileResponse.content) {
          setFileState({
            kind: "error",
            message: fileResponse.error || t("files.failedToRead"),
          });
          return;
        }
  ...
  const handleSave = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.success) {
        if (
          response.error?.includes("hash") ||
          response.error?.includes("mismatch")
        ) {
          ...
          Modal.alert(
            t("common.error"),
            response.error || t("files.failedToSave"),
          );
        }
        return;
      }
  ...
  const handleForceSave = React.useCallback(async () => {
    ...
    try {
      ...
      const serverContent = await readFileContent(sessionId, filePath);
      const currentHash = serverContent
        ? await computeSHA256(serverContent)
        : undefined;
      ...
      {showConflictDiff && externalChange && fileState.kind === "loaded" ? (
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            <Pressable
              onPress={() => setShowConflictDiff(false)}
              hitSlop={8}
              style={{ padding: 4 }}
            >
              <Ionicons
          ...
          >
            <PierreDiffView
              oldFile={{
                name: fileName + " (your changes)",
                contents: editContent,
              }}
              newFile={{
                name: fileName + " (on device)",
                contents: externalChange,
              }}
              diffStyle="unified"
        ...
        >
          ...
          <View
            {...(Platform.OS === "web"
              ? ({ className: "editor-preview-wrap" } as any)
              : {})}
          >
