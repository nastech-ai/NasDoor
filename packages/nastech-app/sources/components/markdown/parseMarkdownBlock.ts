...
function parseTable(
  ...
  for (let i = 2; i < tableLines.length; i++) {
    const rowCells = splitTableRow(tableLines[i]).map((cell) =>
      parseMarkdownSpans(cell, false),
    );
    ...
    if (numberedListMatch) {
      ...
      while (index < lines.length) {
        ...
        allLines.push({
          ...
          indent: nextIndent,
          content: nextTrimmed.slice(nextMatch[0].length),
        });
        index++;
      }
      ...
      blocks.push({
        ...
        items: allLines.map((l) => ({
          number: l.number,
          depth: Math.floor((l.indent - baseIndent) / 2),
