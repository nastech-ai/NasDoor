...
import {
  parseLocalCommandMessage,
  isUserSlashCommandEcho,
} from "./parseLocalCommandMessage";

describe("parseLocalCommandMessage", () => {
  it("hides the local-command caveat wrapper", () => {
    const text =
      "<local-command-caveat>The user typed a slash command.</local-command-caveat>";
...
describe("isUserSlashCommandEcho", () => {
  ...
  it("detects a command-with-args echo with a localId", () => {
    expect(
      isUserSlashCommandEcho("/superpowers:brainstorming make me rich", true),
    ).toBe(true);
  });
  ...
  it("does not match a lone slash or ordinary text", () => {
    ...
    expect(isUserSlashCommandEcho("please run /compact later", true)).toBe(
      false,
    );
  });
