...
function buildTree<T extends { fullPath: string }>(
  files: T[],
): AnyTreeNode<T>[] {
...
function compressTree<T>(node: DirNode<T>) {
  ...
  while (
    ...
    node.name = node.name
      ? `${node.name}${PATH_SEPARATOR}${only.name}`
      : only.name;
    node.path = only.path;
          ...
          (gitStatus.linesAdded > 0 || gitStatus.linesRemoved > 0) ? (
            <View style={styles.headerLineChanges}>
              ...
              {gitStatus.linesRemoved > 0 && (
                <Text style={styles.headerRemoved}>
            ...
            {!hasFiles ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyTitle}>
                  {t("files.noChangesTitle")}
                </Text>
                <Text style={styles.emptySubtitle}>
                  {t("files.noChangesSubtitle")}
                </Text>
              </View>
            ) : (
