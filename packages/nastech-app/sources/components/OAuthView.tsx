  ...
  (props: {
    ...
    React.useEffect(() => {
      ...
      (async () => {
        ...
        const redirectUri =
          props.config.redirectUri || "http://localhost:54545/callback";
      ...
      async (navState: any) => {
        ...
        if (callbackData.code && callbackData.state) {
          ...
          try {
            ...
            tokenExchangeOpacity.value = withTiming(
              0,
              { duration: 300 },
              () => {
    ...
    if (error) {
      return (
        <View style={[styles.errorContainer]}>
          <Text style={[styles.errorText, { color: props.foregroundColor }]}>
            {error}
          </Text>
          <TouchableOpacity style={styles.retryButton} onPress={props.onRetry}>
            <Text
              style={[styles.retryButtonText, { color: props.foregroundColor }]}
            >
              {t("common.retry")}
            </Text>
          </TouchableOpacity>
      ...
      <>
        ...
        {webViewLoading && (
          ...
          >
            <ActivityIndicator
              size="large"
              color={props.foregroundColor || theme.colors.text}
            />
            <Text
              style={[styles.loadingText, { color: props.foregroundColor }]}
            >
              {t("common.loading")}
            </Text>
          </Animated.View>
...
export const OAuthViewUnsupported = React.memo(
  (props: { name: string; command?: string }) => {
    const command =
      props.command || `nastech connect ${props.name.toLowerCase()}`;
