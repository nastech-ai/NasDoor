...
class RealtimeVoiceSessionImpl implements VoiceSession {
  async startSession(config: VoiceSessionConfig): Promise<string | null> {
    console.log(
      "[RealtimeVoiceSessionImpl] conversationInstance:",
      conversationInstance,
    );
    if (!conversationInstance) {
      console.warn(
        "Realtime voice session not initialized - conversationInstance is null",
      );
    ...
    try {
      ...
      const userLanguagePreference =
        storage.getState().settings.voiceAssistantLanguage;
      const elevenLabsLanguage = getElevenLabsCodeFromPreference(
...
export const RealtimeVoiceSession: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log(
      "[RealtimeVoiceSession] Setting conversationInstance:",
      conversation,
    );
    ...
    if (!hasRegistered.current) {
      try {
        ...
        console.log(
          "[RealtimeVoiceSession] Voice session registered successfully",
        );
      } catch (error) {
