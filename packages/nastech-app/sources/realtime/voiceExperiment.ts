...
export type VoiceGatingMode = "direct-byo-agent" | "nastech-server";
...
function isVoiceUpsellVariant(
  value: unknown,
): value is Exclude<VoiceUpsellVariant, "control"> {
  return (
    value === "show-paywall-before-first-voice-chat" ||
    value === "voice-onboarding-and-upsell"
  );
}

export function getVoiceUpsellVariantLabel(
  variant: VoiceUpsellVariant,
): string {
  switch (variant) {
    case "control":
      return "Control";
...
export function getVoiceUpsellVariant(options?: {
  ...
  const rawVariant =
    options?.rawVariant ?? tracking?.getFeatureFlag(VOICE_UPSELL_FLAG_KEY);
...
export function getVoiceExperimentStatus(options: {
  ...
  const gatingMode: VoiceGatingMode =
    options.voiceBypassToken && !!options.voiceCustomAgentId
      ? "direct-byo-agent"
      : "nastech-server";
  const hasOverride =
    !!options.voiceUpsellOverrideEnabled && !!options.voiceUpsellOverride;
  ...
  return {
    ...
    upsellVariantSource: hasOverride
      ? "override"
      : isVoiceUpsellVariant(rawVariant)
        ? "posthog"
        : "default",
    gatingMode,
  };
