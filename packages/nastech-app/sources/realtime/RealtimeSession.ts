...
export async function startRealtimeSession(
  ...
  try {
    ...
    const { voiceBypassToken, voiceCustomAgentId } =
      storage.getState().settings;
    if (voiceBypassToken && voiceCustomAgentId) {
      console.log(
        "[Voice] Bypassing token, custom agent ID:",
        voiceCustomAgentId,
      );
    ...
    if (!response.allowed) {
      ...
      console.log(
        "[Voice] Not allowed (reason: %s), presenting must-pay paywall...",
        response.reason,
      );
    ...
    const { voiceUpsellOverride, devModeEnabled } =
    ...
    if (
      ...
      console.log(
        "[Voice] First voice attempt on free tier, showing soft paywall...",
      );
      incrementVoiceSoftPaywallShown();
    ...
    const systemPrompt = buildVoiceSystemPrompt({
      ...
      voiceMessageCount,
      includePaidVoiceOnboarding:
        !hasPro && voiceUpsellVariant === "voice-onboarding-and-upsell",
    });
    const firstMessage = buildVoiceFirstMessage({
      hasPro,
      onboardingPromptLoadCount,
      includePaidVoiceOnboarding:
        voiceUpsellVariant === "voice-onboarding-and-upsell",
    });
    ...
    currentVoiceConversationId =
      response.conversationId ?? startedConversationId;
    currentVoiceSessionStartedAt = Date.now();
    voiceSessionStarted = true;
...
export function getCurrentVoiceSessionDurationSeconds(): number | undefined {
  ...
  return Math.max(
    0,
    Math.round((Date.now() - currentVoiceSessionStartedAt) / 1000),
  );
}
