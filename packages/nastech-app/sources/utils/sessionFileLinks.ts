...
function parseLineAndColumn(value: string): {
  path: string;
  line: number | null;
  column: number | null;
} {
...
function inferHomeDirectory(
  ...
  const match = normalizedRoot.match(
...
function normalizePath(value: string): string {
  ...
  const prefix = isWindowsAbsolute
    ...
    : isPosixAbsolute
      ? "/"
      : "";
  const rawRemainder = isWindowsAbsolute
    ? withForwardSlashes.slice(3)
    : isPosixAbsolute
...
function resolvePath(
  ...
  if (
    WINDOWS_ABSOLUTE_PATH.test(expandedPath) ||
    POSIX_ABSOLUTE_PATH.test(expandedPath)
  ) {
...
export function parseSessionFileLink(
  ...
  if (
    !looksLikePath(parsedUrl.path) &&
    !looksLikePath(parsedLabel?.path ?? "")
  ) {
    return null;
  }
...
export function splitSessionFileText(
  text: string,
  sessionRoot?: string | null,
): SessionFileTextSegment[] {
