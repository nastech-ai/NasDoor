...
describe("toSnakeCase", () => {
  describe("basic conversions", () => {
    ...
    it("should handle sentences with punctuation", () => {
      expect(toSnakeCase("Hello World!")).toBe("hello_world");
      expect(toSnakeCase("This is a test sentence.")).toBe(
        "this_is_a_test_sentence",
      );
      expect(toSnakeCase("What? Really? Yes!")).toBe("what_really_yes");
      expect(toSnakeCase("Hello, world; how are you?")).toBe(
  ...
  describe("special characters handling", () => {
    ...
    it("should handle path-unsafe characters", () => {
      expect(toSnakeCase("folder/file:name*test")).toBe(
        "folder_file_name_test",
      );
  ...
  describe("multiple spaces and underscores", () => {
    it("should collapse multiple spaces", () => {
      ...
      expect(toSnakeCase("Space    between    words")).toBe(
        "space_between_words",
      );
    });
    ...
    it("should handle existing underscores", () => {
      ...
      expect(toSnakeCase("multiple___underscores")).toBe(
  ...
  describe("real-world examples", () => {
    it("should handle typical file names", () => {
      expect(toSnakeCase("My Document (Final).docx")).toBe(
        "my_document_final_docx",
      );
      expect(toSnakeCase("Screenshot 2024-01-15 at 10.30.45 AM")).toBe(
        "screenshot_2024_01_15_at_10_30_45_am",
      );
      expect(toSnakeCase("Copy of Budget Q4 2023 (v2)")).toBe(
        "copy_of_budget_q4_2023_v2",
      );
    });

    it("should handle typical user input", () => {
      expect(toSnakeCase("John's Profile Picture")).toBe(
        "john_s_profile_picture",
      );
      expect(toSnakeCase("Meeting Notes - January 15th")).toBe(
        "meeting_notes_january_15th",
      );
      expect(toSnakeCase("TODO: Fix this bug ASAP!")).toBe(
        "todo_fix_this_bug_asap",
      );
    });

    it("should handle programming-related text", () => {
      expect(toSnakeCase("getUserById() function")).toBe(
        "getuserbyid_function",
      );
      expect(toSnakeCase("API_KEY environment variable")).toBe(
