...
export async function copySessionMetadataToClipboard(
  session: Session,
): Promise<boolean> {
  if (!session.metadata) {
...
export async function copySessionMetadataAndLogsToClipboard(
  session: Session,
): Promise<boolean> {
  if (!session.metadata) {
    Modal.alert(t("common.error"), t("sessionInfo.failedToCopyMetadata"));
  ...
  try {
    ...
    const sections = ["=== Session Metadata ===", metadata];
