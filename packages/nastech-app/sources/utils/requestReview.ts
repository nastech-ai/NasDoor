  ...
  lock.inLock(async () => {
    try {
      ...
      if (settings.reviewPromptAnswered && settings.reviewPromptLikedApp) {
        ...
        const lastShownStr = localStorage.getString(
          LOCAL_KEYS.STORE_REVIEW_LAST_SHOWN,
        );
        if (lastShownStr) {
          const lastShown = new Date(lastShownStr);
          const now = new Date();
          const daysSinceShown =
            (now.getTime() - lastShown.getTime()) / DAY_IN_MS;
      ...
      if (declinedAtStr) {
        ...
        const daysSinceDeclined =
          (now.getTime() - declinedAt.getTime()) / DAY_IN_MS;
