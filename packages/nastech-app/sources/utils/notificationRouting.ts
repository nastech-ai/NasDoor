...
export function getSessionRouteFromNotificationData(
  data: unknown,
): `/session/${string}` | null {
...
export function getSessionRouteFromNotificationResponse(
  response: unknown,
): `/session/${string}` | null {
  const contentData = getObjectValue(
    getObjectValue(getObjectValue(response, "notification"), "request"),
    "content",
  );
  return getSessionRouteFromNotificationData(
    getObjectValue(contentData, "data"),
  );
}
