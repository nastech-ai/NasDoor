...
export function trackSessionSwitched(
  session: Pick<Session, "id" | "createdAt" | "activeAt" | "updatedAt">,
) {
...
export function trackMessageSent(
  ...
  tracking?.capture("message_sent", {
    source,
    session_agent:
      metadata?.flavor === "gpt" || metadata?.flavor === "openai"
        ? "codex"
        : (metadata?.flavor ?? null),
    session_started_source:
      metadata?.startedBy === "daemon" || metadata?.startedFromDaemon === true
        ? "daemon"
        : metadata?.startedBy === "terminal" ||
            metadata?.startedFromDaemon === false
          ? "cli"
          : null,
