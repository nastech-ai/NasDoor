...
export const zhHant: TranslationStructure = {
  ...
  settings: {
    ...
    about: "關於",
    aboutFooter:
      "NasTech 是一個 Codex 和 Claude Code 行動用戶端。它採用端對端加密，您的帳戶僅儲存在本機裝置上。與 Anthropic 無關聯。",
    whatsNew: "更新日誌",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "以單欄（unified）或並排（split）顯示差異。split 檢視僅在 Web 上可用。",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
  ...
  settingsFeatures: {
    ...
    privacy: "隱私",
    privacyDescription:
      "完全停用所有分析和遙測。不會向 PostHog 或任何其他追蹤服務傳送資料。",
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `您本月已使用超過 ${hours} 小時的語音。這是允許的最大用量。您可以在語音設定中配置自己的 ElevenLabs 代理，以使用您自己的配額。`,
    voiceConversationLimitReached:
      "您本月已達到語音對話的最大次數。我們未來可能會新增按需語音使用功能——如果您遇到此限制，請在 github.com/nastech-ai/nastech/issues 提交 issue。",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "新會話將保留所選輪次完整內容（你的訊息與智能體的回覆），並丟棄其後的所有訊息。",
  ...
  sessionInfo: {
    ...
    worktreeCleanupMessage:
      "Worktree 沒有未提交的變更。是否要刪除 Worktree 檔案？",
  ...
  settingsVoice: {
    ...
    byoTitle: "使用自己的代理",
    byoDescription:
      "使用您自己的 ElevenLabs 代理取代 NasTech 預設代理。無需訂閱 — 直接使用您自己的 ElevenLabs 帳戶連線。您的代理必須定義兩個用戶端工具：messageClaudeCode（向編碼代理傳送文字）和 processPermissionRequest（允許或拒絕工具使用）。透過 {{initialConversationContext}} 動態變數接收工作階段上下文。",
    customAgentId: "ElevenLabs Agent ID",
    customAgentIdNotSet: "未設定",
    customAgentIdDescription:
      "輸入您的 ElevenLabs Agent ID。留空則使用 NasTech 預設代理。",
    ...
    usageFooter:
      "過去 30 天使用的語音時間。免費方案: 20 分鐘。訂閱用戶: 5 小時。每月最多 100 次對話。",
    usageLabel: "語音時間",
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "有終端正在請求連線到您的 NasTech 帳戶。這將允許終端安全地傳送和接收訊息。",
  ...
  machine: {
    ...
    delete: "刪除裝置",
    deleteFooter:
      "從您的帳戶中移除此裝置。工作階段歷史將保留,但您將無法在此裝置上啟動新的工作階段。",
    deleteConfirmTitle: "刪除此裝置?",
    deleteConfirmMessage:
      "裝置將從您的帳戶中移除。工作階段歷史將保留,但在您重新連接守護程序之前,您將無法啟動新的工作階段。",
    deleteFailed: "刪除裝置失敗。",
  },
