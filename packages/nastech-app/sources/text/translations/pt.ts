...
export const pt: TranslationStructure = {
  ...
  settings: {
    ...
    showOfflineMachines: ({ count }: { count: number }) =>
      count === 1
        ? "Mostrar 1 máquina offline"
        : `Mostrar ${count} máquinas offline`,
    ...
    about: "Sobre",
    aboutFooter:
      "NasTech é um cliente móvel para Codex e Claude Code. É totalmente criptografado ponta a ponta e sua conta é armazenada apenas no seu dispositivo. Não é afiliado à Anthropic.",
    whatsNew: "Novidades",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "Mostrar diffs em uma única coluna (unified) ou lado a lado (split). A visualização split funciona apenas na web.",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
    ...
    zenMode: "Modo Zen",
    zenModeDescription:
      "Ocultar elementos da interface para uma experiência sem distrações",
  ...
  settingsFeatures: {
    ...
    groupToolCallsSubtitle:
      "Recolher chamadas consecutivas de ferramentas em um único contêiner",
    privacy: "Privacidade",
    privacyDescription:
      "Desativa completamente toda a análise e telemetria. Nenhum dado será enviado ao PostHog ou qualquer outro serviço de rastreamento.",
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `Você usou ${hours}+ horas de voz este mês. Este é o máximo permitido. Você pode configurar seu próprio agente ElevenLabs nas configurações de voz para usar sua própria cota.`,
    voiceConversationLimitReached:
      "Você atingiu o número máximo de conversas de voz este mês. Podemos adicionar uso de voz sob demanda no futuro — por favor, abra um issue em github.com/nastech-ai/nastech/issues se você atingir este limite.",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "A nova sessão manterá o turno escolhido completo (sua mensagem e a resposta do agente) e descartará as mensagens seguintes.",
    duplicateSheetConfirm: "Duplicar",
    duplicateSheetEmpty:
      "Ainda não há mensagens elegíveis para retrocesso nesta sessão.",
    duplicateRowDisabled:
      "Esta mensagem não pode ser usada como ponto de retrocesso.",
    forkedFromLabel: "Bifurcado de",
    forkedFromSubtitle: "Abrir a sessão da qual foi bifurcada",
    forkErrorOffline:
      "Esta máquina está offline. A bifurcação só está disponível enquanto a máquina da sessão estiver online.",
    forkErrorMissingUuid:
      "O ponto de retrocesso escolhido não existe mais na sessão de origem — tente bifurcar sem truncar.",
    forkErrorMissingMetadata:
      "Faltam metadados da sessão necessários para bifurcar.",
    forkErrorGeneric: "Não foi possível bifurcar a sessão.",
    forkClaudeOnly:
      "A bifurcação atualmente só é suportada para sessões Claude.",
  },
  ...
  sessionInfo: {
    ...
    nastechSessionIdCopied:
      "ID da sessão NasTech copiado para a área de transferência",
    ...
    codexThreadIdCopied:
      "ID da thread do Codex copiado para a área de transferência",
    ...
    worktreeCleanupMessage:
      "O Worktree não tem alterações não confirmadas. Deseja excluir os arquivos do Worktree?",
  ...
  toolGroup: {
    editedFiles: ({ count }: { count: number }) =>
      count === 1 ? "Editou 1 arquivo" : `Editou ${count} arquivos`,
    readFiles: ({ count }: { count: number }) =>
      count === 1 ? "Leu 1 arquivo" : `Leu ${count} arquivos`,
    ranCommands: ({ count }: { count: number }) =>
      count === 1 ? "Executou 1 comando" : `Executou ${count} comandos`,
    searched: ({ count }: { count: number }) =>
      count === 1 ? "Pesquisou 1 vez" : `Pesquisou ${count} vezes`,
    fetchedUrls: ({ count }: { count: number }) =>
      count === 1 ? "Obteve 1 URL" : `Obteve ${count} URLs`,
    ranTasks: ({ count }: { count: number }) =>
      count === 1 ? "Executou 1 tarefa" : `Executou ${count} tarefas`,
    usedTools: ({ count }: { count: number }) =>
      count === 1 ? "Usou 1 ferramenta" : `Usou ${count} ferramentas`,
  },
  ...
  files: {
    ...
    fileConflictDescription:
      "Este arquivo foi modificado no dispositivo enquanto você o editava. Recarregue para ver a versão mais recente.",
    reload: "Recarregar",
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "Um terminal está solicitando conexão à sua conta NasTech. Isso permitirá que o terminal envie e receba mensagens com segurança.",
  ...
  machine: {
    ...
    deleteFooter:
      "Remove esta máquina da sua conta. O histórico de sessões será preservado, mas você não poderá iniciar novas sessões nesta máquina.",
    deleteConfirmTitle: "Excluir esta máquina?",
    deleteConfirmMessage:
      "A máquina será removida da sua conta. O histórico de sessões será preservado, mas você não poderá iniciar novas sessões até reconectar o daemon.",
  ...
  imageUpload: {
    ...
    permissionMessage:
      "Permita o acesso à sua biblioteca de fotos para anexar imagens às mensagens.",
    limitTitle: "Limite de imagens atingido",
    limitMessage: ({ max }: { max: number }) =>
      `Você pode anexar até ${max} imagens por mensagem.`,
    fileTooLargeTitle: "Arquivo muito grande",
    fileTooLargeMessage: ({ name, maxMb }: { name: string; maxMb: number }) =>
      `"${name}" excede o limite de ${maxMb}MB e não foi adicionado.`,
    ...
    notSupportedMessage:
      "Este agente não suporta anexos de imagem. Apenas o texto foi enviado.",
  },
