...
export const ca: TranslationStructure = {
  ...
  settings: {
    ...
    showOfflineMachines: ({ count }: { count: number }) =>
      count === 1
        ? "Mostra 1 màquina fora de línia"
        : `Mostra ${count} màquines fora de línia`,
    ...
    about: "Quant a",
    aboutFooter:
      "NasTech és un client mòbil de Codex i Claude Code. Tot està xifrat punt a punt i el teu compte es guarda només al teu dispositiu. No està afiliat amb Anthropic.",
    whatsNew: "Novetats",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "Mostra les diferències en una sola columna (unified) o una al costat de l'altra (split). La vista split només funciona al web.",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
    ...
    zenMode: "Mode Zen",
    zenModeDescription:
      "Amaga elements de la interfície per a una experiència sense distraccions",
    zenModeDetail: "Configura quins elements amagar",
  },
  ...
  settingsFeatures: {
    ...
    groupToolCallsSubtitle:
      "Replega les crides consecutives a eines en un sol contenidor",
    privacy: "Privadesa",
    privacyDescription:
      "Desactiva completament tota l'analítica i telemetria. No s'enviaran dades a PostHog ni a cap altre servei de seguiment.",
    ...
    imageUploadSubtitle:
      "Adjunta imatges als missatges perquè Claude les analitzi",
  },
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `Has utilitzat ${hours}+ hores de veu aquest mes. Aquest és el màxim permès. Pots configurar el teu propi agent ElevenLabs a la configuració de veu per utilitzar la teva pròpia quota.`,
    voiceConversationLimitReached:
      "Has assolit el nombre màxim de converses de veu aquest mes. És possible que en el futur afegim ús de veu sota demanda — si us plau, crea un issue a github.com/nastech-ai/nastech/issues si arribes a aquest límit.",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "La nova sessió conservarà el torn escollit complet (el teu missatge i la resposta de l'agent) i descartarà els missatges següents.",
    duplicateSheetConfirm: "Duplica",
    duplicateSheetEmpty:
      "Encara no hi ha missatges per retrocedir en aquesta sessió.",
    duplicateRowDisabled:
      "Aquest missatge no es pot usar com a punt de retrocés.",
    forkedFromLabel: "Bifurcat de",
    forkedFromSubtitle: "Obre la sessió de la qual prové la bifurcació",
    forkErrorOffline:
      "La màquina està fora de línia. La bifurcació només està disponible mentre la màquina de la sessió estigui en línia.",
    forkErrorMissingUuid:
      "El punt de retrocés escollit ja no existeix a la sessió origen — prova a bifurcar sense truncar.",
    forkErrorMissingMetadata:
      "Falten metadades de la sessió necessàries per bifurcar.",
    forkErrorGeneric: "No s'ha pogut bifurcar la sessió.",
    forkClaudeOnly:
      "La bifurcació només està disponible per a sessions de Claude.",
  },
  ...
  sessionInfo: {
    ...
    nastechSessionIdCopied:
      "ID de la sessió de NasTech copiat al porta-retalls",
    ...
    worktreeCleanupMessage:
      "El Worktree no té canvis sense confirmar. Vols eliminar els fitxers del Worktree?",
  ...
  toolGroup: {
    editedFiles: ({ count }: { count: number }) =>
      count === 1 ? "S'ha editat 1 fitxer" : `S'han editat ${count} fitxers`,
    readFiles: ({ count }: { count: number }) =>
      count === 1 ? "S'ha llegit 1 fitxer" : `S'han llegit ${count} fitxers`,
    ranCommands: ({ count }: { count: number }) =>
      count === 1
        ? "S'ha executat 1 comanda"
        : `S'han executat ${count} comandes`,
    searched: ({ count }: { count: number }) =>
      count === 1 ? "S'ha cercat 1 vegada" : `S'ha cercat ${count} vegades`,
    fetchedUrls: ({ count }: { count: number }) =>
      count === 1 ? "S'ha obtingut 1 URL" : `S'han obtingut ${count} URLs`,
    ranTasks: ({ count }: { count: number }) =>
      count === 1 ? "S'ha executat 1 tasca" : `S'han executat ${count} tasques`,
    usedTools: ({ count }: { count: number }) =>
      count === 1 ? "S'ha usat 1 eina" : `S'han usat ${count} eines`,
  },
  ...
  files: {
    ...
    fileConflictDescription:
      "Aquest fitxer s'ha modificat al dispositiu mentre l'editaves. Recarrega per veure la darrera versió.",
    reload: "Recarregar",
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "Un terminal està sol·licitant connectar-se al teu compte de NasTech. Això permetrà al terminal enviar i rebre missatges de forma segura.",
  ...
  machine: {
    ...
    deleteFooter:
      "Elimina aquesta màquina del teu compte. L'historial de sessions es conservarà, però no podràs iniciar noves sessions en aquesta màquina.",
    deleteConfirmTitle: "Eliminar aquesta màquina?",
    deleteConfirmMessage:
      "La màquina s'eliminarà del teu compte. L'historial de sessions es conservarà, però no podràs iniciar noves sessions fins que tornis a connectar el dimoni.",
  ...
  imageUpload: {
    ...
    permissionMessage:
      "Permet l'accés a la teva biblioteca de fotos per adjuntar imatges als missatges.",
    limitTitle: "Límit d'imatges assolit",
    limitMessage: ({ max }: { max: number }) =>
      `Pots adjuntar fins a ${max} imatges per missatge.`,
    fileTooLargeTitle: "Fitxer massa gran",
    fileTooLargeMessage: ({ name, maxMb }: { name: string; maxMb: number }) =>
      `"${name}" supera el límit de ${maxMb}MB i no s'ha afegit.`,
    ...
    notSupportedMessage:
      "Aquest agent no admet imatges adjuntes. Només s'ha enviat el text.",
  },
