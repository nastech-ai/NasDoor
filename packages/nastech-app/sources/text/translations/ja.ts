...
export const ja: TranslationStructure = {
  ...
  settings: {
    ...
    about: "このアプリについて",
    aboutFooter:
      "NasTechはCodexとClaude Codeのモバイルクライアントです。完全なエンドツーエンド暗号化を採用し、アカウントはデバイスにのみ保存されます。Anthropicとは提携していません。",
    whatsNew: "新機能",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "差分を1列（unified）または横並び（split）で表示します。split 表示は Web 専用です。",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
  ...
  settingsFeatures: {
    ...
    privacy: "プライバシー",
    privacyDescription:
      "すべての分析とテレメトリを完全に無効にします。PostHogやその他のトラッキングサービスにデータは送信されません。",
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `今月${hours}時間以上の音声を使用しました。これは許可される最大量です。音声設定で独自の ElevenLabs エージェントを設定して、自分のクォータを使用できます。`,
    voiceConversationLimitReached:
      "今月の音声会話の最大数に達しました。将来的にオンデマンドの音声利用を追加する可能性があります。この制限に達した場合は、github.com/nastech-ai/nastech/issues で issue を作成してください。",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "新しいセッションは選んだターン全体（あなたのメッセージとエージェントの応答）を保持し、それ以降のメッセージは破棄します。",
    duplicateSheetConfirm: "複製",
    duplicateSheetEmpty:
      "このセッションには巻き戻し可能なメッセージがまだありません。",
    duplicateRowDisabled: "このメッセージは巻き戻しポイントに使えません。",
    forkedFromLabel: "フォーク元",
    forkedFromSubtitle: "フォーク元のセッションを開く",
    forkErrorOffline:
      "マシンがオフラインです。セッションのマシンがオンラインの間のみフォークできます。",
    forkErrorMissingUuid:
      "選んだ巻き戻しポイントがソースセッションに存在しません — 切り詰めなしのフォークをお試しください。",
    forkErrorMissingMetadata:
      "フォークに必要なセッションのメタデータがありません。",
  ...
  sessionInfo: {
    ...
    nastechSessionIdCopied:
      "NasTech Session IDがクリップボードにコピーされました",
    ...
    worktreeCleanupMessage:
      "Worktreeにコミットされていない変更はありません。Worktreeのファイルを削除しますか？",
  ...
  files: {
    ...
    fileConflictDescription:
      "編集中にデバイス上でファイルが変更されました。最新版を表示するには再読み込みしてください。",
    reload: "再読み込み",
    overwrite: "上書き",
  },
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "ターミナルがNasTechアカウントへの接続を要求しています。これにより、ターミナルは安全にメッセージを送受信できるようになります。",
  ...
  machine: {
    ...
    delete: "マシンを削除",
    deleteFooter:
      "このマシンをアカウントから削除します。セッション履歴は保持されますが、このマシンで新しいセッションを起動できなくなります。",
    deleteConfirmTitle: "このマシンを削除しますか？",
    deleteConfirmMessage:
      "マシンがアカウントから削除されます。セッション履歴は保持されますが、デーモンを再接続するまで新しいセッションを起動できません。",
  ...
  imageUpload: {
    ...
    notSupportedMessage:
      "このエージェントは画像の添付に対応していません。テキストのみが送信されました。",
  },
