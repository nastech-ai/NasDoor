...
export const zhHans: TranslationStructure = {
  ...
  settings: {
    ...
    about: "关于",
    aboutFooter:
      "NasTech 是一个 Codex 和 Claude Code 移动客户端。它采用端到端加密，您的账户仅存储在本地设备上。与 Anthropic 无关联。",
    whatsNew: "更新日志",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "以单列（unified）或并排（split）显示差异。split 视图仅在 Web 上可用。",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
  ...
  settingsFeatures: {
    ...
    privacy: "隐私",
    privacyDescription:
      "完全禁用所有分析和遥测。不会向 PostHog 或任何其他跟踪服务发送数据。",
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `您本月已使用超过 ${hours} 小时的语音。这是允许的最大用量。您可以在语音设置中配置自己的 ElevenLabs 代理，以使用您自己的配额。`,
    voiceConversationLimitReached:
      "您本月已达到语音对话的最大次数。我们未来可能会添加按需语音使用功能——如果您遇到此限制，请在 github.com/nastech-ai/nastech/issues 提交 issue。",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "新会话将保留所选轮次完整内容（你的消息与智能体的回复），并丢弃其后的所有消息。",
  ...
  sessionInfo: {
    ...
    worktreeCleanupMessage:
      "Worktree 没有未提交的更改。是否要删除 Worktree 文件？",
  ...
  settingsVoice: {
    ...
    byoTitle: "使用自己的代理",
    byoDescription:
      "使用您自己的 ElevenLabs 代理代替 NasTech 默认代理。无需订阅 — 直接使用您自己的 ElevenLabs 账户连接。您的代理必须定义两个客户端工具：messageClaudeCode（向编码代理发送文本）和 processPermissionRequest（允许或拒绝工具使用）。通过 {{initialConversationContext}} 动态变量接收会话上下文。",
    customAgentId: "ElevenLabs Agent ID",
    customAgentIdNotSet: "未配置",
    customAgentIdDescription:
      "输入您的 ElevenLabs Agent ID。留空则使用 NasTech 默认代理。",
    ...
    usageFooter:
      "过去 30 天使用的语音时间。免费方案: 20 分钟。订阅用户: 5 小时。每月最多 100 次对话。",
    usageLabel: "语音时间",
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "有终端正在请求连接到您的 NasTech 账户。这将允许终端安全地发送和接收消息。",
  ...
  machine: {
    ...
    delete: "删除设备",
    deleteFooter:
      "从您的账户中移除此设备。会话历史将保留，但您无法再在此设备上启动新会话。",
    deleteConfirmTitle: "删除此设备？",
    deleteConfirmMessage:
      "设备将从您的账户中移除。会话历史将保留，但在您重新连接守护进程之前，您将无法启动新会话。",
    deleteFailed: "删除设备失败。",
  },
