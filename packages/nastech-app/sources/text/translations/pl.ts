...
export const pl: TranslationStructure = {
  ...
  settings: {
    ...
    showOfflineMachines: ({ count }: { count: number }) => {
      ...
      if (mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14))
        return `Pokaż ${count} maszyny offline`;
    ...
    about: "O aplikacji",
    aboutFooter:
      "NasTech to mobilny klient Codex i Claude Code. Jest w pełni szyfrowany end-to-end, a Twoje konto jest przechowywane tylko na Twoim urządzeniu. Nie jest powiązany z Anthropic.",
    whatsNew: "Co nowego",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "Pokazuj różnice w jednej kolumnie (unified) lub obok siebie (split). Widok split działa tylko w przeglądarce.",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
    ...
    zenMode: "Tryb Zen",
    zenModeDescription:
      "Ukryj elementy interfejsu dla skupionego doświadczenia",
  ...
  settingsFeatures: {
    ...
    groupToolCallsSubtitle:
      "Zwijaj kolejne wywołania narzędzi w jeden kontener",
    privacy: "Prywatność",
    privacyDescription:
      "Całkowicie wyłącza wszystkie analizy i telemetrię. Żadne dane nie będą wysyłane do PostHog ani żadnego innego serwisu śledzącego.",
    ...
    imageUploadSubtitle:
      "Dołącz obrazy do wiadomości, aby Claude mógł je przeanalizować",
  },
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `Wykorzystałeś ${hours}+ godzin głosu w tym miesiącu. To jest maksymalny dozwolony limit. Możesz skonfigurować własnego agenta ElevenLabs w ustawieniach głosu, aby korzystać z własnego limitu.`,
    voiceConversationLimitReached:
      "Osiągnąłeś maksymalną liczbę rozmów głosowych w tym miesiącu. Możliwe, że w przyszłości dodamy głosowe korzystanie na żądanie — prosimy o zgłoszenie na github.com/nastech-ai/nastech/issues, jeśli napotkasz ten limit.",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "Nowa sesja zachowa wybraną turę w całości (twoja wiadomość i odpowiedź agenta) i odrzuci wszystkie kolejne wiadomości.",
    duplicateSheetConfirm: "Duplikuj",
    duplicateSheetEmpty:
      "W tej sesji nie ma jeszcze wiadomości, do których można się cofnąć.",
    duplicateRowDisabled:
      "Tej wiadomości nie można użyć jako punktu cofnięcia.",
    forkedFromLabel: "Rozwidlone z",
    forkedFromSubtitle: "Otwórz sesję, z której powstało rozwidlenie",
    forkErrorOffline:
      "Maszyna jest offline. Rozwidlenie jest dostępne tylko gdy maszyna sesji jest online.",
    forkErrorMissingUuid:
      "Wybrany punkt cofnięcia nie istnieje już w sesji źródłowej — spróbuj rozwidlić bez przycinania.",
    forkErrorMissingMetadata:
      "Brak metadanych sesji wymaganych do rozwidlenia.",
    forkErrorGeneric: "Nie udało się rozwidlić sesji.",
    forkClaudeOnly:
      "Rozwidlenie jest obecnie obsługiwane tylko dla sesji Claude.",
  },
  ...
  sessionInfo: {
    ...
    worktreeCleanupMessage:
      "Worktree nie ma niezatwierdzonych zmian. Czy chcesz usunąć pliki Worktree?",
  ...
  toolGroup: {
    editedFiles: ({ count }: { count: number }) =>
      `${plural({ count, one: "Edytowano 1 plik", few: `Edytowano ${count} pliki`, many: `Edytowano ${count} plików` })}`,
    readFiles: ({ count }: { count: number }) =>
      `${plural({ count, one: "Odczytano 1 plik", few: `Odczytano ${count} pliki`, many: `Odczytano ${count} plików` })}`,
    ranCommands: ({ count }: { count: number }) =>
      `${plural({ count, one: "Wykonano 1 polecenie", few: `Wykonano ${count} polecenia`, many: `Wykonano ${count} poleceń` })}`,
    searched: ({ count }: { count: number }) =>
      `${plural({ count, one: "Wyszukano 1 raz", few: `Wyszukano ${count} razy`, many: `Wyszukano ${count} razy` })}`,
    fetchedUrls: ({ count }: { count: number }) =>
      `${plural({ count, one: "Pobrano 1 URL", few: `Pobrano ${count} URLe`, many: `Pobrano ${count} URLi` })}`,
    ranTasks: ({ count }: { count: number }) =>
      `${plural({ count, one: "Wykonano 1 zadanie", few: `Wykonano ${count} zadania`, many: `Wykonano ${count} zadań` })}`,
    usedTools: ({ count }: { count: number }) =>
      `${plural({ count, one: "Użyto 1 narzędzie", few: `Użyto ${count} narzędzia`, many: `Użyto ${count} narzędzi` })}`,
  },
  ...
  files: {
    ...
    changedFiles: ({ count }: { count: number }) =>
      `${count} ${count === 1 ? "zmieniony plik" : "zmienionych plików"}`,
    ...
    fileConflictDescription:
      "Ten plik został zmodyfikowany na urządzeniu podczas edycji. Załaduj ponownie aby zobaczyć najnowszą wersję.",
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "Terminal żąda połączenia z Twoim kontem NasTech. Pozwoli to terminalowi bezpiecznie wysyłać i odbierać wiadomości.",
  ...
  machine: {
    ...
    delete: "Usuń maszynę",
    deleteFooter:
      "Usuń tę maszynę ze swojego konta. Historia sesji zostanie zachowana, ale nie będziesz mógł uruchamiać nowych sesji na tej maszynie.",
    deleteConfirmTitle: "Usunąć tę maszynę?",
    deleteConfirmMessage:
      "Maszyna zostanie usunięta z twojego konta. Historia sesji zostanie zachowana, ale nie będziesz mógł uruchamiać nowych sesji, dopóki ponownie nie podłączysz demona.",
  ...
  imageUpload: {
    ...
    permissionMessage:
      "Zezwól na dostęp do biblioteki zdjęć, aby załączać obrazy do wiadomości.",
    limitTitle: "Osiągnięto limit obrazów",
    limitMessage: ({ max }: { max: number }) =>
      `Możesz dołączyć maksymalnie ${max} obrazów na wiadomość.`,
    fileTooLargeTitle: "Plik zbyt duży",
    fileTooLargeMessage: ({ name, maxMb }: { name: string; maxMb: number }) =>
      `"${name}" przekracza limit ${maxMb}MB i nie został dodany.`,
    ...
    notSupportedMessage:
      "Ten agent nie obsługuje załączników obrazów. Wysłano tylko tekst.",
  },
