...
export const ru: TranslationStructure = {
  ...
  settings: {
    ...
    showOfflineMachines: ({ count }: { count: number }) => {
      ...
      if (lastOne >= 2 && lastOne <= 4)
        return `Показать ${count} оффлайн-машины`;
      return `Показать ${count} оффлайн-машин`;
    },
    ...
    about: "О программе",
    aboutFooter:
      "NasTech — мобильное приложение для работы с Codex и Claude Code. Использует сквозное шифрование, все данные аккаунта хранятся только на вашем устройстве. Не связано с Anthropic.",
    whatsNew: "Что нового",
  ...
  settingsAppearance: {
    ...
    diffStyleDescription:
      "Показывать различия в одну колонку (unified) или рядом (split). Режим split доступен только на web.",
    diffStyleOptions: {
      unified: "Unified",
      split: "Split",
    },
  ...
  settingsFeatures: {
    ...
    groupToolCallsSubtitle:
      "Сворачивать подряд идущие вызовы инструментов в один блок",
    privacy: "Конфиденциальность",
    privacyDescription:
      "Полностью отключает всю аналитику и телеметрию. Никакие данные не будут отправляться в PostHog или другие сервисы отслеживания.",
    ...
    imageUploadSubtitle:
      "Прикрепляйте изображения к сообщениям для анализа Claude",
  },
  ...
  errors: {
    ...
    voiceHardLimitReached: ({ hours }: { hours: number }) =>
      `Вы использовали ${hours}+ часов голосового общения в этом месяце. Это максимально допустимый лимит. Вы можете настроить собственного агента ElevenLabs в настройках голоса, чтобы использовать свою квоту.`,
    voiceConversationLimitReached:
      "Вы достигли максимального количества голосовых разговоров в этом месяце. Возможно, в будущем мы добавим голосовое использование по запросу — пожалуйста, создайте заявку на github.com/nastech-ai/nastech/issues, если вы столкнулись с этим ограничением.",
  ...
  sessionInfo: {
    ...
    resumeSessionSameMachineOnly:
      "This session can only be resumed on the same machine it started on.",
    resumeSessionMachineOffline:
      "This machine is offline. Resume is only available while it is online.",
    resumeSessionNeedsNasTechAgent:
      "Resume is unavailable on this machine. Run `nastech-agent auth login` to enable it.",
    ...
    resumeSessionMissingBackendId:
      "This session does not have a resumable Claude or Codex identifier.",
    resumeSessionUnexpectedDirectoryPrompt:
      "Resume cannot create directories. Start the session manually from its original path.",
    ...
    worktreeCleanupMessage:
      "В Worktree нет незафиксированных изменений. Хотите удалить файлы Worktree?",
  ...
  session: {
    ...
    duplicateSheetSubtitle:
      "Новая сессия сохранит выбранный ход целиком (ваше сообщение и ответ агента) и отбросит все следующие запросы.",
    duplicateSheetConfirm: "Откатить",
    duplicateSheetEmpty:
      "В этой сессии пока нет сообщений, к которым можно откатиться.",
    ...
    forkErrorOffline:
      "Машина оффлайн. Форк доступен, только пока машина с сессией онлайн.",
    forkErrorMissingUuid:
      "Выбранная точка отката больше не существует в исходной сессии — попробуйте форк без обрезки.",
  ...
  toolGroup: {
    editedFiles: ({ count }: { count: number }) =>
      `${plural({ count, one: "Отредактирован", few: "Отредактировано", many: "Отредактировано" })} ${count} ${plural({ count, one: "файл", few: "файла", many: "файлов" })}`,
    readFiles: ({ count }: { count: number }) =>
      `${plural({ count, one: "Прочитан", few: "Прочитано", many: "Прочитано" })} ${count} ${plural({ count, one: "файл", few: "файла", many: "файлов" })}`,
    ranCommands: ({ count }: { count: number }) =>
      `${plural({ count, one: "Выполнена", few: "Выполнено", many: "Выполнено" })} ${count} ${plural({ count, one: "команда", few: "команды", many: "команд" })}`,
    searched: ({ count }: { count: number }) =>
      `${plural({ count, one: "Выполнен", few: "Выполнено", many: "Выполнено" })} ${count} ${plural({ count, one: "поиск", few: "поиска", many: "поисков" })}`,
    fetchedUrls: ({ count }: { count: number }) =>
      `${plural({ count, one: "Загружен", few: "Загружено", many: "Загружено" })} ${count} URL`,
    ranTasks: ({ count }: { count: number }) =>
      `${plural({ count, one: "Выполнена", few: "Выполнено", many: "Выполнено" })} ${count} ${plural({ count, one: "задача", few: "задачи", many: "задач" })}`,
    usedTools: ({ count }: { count: number }) =>
      `${plural({ count, one: "Использован", few: "Использовано", many: "Использовано" })} ${count} ${plural({ count, one: "инструмент", few: "инструмента", many: "инструментов" })}`,
  },
  ...
  files: {
    ...
    deleted: "Удалён",
    changedFiles: ({ count }: { count: number }) =>
      `${count} ${count === 1 ? "изменённый файл" : count < 5 ? "изменённых файла" : "изменённых файлов"}`,
    allFiles: "Все файлы",
    ...
    fileConflictDescription:
      "Файл был изменён на устройстве пока вы его редактировали. Перезагрузите чтобы увидеть актуальную версию.",
  ...
  terminal: {
    ...
    terminalRequestDescription:
      "Терминал запрашивает подключение к вашему аккаунту NasTech. Это позволит терминалу безопасно отправлять и получать сообщения.",
  ...
  machine: {
    ...
    deleteFooter:
      "Удаляет машину из вашего аккаунта. История сессий сохраняется, но вы больше не сможете запускать новые сессии на ней.",
    deleteConfirmTitle: "Удалить эту машину?",
    deleteConfirmMessage:
      "Машина будет удалена из вашего аккаунта. История сессий сохраняется, но вы больше не сможете запускать новые сессии, пока не подключите демон заново.",
  ...
  imageUpload: {
    ...
    permissionMessage:
      "Разрешите доступ к вашей библиотеке фото, чтобы прикреплять изображения к сообщениям.",
    limitTitle: "Достигнут лимит изображений",
    limitMessage: ({ max }: { max: number }) =>
      `Можно прикрепить не более ${max} изображений на сообщение.`,
    fileTooLargeTitle: "Файл слишком большой",
    fileTooLargeMessage: ({ name, maxMb }: { name: string; maxMb: number }) =>
      `"${name}" превышает лимит ${maxMb}МБ и не был добавлен.`,
    uploadFailedTitle: "Ошибка загрузки",
    uploadFailedMessage: ({ count }: { count: number }) =>
      count === 1
    ...
    notSupportedMessage:
      "Этот агент не поддерживает изображения. Отправлен только текст.",
  },
