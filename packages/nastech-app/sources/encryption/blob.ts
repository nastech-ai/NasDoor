...
export function encryptBlob(data: Uint8Array, key: Uint8Array): Uint8Array {
  ...
  const dataStandalone =
    data.byteOffset === 0 && data.buffer.byteLength === data.length
      ? data
      : data.slice();
  const keyStandalone =
    key.byteOffset === 0 && key.buffer.byteLength === key.length
      ? key
      : key.slice();
  ...
  const encryptedStandalone =
    encrypted.byteOffset === 0 &&
    encrypted.buffer.byteLength === encrypted.length
      ? encrypted
      : encrypted.slice();
