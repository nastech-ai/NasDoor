...
export const SettingsSchema = z.object({
  ...
  diffStyle: z
    .enum(["unified", "split"])
    .describe("Diff view style (split is web-only)"),
  analyticsOptOut: z
    .boolean()
  ...
  expResumeSession: z
    .boolean()
    .describe("Enable experimental session resume feature"),
  fileDiffsSidebar: z
    .boolean()
    .describe("Show the file diffs sidebar next to the chat on desktop"),
  groupToolCalls: z
    .boolean()
    .describe(
  ...
  expImageUpload: z
    .boolean()
    .describe("Enable experimental image upload in chat"),
  ...
  voiceCustomAgentId: z
    .string()
    .nullable()
    .describe("Custom ElevenLabs agent ID (null to use NasTech default)"),
  voiceBypassToken: z
    .boolean()
    .describe(
      "Bypass NasTech server token and connect directly to ElevenLabs (requires custom agent ID)",
    ),
  preferredLanguage: z
    .string()
    .nullable()
  ...
  agentDefaultOverrides: AgentDefaultOverridesSchema.describe(
    "User-selected agent defaults. Missing values use code defaults and are not sent as agent metadata.",
  ),
