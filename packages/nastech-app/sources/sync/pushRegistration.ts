...
export async function requestPushPermissionOrOpenSettings(): Promise<PushPermissionRequestResult> {
  ...
  if (existingPermission.canAskAgain) {
    const requestedPermission = normalizePushPermission(
...
export async function syncCurrentPushToken(
...
): Promise<SyncCurrentPushTokenResult> {
  if (Platform.OS === "web") {
    return {
      registered: false,
      token: null,
  ...
  if (!permission.granted) {
    ...
    permission = normalizePushPermission(
      await Notifications.requestPermissionsAsync(),
    );
    if (!permission.granted) {
      return {
...
export function getCurrentPushDeviceMetadata(): CurrentPushDeviceMetadata {
  const deviceParts = [
    Device.deviceName,
    Device.modelName && Device.modelName !== Device.deviceName
      ? Device.modelName
      : null,
  ...
  const appParts = [
    Application.nativeApplicationVersion
      ? `NasTech ${Application.nativeApplicationVersion}`
      : null,
