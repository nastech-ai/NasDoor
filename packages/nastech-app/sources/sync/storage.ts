...
function useDeepEqual<T>(
  selector: (state: StorageState) => T,
...
function buildSessionRowData(
  ...
  return {
    ...
    completedTodosCount:
      session.todos?.filter((todo) => todo.status === "completed").length ?? 0,
...
interface StorageState {
  ...
  setRealtimeMode: (
    mode: "idle" | "agent-speaking" | "user-speaking",
    immediate?: boolean,
  ) => void;
  ...
  if (activeSessions.length > 0) {
    listData.push({
      ...
      sessions: activeSessions.map((s) =>
        buildSessionRowData(s, unreadSessionIds),
      ),
    });
  }
    ...
    if (currentDateString !== dateString) {
      ...
      if (currentDateGroup.length > 0 && currentDateString) {
        ...
        currentDateGroup.forEach((sess) => {
          listData.push({
            type: "session",
            session: buildSessionRowData(sess, unreadSessionIds),
          });
        });
      }
  ...
  return {
    ...
    applySessions: (
      ...
      set((state) => {
        ...
        sessions.forEach((session) => {
          ...
          const existingPermissionModeRaw =
            state.sessions[session.id]?.permissionMode ?? null;
          const existingPermissionMode =
            existingPermissionModeRaw === "default" &&
          ...
          const existingModelModeRaw =
            state.sessions[session.id]?.modelMode ?? null;
          const existingModelMode =
            existingModelModeRaw === "default" && savedModelMode !== "default"
              ? null
              : existingModelModeRaw;
          const resolvedModelMode =
            existingModelMode ?? savedModelMode ?? session.modelMode ?? null;
          const existingEffortLevel =
            state.sessions[session.id]?.effortLevel ?? null;
        ...
        sessions.forEach((session) => {
          ...
          const wasActive =
            oldSession.thinking === true ||
            (oldSession.agentState?.requests &&
              Object.keys(oldSession.agentState.requests).length > 0);
          const newSession = mergedSessions[session.id];
          if (!newSession || !wasActive) return;
          const isNowIdle =
            newSession.thinking !== true &&
            newSession.presence === "online" &&
            (!newSession.agentState?.requests ||
              Object.keys(newSession.agentState.requests).length === 0);
    ...
    applyMessages: (sessionId: string, messages: NormalizedMessage[]) => {
      ...
      set((state) => {
        ...
        const existingSession: SessionMessages = state.sessionMessages[
          sessionId
        ] || {
          messages: [],
          messagesMap: {},
    ...
    applyOlderMessagesLoading: (sessionId: string, isLoading: boolean) =>
      set((state) => {
        const existing = state.sessionMessages[sessionId];
        if (!existing) {
          return state;
    ...
    applyGitStatus: (pathKey: string, status: GitStatus | null) =>
      set((state) => ({
        ...state,
        pathGitStatus: {
          ...state.pathGitStatus,
    ...
    applyGitStatusFiles: (pathKey: string, files: GitStatusFiles | null) =>
      set((state) => {
    ...
    applyProjectFiles: (pathKey: string, files: ProjectFilesList | null) =>
      set((state) => ({
        ...state,
        pathProjectFiles: {
    ...
    incrementVoiceSessionGeneration: () =>
      set((state) => ({
        ...state,
    ...
    updateSessionPermissionMode: (sessionId: string, mode: string | null) =>
      set((state) => {
        const session = state.sessions[sessionId];
        if (!session) return state;
    ...
    updateSessionEffortLevel: (sessionId: string, level: string | null) =>
      set((state) => {
        const session = state.sessions[sessionId];
        if (!session) return state;
    ...
    deleteSession: (sessionId: string) =>
      set((state) => {
        ...
        const { [sessionId]: _fileCache, ...remainingFileCache } =
          state.sessionFileCache;
    ...
    setCurrentViewingSession: (sessionId: string | null) =>
      set((state) => {
        ...
        const next =
          sessionId && state.unreadSessionIds.has(sessionId)
            ? (() => {
                const s = new Set(state.unreadSessionIds);
                s.delete(sessionId);
                return s;
              })()
...
export function useAllMachines(options?: {
  ...
  return storage(
    useShallow((state) => {
      ...
      const machines = Object.values(state.machines).sort(
        (a, b) => b.createdAt - a.createdAt,
      );
...
export function useSessionGitStatusFiles(
  sessionId: string,
): GitStatusFiles | null {
  return storage(
    useShallow((state) => {
...
export function useSessionProjectFiles(
  sessionId: string,
...
export function useSessionFileCache(sessionId: string, filePath: string) {
  return storage(
    useShallow(
      (state) => state.sessionFileCache[sessionId]?.[filePath] ?? null,
    ),
  );
}
