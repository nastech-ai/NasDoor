...
export async function requestAttachmentUpload(
  ...
  const response = await fetch(
    `${API_ENDPOINT}/v1/sessions/${sessionId}/attachments/request-upload`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    if (response.status === 413) {
      throw new Error(
        `Attachment too large (max ${MAX_FILE_SIZE / 1024 / 1024}MB)`,
      );
    }
...
export async function uploadEncryptedBlob(
  ...
  if (upload.method === "POST") {
    ...
    const cleanup = await appendFormFile(
      formData,
      encryptedData,
      "file",
      "blob",
    ...
    try {
      ...
      throw new Error(
        `Blob upload (POST) network error to ${upload.uploadUrl}: ${message}`,
      );
    }
    await cleanup();
    if (!response.ok) {
      throw new Error(
        `Blob upload (POST) failed: ${response.status} ${response.statusText} at ${upload.uploadUrl}`,
      );
    }
    return;
  }
  ...
  try {
    ...
    throw new Error(
      `Blob upload (PUT) network error to ${upload.uploadUrl}: ${message}`,
    );
  }

  if (!response.ok) {
    throw new Error(
      `Blob upload (PUT) failed: ${response.status} ${response.statusText} at ${upload.uploadUrl}`,
    );
  }
}
...
export async function downloadEncryptedAttachment(
  ...
  const requestRes = await fetch(
    `${API_ENDPOINT}/v1/sessions/${sessionId}/attachments/request-download`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  const { downloadUrl: rawDownloadUrl } = (await requestRes.json()) as {
    downloadUrl: string;
  };
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    throw new Error(
      `Attachment download network error from ${downloadUrl}: ${message}`,
    );
  }
  if (!blobRes.ok) {
    throw new Error(
      `Attachment download failed: ${blobRes.status} ${blobRes.statusText} from ${downloadUrl}`,
    );
  }
