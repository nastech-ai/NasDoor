...
export async function fetchNasTechSessions(
  limit = 20,
  offset = 0,
): Promise<NasTechSession[]> {
  const res = await nastechFetch(
...
export async function fetchNasTechMessages(
  sessionId: string,
): Promise<NasTechMessage[]> {
...
export async function deleteNasTechSession(sessionId: string): Promise<void> {
  const res = await nastechFetch(`/api/sessions/${sessionId}`, {
    method: "DELETE",
  });
