...
describe("modeHacks", () => {
  it("capitalizes plan/build when key and name are lowercase", () => {
    expect(
      hackMode({ key: "build", name: "build", description: null }).name,
    ).toBe("Build");
    expect(
      hackMode({ key: "plan", name: "plan", description: null }).name,
    ).toBe("Plan");
  });

  it("normalizes build and plan duplicated labels", () => {
    expect(
      hackMode({ key: "build", name: "build, build", description: null }).name,
    ).toBe("Build");
    expect(
      hackMode({ key: "plan", name: "plan/plan", description: null }).name,
    ).toBe("Plan");
  });
