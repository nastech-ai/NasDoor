...
export interface SpawnSessionOptions {
   ...
   * NasTech session attaches to a pre-existing on-disk Claude conversation
   * file. Used by the session fork / duplicate flow.
   */
  resumeClaudeSessionId?: string;
  /** NasTech session id this fork was branched from (lineage). */
  parentSessionId?: string;
  /** NasTech message id used as the rewind point (only set for "duplicate"). */
...
export async function machineSpawnNewSession(
  ...
  try {
    ...
    >(machineId, "spawn-nastech-session", {
      type: "spawn-in-directory",
      directory,
 ...
 * the new Claude session UUID. Caller then spawns a fresh NasTech session
 * with `resumeClaudeSessionId` set to that UUID to attach a new NasTech
 * session row to the copied conversation.
 */
export async function claudeForkSession(
  ...
  try {
    ...
    >(machineId, "claude-fork-session", { directory, claudeSessionId });
    return result;
  } catch (error) {
    return {
      type: "error",
...
export async function claudeListRewindPoints(
  ...
  try {
    const result = await apiSocket.machineRPC<
      ClaudeListRewindPointsResult,
      {
        directory: string;
        claudeSessionId: string;
      }
    >(machineId, "claude-list-rewind-points", { directory, claudeSessionId });
    return result;
  } catch (error) {
    return {
      type: "error",
      errorMessage:
        error instanceof Error ? error.message : "Failed to list rewind points",
    };
  }
}
...
export async function claudeDuplicateSession(
  ...
  try {
    ...
    >(machineId, "claude-duplicate-session", {
      directory,
      claudeSessionId,
      cutAfterUuid,
    });
...
export async function machineResumeSession(
  ...
  try {
    ...
    >(machineId, "resume-nastech-session", {
      sessionId,
      model,
      permissionMode,
    });
 ...
 *   2. Spawn a new NasTech session on the same machine with
 ...
 * RPC into env vars, then into the new NasTech session's metadata at start
