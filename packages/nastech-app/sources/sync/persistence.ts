...
const VOICE_ONBOARDING_PROMPT_LOAD_COUNT_KEY =
  "voice-onboarding-prompt-load-count";
...
export function loadThemePreference(): import("./localSettings").ThemePreference {
  const localSettings = mmkv.getString("local-settings");
  if (localSettings) {
    try {
      const parsed = JSON.parse(localSettings);
...
export function loadNewSessionDraft(): NewSessionDraft | null {
  ...
  try {
    ...
    const agentType: NewSessionAgentType =
      ...
      parsed.agentType === "gemini" ||
      parsed.agentType === "openclaw"
        ? parsed.agentType
        : "claude";
    ...
    const modelMode: string =
      typeof parsed.modelMode === "string" ? parsed.modelMode : "default";
    const sessionType: NewSessionSessionType =
      parsed.sessionType === "worktree" ? "worktree" : "simple";
    const worktreeKey =
      typeof parsed.worktreeKey === "string" ? parsed.worktreeKey : null;
    const updatedAt =
...
export function incrementVoiceOnboardingPromptLoadCount() {
  mmkv.set(
    ...
    getVoiceOnboardingPromptLoadCount() + 1,
  );
}
