...
describe("Phase 0 permission skipping issue", () => {
  it("should handle permissions when AgentState and matching tools arrive together", () => {
    ...
    const writeTool = result.messages.find(
      (m) => m.kind === "tool-call" && m.tool?.name === "Write",
    );
    const readTool = result.messages.find(
      (m) => m.kind === "tool-call" && m.tool?.name === "Read",
    );
  ...
  it("should handle case where tools arrive first, then AgentState arrives later", () => {
    ...
    const toolBeforePermission = result1.messages.find(
      (m) => m.kind === "tool-call",
    );
