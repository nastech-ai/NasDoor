  ...
  if (msg.role === "agent") {
    for (const content of msg.content) {
      ...
      if (content.type === "text") {
        const limitMatch = content.text.match(
      ...
      // Check for mcp__nastech__change_title tool calls
      if (
        content.type === "tool-call" &&
        content.name === "mcp__nastech__change_title"
      ) {
      ...
      if (
        content.type === "tool-call" &&
        (content.name === "EnterPlanMode" || content.name === "enter_plan_mode")
      ) {
        return {
          type: "message",
