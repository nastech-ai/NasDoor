...
function getVisibleSidechainPrompt(
  owner: ReducerMessage | null,
): string | null {
...
export function reducer(
      ...
      for (const [permId, request] of Object.entries(agentState.requests)) {
        ...
        if (existingMessageId) {
          ...
          if (message?.tool && !message.tool.permission) {
            if (ENABLE_LOGGING) {
              console.log(
                `[REDUCER] Updating existing tool ${permId} with permission`,
              );
            }
          ...
          if (ENABLE_LOGGING) {
            console.log(
              `[REDUCER] Creating new message for permission ${permId}`,
            );
          }
        ...
        if (messageId) {
          ...
          if (message?.tool) {
            ...
            if (!message.tool.permission) {
              ...
              message.tool.permission.allowedTools =
                completed.allowedTools || undefined;
              message.tool.permission.decision =
          ...
          if (incomingToolIds.has(permId)) {
            if (ENABLE_LOGGING) {
              console.log(
                `[REDUCER] Storing permission ${permId} for incoming tool`,
              );
            }
        ...
        if (c.type === "tool-call") {
          ...
          if (existingMessageId) {
            ...
            let toolCall: ToolCall = {
              ...
              state: "running" as const,
              input: permission
                ? mergeToolInputs(permission.arguments, c.input)
                : c.input,
            ...
            if (permission) {
              if (ENABLE_LOGGING) {
                console.log(
                  `[REDUCER] Found stored permission for tool ${c.id}`,
                );
              }
              toolCall.permission = {
                id: c.id,
  ...
  for (const msg of sidechainMessages) {
    ...
    if (msg.role === "agent" && msg.content[0]?.type === "sidechain") {
      ...
      if (
        isDuplicateSidechainPrompt(
          existingSidechain,
          ownerPrompt,
          msg.content[0].prompt,
        )
      ) {
      ...
      for (let c of msg.content) {
        if (c.type === "text" || c.type === "thinking") {
          ...
          if (
            c.type === "text" &&
            isDuplicateSidechainPrompt(existingSidechain, ownerPrompt, text)
          ) {
            continue;
          }
          let mid = allocateId();
          ...
          let sidechainMessageId = state.sidechainToolIdToMessageId.get(
            c.tool_use_id,
          );
              ...
              if (c.permissions) {
                ...
                if (sidechainMessage.tool.permission) {
                  const existingDecision =
                    sidechainMessage.tool.permission.decision;
                  sidechainMessage.tool.permission = {
                    ...sidechainMessage.tool.permission,
                    id: c.tool_use_id,
                    status:
                      c.permissions.result === "approved"
                        ? "approved"
                        : "denied",
                    date: c.permissions.date,
                    mode: c.permissions.mode,
          ...
          if (permissionMessageId) {
            ...
            if (
              ...
              if (c.permissions) {
                ...
                if (permissionMessage.tool.permission) {
                  const existingDecision =
                    permissionMessage.tool.permission.decision;
