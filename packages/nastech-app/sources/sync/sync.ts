...
class Sync {
  ...
  private async uploadAttachmentsForSession(
    ...
    for (const attachment of attachments) {
      try {
        ...
        console.error(
          `[attachments] Failed to upload ${attachment.name}:`,
          err,
        );
        failed++;
  ...
  async sendMessage(
      ...
      if (uploaded.length > 0) {
        ...
        for (const att of uploaded) {
          ...
          const encryptedFileRecord =
            await encryption.encryptRawRecord(fileRecord);
          const fileLocalId = randomUUID();
          const fileNormalized = normalizeRawMessage(
            fileLocalId,
            fileLocalId,
            Date.now(),
            fileRecord,
    ...
    const content: RawRecord = {
      ...
      meta: {
        ...
        ...(modeMeta.permissionMode !== undefined
  ...
  private prefetchOlderMessagesInBackground = async (sessionId: string) => {
    ...
    while (true) {
      ...
      try {
        ...
        log.log(
          `💬 prefetchOlderMessagesInBackground: error for ${sessionId}, stopping: ${String(error)}`,
        );
        return;
      }

      await new Promise((resolve) =>
        setTimeout(resolve, SLEEP_BETWEEN_PAGES_MS),
      );
    }
  };
  ...
  private fetchInitialLatestPage = async (
    ...
    const response = await fetch(
      `${API_ENDPOINT}/api/sessions/${sessionId}/messages`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch initial page for ${sessionId}: ${response.status}`,
      );
    }
    const data = await response.json();
    ...
    storage
      .getState()
      .applyOlderMessagesPagination(sessionId, { hasMore: false });
  };
  ...
  private fetchForwardSince = async (
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to forward-sync ${sessionId}: ${response.status}`,
      );
    }
    const data = await response.json();
  ...
  private mapNasTechRawMsg = (
    ...
    const createdAt = rawTs
      ? typeof rawTs === "number"
        ? rawTs * 1000
        : new Date(rawTs as string).getTime()
      : Date.now();
    const text =
  ...
  loadOlderMessages = async (sessionId: string) => {
    ...
    if (
      !sessionMessages ||
      sessionMessages.isLoadingOlder ||
      !sessionMessages.hasMoreOlder
    ) {
      return;
    }
  ...
  private handleUpdate = async (update: unknown) => {
    ...
    if (updateData.body.t === "new-message") {
      ...
      let encryption = this.encryption.getSessionEncryption(
        updateData.body.sid,
      );
      if (!encryption) {
        await this.sessionsSync.awaitQueue();
      ...
      let sessionEncryption = this.encryption.getSessionEncryption(
        updateData.body.id,
      );
      if (!session || !sessionEncryption) {
        await this.sessionsSync.awaitQueue();
        session = storage.getState().sessions[updateData.body.id];
        sessionEncryption = this.encryption.getSessionEncryption(
          updateData.body.id,
        );
      }
      if (session) {
        if (!sessionEncryption) {
          console.error(
            `Session encryption not found for ${updateData.body.id} after sync`,
          );
          this.fetchSessions();
          return;
        }
      ...
      if (accountUpdate.settings?.value) {
        try {
          ...
          this.applyServerSettings(
            parsedSettings,
            accountUpdate.settings.version,
          );
          log.log(
