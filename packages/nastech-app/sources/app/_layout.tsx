  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        // Start background daemon (Android only — no-op on iOS)
        initDaemon().catch((e) => console.warn("[Daemon] init error:", e));
      } catch (error) {
  ...
  const handleNotificationResponse = React.useCallback(
    async (response: Notifications.NotificationResponse | null) => {
      ...
      if (handledNotificationIds.current.has(responseId)) {
        console.log(
          `[PUSH ROUTING] Duplicate notification response ignored: ${responseId}`,
        );
        return;
      }
      ...
      try {
        if (
          ...
          console.log(
            `[PUSH ROUTING] Ignoring non-default action: ${response.actionIdentifier}`,
          );
          return;
        }
        ...
        if (!route) {
          console.log(
            "[PUSH ROUTING] No session route found in notification.request.content.data",
          );
          return;
        }
