...
export default function ArtifactsScreen() {
  ...
  React.useEffect(() => {
    ...
    console.log(
      `📱 ArtifactsScreen: Current artifacts count: ${artifacts.length}`,
    );
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        if (!credentials) {
          console.log(
            "📱 ArtifactsScreen: No credentials available, skipping fetch",
          );
          return;
        }
  ...
  React.useEffect(() => {
    console.log(
      `📱 ArtifactsScreen: Artifacts array updated, count: ${artifacts.length}`,
    );
  ...
  const keyExtractor = React.useCallback(
    (item: DecryptedArtifact) => item.id,
    [],
  );
