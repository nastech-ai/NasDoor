...
export default function TerminalConnectScreen() {
  ...
  const handleConnect = async () => {
    if (publicKey) {
      // Convert the hash key format to the expected nastech:// URL format
      const authUrl = `nastech://terminal?${publicKey}`;
    ...
    <ItemList>
      ...
      <ItemGroup title={t("terminal.connectionDetails")}>
        ...
        <Item
          ...
          icon={
            <Ionicons name="lock-closed-outline" size={29} color="#34C759" />
          }
          showChevron={false}
        />
      </ItemGroup>
