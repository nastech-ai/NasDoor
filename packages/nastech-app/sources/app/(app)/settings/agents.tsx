...
export default function AgentDefaultsSettingsScreen() {
  ...
  const [agentDefaultOverrides, setAgentDefaultOverrides] = useSettingMutable(
    ...
    return (
      <React.Fragment key={`${agent}-${config.field}`}>
        <Item
          ...
          onPress={() =>
            setExpanded(isExpanded ? null : { agent, field: config.field })
          }
        />
        {isExpanded && (
          <>
  ...
  return (
    <ItemList style={{ paddingTop: 0 }}>
      <ItemGroup title="Agent Defaults">
        <Item
      ...
      {agentKeys.map((agent) => {
        ...
        const effortOptions = getEffortLevelsForModel(
          agent,
