...
export default React.memo(function VoiceSettingsScreen() {
  ...
  const [voiceCustomAgentId, setVoiceCustomAgentId] =
    useSettingMutable("voiceCustomAgentId");
  const [voiceBypassToken, setVoiceBypassToken] =
    useSettingMutable("voiceBypassToken");
  const [voiceUpsellOverride, setVoiceUpsellOverride] = useLocalSettingMutable(
    "voiceUpsellOverride",
  );
  ...
  const [voiceLocalCounters, setVoiceLocalCounters] = React.useState(() =>
    getVoiceLocalCounters(),
  );
    ...
    Modal.alert(
      ...
      [
        ...
        { text: "Control", onPress: () => setVoiceUpsellOverride("control") },
        {
          text: "Soft Paywall",
          onPress: () =>
            setVoiceUpsellOverride("show-paywall-before-first-voice-chat"),
        },
        {
          text: "Onboarding + Upsell",
          onPress: () => setVoiceUpsellOverride("voice-onboarding-and-upsell"),
        },
      ],
    );
  ...
  const developerExperimentSubtitle = React.useMemo(() => {
    const upsellVariant = getVoiceUpsellVariantLabel(
      voiceExperimentStatus.upsellVariant,
    );
    const gatingMode =
      voiceExperimentStatus.gatingMode === "direct-byo-agent"
    ...
    <ItemList style={{ paddingTop: 0 }}>
        ...
        >
          <View style={{ paddingHorizontal: 16, paddingVertical: 8 }}>
            <UsageBar
              ...
              color={
                usage.usedSeconds >= usage.limitSeconds ? "#FF3B30" : "#007AFF"
              }
            />
            <Text style={{ fontSize: 13, color: "#8E8E93", marginTop: 4 }}>
              {formatVoiceTime(usage.usedSeconds)} /{" "}
              {formatVoiceTime(usage.limitSeconds)}
            </Text>
            <UsageBar
      ...
      >
        <Item
          ...
          subtitle={
            voiceCustomAgentId ?? t("settingsVoice.customAgentIdNotSet")
          }
