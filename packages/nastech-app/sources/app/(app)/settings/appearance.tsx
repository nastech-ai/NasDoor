...
function applyTheme(nextTheme: ThemePreference) {
  if (nextTheme === "adaptive") {
    ...
    const color = (themeObj?.colors?.groupped?.background ??
      "#000000") as string;
    UnistylesRuntime.setRootViewBackgroundColor(color);
    SystemUI.setBackgroundColorAsync(color);
  }
...
const THEME_GROUPS: {
  title: string;
  themes: { id: ThemePreference; label: string }[];
}[] = [
  {
    title: "Black",
    themes: [
    ...
    <ItemList style={{ paddingTop: 0 }}>
      ...
      >
        ...
        <Item
          ...
          icon={
            <Ionicons name="arrow-redo-outline" size={29} color="#5856D6" />
          }
          rightElement={
            <Switch
