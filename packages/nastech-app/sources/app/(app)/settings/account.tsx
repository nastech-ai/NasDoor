...
function formatPushPermissionLabel(
  permission: PushPermissionInfo | null,
): string {
  if (!permission) {
    return "Loading";
  }
...
function formatPushPermissionSubtitle(
  permission: PushPermissionInfo | null,
): string {
  if (!permission) {
...
function buildPushTokenSubtitle(
  pushToken: PushToken,
  options: {
...
export default React.memo(() => {
  ...
  const [analyticsOptOut, setAnalyticsOptOut] =
    useSettingMutable("analyticsOptOut");
  ...
  const [pushPermission, setPushPermission] =
    useState<PushPermissionInfo | null>(null);
  ...
  const [requestingPushPermission, setRequestingPushPermission] =
    useState(false);
  const [refreshingPushToken, setRefreshingPushToken] = useState(false);
  const [deletingPushToken, setDeletingPushToken] = useState<string | null>(
    null,
  );

  // Get the current secret key
  const currentSecret = auth.credentials?.secret || "";
  const formattedSecret = currentSecret
    ? formatSecretKeyForBackup(currentSecret)
    : "";
  ...
  const loadPushSettings = useCallback(
  ...
  const handlePushPermissionRequest = useCallback(async () => {
    ...
    try {
      ...
      if (result.openedSettings) {
        Modal.alert(
          "Open Settings",
          "The system will not show the permission prompt again, so NasTech opened Settings instead.",
        );
        return;
      }
  ...
  const handleDeletePushToken = useCallback(
      ...
      <ItemList>
            ...
            return (
              <ItemGroup title={t("settings.connectedAccounts")}>
                {displayServices.map((service) => {
                  const serviceInfo =
                    knownServices[service as keyof typeof knownServices];
                  ...
                  return (
                    <Item
                      ...
                      subtitle={t("settingsAccount.tapToDisconnect")}
                      onPress={() =>
                        handleDisconnectService(service, serviceInfo.name)
                      }
        ...
        >
          ...
          <Item
            ...
            disabled={
              refreshingPushToken || loadingPushSettings || !auth.credentials
            }
            showChevron={false}
          />
        </ItemGroup>
                ...
                return (
                  <Item
                    ...
                    onPress={
                      isCurrentDevice
                        ? undefined
                        : () => handleDeletePushToken(pushToken)
                    }
