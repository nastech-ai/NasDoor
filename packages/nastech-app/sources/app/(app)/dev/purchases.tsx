...
export default function PurchasesDevScreen() {
  ...
  const sortedEntitlements = React.useMemo(() => {
    return Object.entries(purchases.entitlements).sort(([a], [b]) =>
      a.localeCompare(b),
    );
  ...
  const fetchOfferings = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "Current offering:",
          result.offerings.current?.identifier || "None",
        );

        if (result.offerings.current) {
          console.log("\nCurrent Offering Packages:");
          Object.entries(
            result.offerings.current.availablePackages || {},
          ).forEach(([key, pkg]: [string, any]) => {
            console.log(
              `  - ${key}: ${pkg.product.identifier} (${pkg.product.priceString})`,
            );
          });
        }

        console.log("\nAll Offerings:");
        Object.entries(result.offerings.all || {}).forEach(
          ([id, offering]: [string, any]) => {
            console.log(
    ...
    <>
      ...
      <ItemList>
        ...
        <ItemGroup
          ...
          footer="Enter a product ID to purchase"
        >
          <View
            style={{
              backgroundColor: "#fff",
        ...
        {offerings && (
          <ItemGroup
            title="Offerings"
            footer="Check console logs for full details"
          >
            <Item
        ...
        <ItemGroup title="Debug Info">
          <Item
            ...
            detail={
              sync.revenueCatInitialized ? "Initialized" : "Not Initialized"
            }
            showChevron={false}
          />
          <Item
