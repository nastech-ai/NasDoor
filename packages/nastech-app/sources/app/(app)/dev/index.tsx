...
export default function DevScreen() {
  ...
  const [verboseLogging, setVerboseLogging] =
    useLocalSettingMutable("verboseLogging");
  const [consoleLoggingEnabled, setConsoleLoggingEnabled] =
    useLocalSettingMutable("consoleLoggingEnabled");
  ...
  const handleEditLogServerUrl = async () => {
    ...
    if (newUrl !== undefined && newUrl !== currentUrl) {
      if (!newUrl || !newUrl.trim()) {
        ...
        Modal.alert(
          "Success",
          "Remote logging disabled. Restart app for changes to take effect.",
        );
      } else {
        const validation = validateServerUrl(newUrl);
        if (validation.valid) {
          ...
          Modal.alert(
            "Success",
            "Log server URL updated. Restart app for changes to take effect.",
          );
        } else {
          Modal.alert(
            "Invalid URL",
    ...
    <ItemList>
      ...
      <ItemGroup title="Debug Options">
        ...
        <Item
          ...
          rightElement={
            <Switch value={verboseLogging} onValueChange={setVerboseLogging} />
          }
          showChevron={false}
        />
        <Item
      ...
      <ItemGroup title="Component Demos">
        ...
        <Item
          ...
          icon={
            <Ionicons name="swap-vertical-outline" size={28} color="#007AFF" />
          }
      ...
      <ItemGroup
        title="Test Features"
        footer="These actions may affect app stability"
      >
        <Item
        ...
        <Item
          ...
          onPress={() => {
            ...
            Modal.alert(
              "Done",
              "Changelog reset. Restart app to see the banner.",
            );
          }}
        />
        <Item
