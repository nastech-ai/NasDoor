...
const SAMPLE_PATHS: PickerItem[] = [
  { key: "nastech", label: "~/projects/nastech/nastech" },
...
function SessionComposerDemo() {
  ...
  const [selectedPath, setSelectedPath] = React.useState("nastech");
  ...
  React.useEffect(() => {
    const defaultEffort = getDefaultEffortKeyForModel(
      selectedAgent,
      currentModelKey,
    );
  ...
  // Display values
  const machineName =
    SAMPLE_MACHINES.find((m) => m.key === selectedMachine)?.label ?? "";
  const pathName =
    SAMPLE_PATHS.find((p) => p.key === selectedPath)?.label ?? "";
  const worktreeLabel =
