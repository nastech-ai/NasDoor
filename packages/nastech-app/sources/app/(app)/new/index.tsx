    ...
    >
      <View style={sheetStyles.overlay}>
        <TouchableWithoutFeedback onPress={onClose}>
          <Animated.View
            style={[sheetStyles.backdrop, { opacity: fadeAnim }]}
          />
        </TouchableWithoutFeedback>
        <Animated.View
          style={[
...
function PathPickerContent({
  ...
  const isCustomPath =
    currentValue.trim().length > 0 && matchedItemKey === null;
      ...
      <View style={pickerStyles.titleRow}>
        ...
        {Platform.OS !== "web" && onDone && (
          ...
          >
            ...
            >
              <Ionicons name="checkmark" size={20} color={doneIconColor} />
            </GlassView>
          </Pressable>
        )}
      </View>
...
const PromptInput = React.memo(
  React.forwardRef<MultiTextInputHandle, PromptInputProps>(
...
function NewSessionScreen() {
  ...
  React.useEffect(() => {
    ...
    draft.setWorktreeKey(
      worktreeKey === "__none__" || worktreeKey === "__new__"
        ? null
        : worktreeKey,
    );
  }, [worktreeKey]);
  ...
  const machineItems = React.useMemo<PickerItem[]>(() => {
    ...
    return sorted.map((m) => ({
      ...
      subtitle: isMachineOnline(m)
        ? t("status.online")
        : t("status.lastSeen", { time: formatLastSeen(m.activeAt, false) }),
  ...
  const [debouncedResolvedSelectedPath, setDebouncedResolvedSelectedPath] =
    React.useState<string | null>(resolvedSelectedPath);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    listWorktrees(selectedMachineId, debouncedResolvedSelectedPath).then(
      (worktrees) => {
        if (cancelled) return;
        setWorktreeItems(
          worktrees.map((wt) => ({
  ...
  React.useEffect(() => {
    if (
      ...
      !availableAgents.find((a) => a.key === selectedAgent)
    ) {
  ...
  React.useEffect(() => {
    const defaultPermIdx = permissionModes.findIndex(
      (m) => m.key === effectiveAgentDefaults.permissionMode,
    );
    setPermissionIndex(defaultPermIdx >= 0 ? defaultPermIdx : 0);

    const defaultModelIdx = modelModes.findIndex(
      (m) => m.key === effectiveAgentDefaults.modelMode,
    );
  ...
  const agent =
    availableAgents.find((a) => a.key === selectedAgent) ?? ALL_AGENTS[0];
  const currentPermission =
    permissionModes[permissionIndex] ?? permissionModes[0];
  const currentEffort = effortLevels[effortIndex] ?? effortLevels[0];
  const permissionStyle =
    currentPermission?.key !== "default"
      ? getPermissionStyle(currentPermission.key)
      : null;
  ...
  const worktreeLabel =
    worktreeKey === "__none__"
      ...
      : worktreeKey === "__new__"
        ? "new worktree"
        : worktreeItems.find((wt) => wt.key === worktreeKey)?.label ||
          worktreeKey;
  ...
  const pickerData = React.useMemo(() => {
    switch (activePicker) {
      ...
      case "worktree":
        return {
          title: "Worktree",
          fixedItems: WORKTREE_FIXED_ITEMS,
          items: worktreeItems,
  ...
  const handleSend = React.useCallback(
    async (approvedNewDirectoryCreation: boolean = false) => {
        ...
        switch (result.type) {
          case "success":
            ...
            const permissionOverride =
              currentPermission.key === effectiveAgentDefaults.permissionMode
                ? null
                : currentPermission.key;
            const modelOverride =
              currentModelKey === effectiveAgentDefaults.modelMode
                ? null
                : currentModelKey;
            const currentEffortKey = currentEffort?.key ?? null;
            const effortOverride =
              currentEffortKey === effectiveAgentDefaults.effortLevel
                ? null
                : currentEffortKey;
            storage
              .getState()
            ...
            storage
              .getState()
              .updateSessionModelMode(result.sessionId, modelOverride);
            storage
              .getState()
              .updateSessionEffortLevel(result.sessionId, effortOverride);
  ...
  const composerInputRef =
    React.useRef<import("@/components/MultiTextInput").MultiTextInputHandle>(
      null,
    );
  React.useEffect(() => {
    const timeout = setTimeout(() => {
  ...
  return (
    <KeyboardAvoidingView
      ...
      keyboardVerticalOffset={
        Platform.OS === "ios" ? Constants.statusBarHeight + headerHeight : 0
      }
      ...
      <View style={styles.inner}>
        ...
        >
              ...
              <>
                ...
                <View style={styles.configRowWithToggle}>
                  ...
                  >
                    ...
                    />
                    <Text
                      style={[styles.configLabel, { flex: 1 }]}
                      numberOfLines={1}
                    >
                      {pathName}
                    </Text>
                  ...
                  {showPermission && (
                    <Pressable
                      onPress={() => {
                        ...
                        showFlash(
                          permissionModes[
                            (permissionIndex + 1) % permissionModes.length
                          ]?.name ?? "default",
                        );
                      }}
                      hitSlop={{ top: 4, bottom: 4, left: 4, right: 4 }}
                    ...
                    >
                      <Ionicons
                        ...
                        size={14}
                        color={
                          permissionStyle?.color ?? theme.colors.textSecondary
                        }
                      />
                    </Pressable>
                  )}
          ...
          {flashText !== "" && !activePicker && (
            <Animated.View
              style={[styles.flashLabel, { opacity: flashOpacity }]}
              pointerEvents="none"
            >
              <Text
                style={[
                  styles.flashLabelText,
                  { color: theme.colors.textSecondary },
                ]}
              >
                {flashText}
