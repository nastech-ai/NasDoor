...
export default React.memo(function FileScreen() {
  ...
  const gitDiffPath = resolvedPath?.withinSessionRoot
  ...
  const [diffContent, setDiffContent] = React.useState<string | null>(
    () => cached?.diff ?? null,
  );
      ...
      try {
        ...
        if (isBinaryFile(filePath)) {
          if (!isCancelled) {
            ...
            storage
              .getState()
              .applyFileCache(sessionId!, filePath, "", null, true);
            setIsLoading(false);
          }
          return;
        }
        ...
        if (!isCancelled) {
          if (response.success && response.content) {
            ...
            try {
              ...
              storage
                .getState()
                .applyFileCache(sessionId!, filePath, "", fetchedDiff, true);
              return;
            }

            const hasNullBytes = rawBytes.some((byte) => byte === 0);
            const nonPrintableCount = decodedContent
              .split("")
              .filter((char) => {
            ...
            const isBinary =
              hasNullBytes || nonPrintableCount / decodedContent.length > 0.1;
          ...
          >
            <Text
              style={{
                ...
                fontWeight: "600",
                color:
                  displayMode === "file" ? "white" : theme.colors.textSecondary,
                ...Typography.default(),
              }}
            >
              {t("files.file")}
