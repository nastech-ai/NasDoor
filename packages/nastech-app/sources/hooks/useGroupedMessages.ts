...
export function useGroupedMessages(
  ...
  return React.useMemo(() => {
    if (!enabled) {
      return messages.map(
        (msg) => ({ type: "message", id: msg.id, message: msg }) as TextItem,
      );
    }
...
export function generateGroupSummary(messages: Message[]): string {
  ...
  if (counts.edit)
    parts.push(t("toolGroup.editedFiles", { count: counts.edit }));
  if (counts.read) parts.push(t("toolGroup.readFiles", { count: counts.read }));
  if (counts.terminal)
    parts.push(t("toolGroup.ranCommands", { count: counts.terminal }));
  if (counts.search)
    parts.push(t("toolGroup.searched", { count: counts.search }));
  ...
  if (counts.other)
    parts.push(t("toolGroup.usedTools", { count: counts.other }));

  return (
    parts.join(", ") || t("toolGroup.usedTools", { count: messages.length })
  );
}
