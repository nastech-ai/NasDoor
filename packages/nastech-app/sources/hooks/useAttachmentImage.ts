...
async function loadAttachmentDataUri(
  ...
  if (!blobKey) {
    console.warn(
      `[attachment-image] no blobKey for session ${sessionId} (ref=${ref})`,
    );
    return null;
  }
  if (blobKey.length !== 32) {
    console.warn(
      `[attachment-image] blobKey wrong length: ${blobKey.length} (ref=${ref})`,
    );
    return null;
  }
  ...
  if (!decrypted) {
    console.warn(
      `[attachment-image] decrypt returned null for ${ref} (encrypted.length=${encrypted.length})`,
    );
    return null;
  }
...
export function useAttachmentImage(
  ...
  React.useEffect(() => {
    ...
    if (!promise) {
      promise = loadAttachmentDataUri(sessionId, ref).finally(() => {
        inFlight.delete(ref);
      });
