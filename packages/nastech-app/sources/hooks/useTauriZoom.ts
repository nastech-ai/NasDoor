...
const WEB_ZOOM_CLASS = "nastech-app-zoomed";

const clampZoom = (zoom: number) =>
  Math.max(MIN_APP_ZOOM, Math.min(MAX_APP_ZOOM, zoom));
...
export function useTauriZoom() {
  useEffect(() => {
    ...
    if (!inTauri) {
      root.style.setProperty("--nastech-app-zoom", String(DEFAULT_APP_ZOOM));
      root.classList.add(WEB_ZOOM_CLASS);
      return () => {
        root.classList.remove(WEB_ZOOM_CLASS);
        root.style.removeProperty("--nastech-app-zoom");
      };
    }
