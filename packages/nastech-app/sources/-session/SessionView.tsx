...
import {
  getCurrentVoiceConversationId,
  getCurrentVoiceSessionDurationSeconds,
...
export const SessionView = React.memo((props: { id: string }) => {
  ...
  const [overlayHistory, setOverlayHistory] = React.useState<{
    stack: OverlayEntry[];
    cursor: number;
  }>({ stack: [{ kind: "none" }], cursor: 0 });
  const overlayCurrent = overlayHistory.stack[overlayHistory.cursor] ?? {
    kind: "none",
  };
  const diffViewOpen = overlayCurrent.kind === "diff";
  const fileViewPath =
    overlayCurrent.kind === "file" ? overlayCurrent.path : null;
  const scrollToFile =
    overlayCurrent.kind === "diff" ? overlayCurrent.file : null;
  ...
  const handleSidebarFilePress = React.useCallback(
    (file: GitFileStatus) => {
  ...
  const handleAllFilesFilePress = React.useCallback(
    (filePath: string) => {
  ...
  const [headerRightSlot, setHeaderRightSlot] =
  ...
  const canOverlayForward =
    overlayHistory.cursor < overlayHistory.stack.length - 1;
  React.useEffect(() => {
      ...
      <View
        style={{
          ...
          paddingTop: !(
            ...
            ? safeArea.top +
              headerHeight +
              (!isTablet && realtimeStatus !== "disconnected" ? 32 : 0)
            : 0,
        }}
      >
        {!isDataReady ? (
      ...
      >
        <View style={{ width: sidebarWidth, flex: 1 }}>
          <FilesSidebar
            sessionId={sessionId}
            selectedPath={
              sidebarMode === "changes" ? scrollToFile : fileViewPath
            }
...
const ChatComposer = React.memo(function ChatComposer(
  ...
  const applyDraft = React.useCallback((text: string) => {
    inputHandleRef.current?.setTextAndSelection(text, {
      start: text.length,
      end: text.length,
    });
    setMessage(text);
...
function SessionViewLoaded({
  ...
  const zenMode = useLocalSetting("zenMode");
  const sessionInputHorizontalPadding =
  ...
  const { canResume, resumeSession, resumingSession } =
  ...
  const { selectedImages, pickImages, removeImage, clearImages, addImages } =
    useImagePicker();
  ...
  const handleMicrophonePress = React.useCallback(async () => {
    ...
    if (realtimeStatus === "disconnected" || realtimeStatus === "error") {
      try {
        ...
        const conversationId = await startRealtimeSession(
          sessionId,
          initialPrompt,
        );
        if (conversationId) {
          const hasPro =
            storage.getState().purchases.entitlements["pro"] ?? false;
  ...
  const composer = (
    <ChatComposer
      ...
      onFileViewerPress={
        experiments && !isTablet ? handleFileViewerPress : undefined
      }
    ...
    <>
      ...
      <View
        style={{
          ...
          paddingBottom:
            safeArea.bottom +
            (isRunningOnMac() || Platform.OS === "web" ? 8 : 0),
        }}
      >
        <AgentContentView
          content={content}
...
function InactiveArchivedHint(props: {
  resumeCommandBlock: NonNullable<
    ReturnType<typeof getResumeCommandBlock>
  > | null;
  canResume: boolean;
  resuming: boolean;
  onResume: () => void;
  ...
  return (
    ...
    >
      <View style={{ paddingHorizontal: 8, gap: 4 }}>
        <Text style={hintTextStyle}>{t("session.inactiveArchived")}</Text>
        {props.canResume
          ? null
      ...
      {props.canResume ? (
        ...
        props.resumeCommandBlock && (
          <ResumeCommandCopyBlock
            resumeCommandBlock={props.resumeCommandBlock}
          />
        )
      )}
    </View>
...
function ResumeCommandCopyBlock({
  resumeCommandBlock,
}: {
